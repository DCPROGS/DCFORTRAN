	subroutine VJDISP5(avcur,avcurcon,avolt,avoltcon,nsamp,nvjump,
...
c	real*4 xdisp(20480),ydisp(3,20480)
	...
	real*4    	xdisp(:),ydisp(:,:)
	integer   	itype(10)
	character 	title1*79,def*30,cans*30,getint*11
...
203	continue
	if(itype(ipuls).eq.1) then		!ramp
	   ...
	   call NEWPEN(15)		!white -for wrstring18 call
	   call wrstring18(8,64,
     &	'X-scale = '//ccal//' to '//ccal1//' mV',15,0)
	endif
c Plot data
	call NEWPEN(12)		!red -for wrstring18 call
	if(iplot.eq.1.and.naver.gt.0) then
	   do 2 i=1,nplot
	    call grapix(xdisp(i),ydisp(1,i),ix,iy)
	    call wpixel(ix,iy,12)		!drug=red
2	   continue
	   call wrstring18(8,464,'Agonist     ',12,0)
	endif
	if(naver.eq.0) call wrstring18(8,464,'(No agonist)',12,0)
	call NEWPEN(11)		!light blue -for wrstring18 call
	if(iplot.eq.1.and.navc.gt.0) then
	   do 3 i=1,nplot
	    call grapix(xdisp(i),ydisp(2,i),ix,iy)
	    call wpixel(ix,iy,11)		!con=light blue
3	   continue
	   call wrstring18(8,448,'Control     ',11,0)
	endif
	if(navc.eq.0) call wrstring18(8,448,'(No control)',11,0)
	call NEWPEN(14)		!yellow -for wrstring18 call
...
6	   continue
	...
	call NEWPEN(15)		!white -for wrstring18 call
	call wrstring18(8,64,'Y-scale = '//ccal//' to '//ccal1//unit,
     &	15,0)
	goto 999
c
c
...
154	   continue
	...
	if(naver.eq.0) call wrstring18(8,464,' (No agonist)')
	call NEWPEN(11)		!light blue -for wrstring18 call
	if(navc.gt.0) then
	   do 153 i=1,nsamp1
	    call grapix(xdisp(i),ydisp(2,i),ix,iy)
...
c	ipen=15
	...
	call gralin(xmin,ymax)     !y axis -now dark blue
	call NEWPEN(15)
	call REALTOCH(ymin,ccal,inchar)
	call REALTOCH(ymax,ccal1,inchar)
	call wrstring18(8,64,' Y-scale = '//ccal//' to '//ccal1//' mV'
     &  ,15,0)
...
999	continue
	...
	call NEWPEN(15)		!for wrstring18 call
	call wrstring18(8,32,'ANY KEY to continue',15,0)
	call ANYKEY
	goto 111
c	RETURN
	end
