...
c
	...
	integer nsc1(50),im1(50,100),jm1(50,100)	!needed for getrev
c	integer link(100,100)
...
80	format(
	...
	if(nmr.gt.0) then
	   ...
	   print 81,i
81	   format(
     & ' (1) Use spanning tree to locate cycles',/,
     & ' (2) Use ordered 4-state cycles',/,
     & ' Option number [',i2,'] = ')
	   call INPUTi(i)
	   useprim=i.eq.1
	endif
...
c=========TEMPORARY FIX
c	if(isetmr(1).lt.1.or.isetmr(1).gt.ncyc) then
c	   do i=1,ncyc
c		isetmr(i)=i
c	   enddo
c	endif
c=========END TEMPORARAY FIX
c
	if(useprim) then
	   ncyc=nmr
	   call GETREVT(ncyc,nsc,im,jm,ncyc1,nsc1,im1,jm1,k,
     &     titlep,npar,irate,jrate,.false.)
	else
c 	   Check for cubes in the model
	   call FINDCUBE()		!all args in commons
c 		Now call GETREV with both nsc1, im1() etc from .ini,
c		AND nsc, im() etc from QMECHS.
c 		GETREV now sets obeymr(i)=true if cycle #1 obeys MR
	   call GETREV(ncyc,nsc,im,jm,ncyc1,nsc1,im1,jm1,titlep,
     & 	npar,irate,jrate,.false.)
	endif
