	subroutine GMAT3(AB,QM,GAB,km,kr1,kr2,RECIP)
...
C
	allocatable::temp
	real*8 temp(:,:)
	real*8 QM(km,km),GAB(kr1,kr2),RECIP(km,km)
	real*8 one
	INTEGER AB
...
C GET SUBSCRIPT FOR QAA ETC
	II=I*10+I
C GET QII IN GAB
	call SUBMAT(QM,II,RECIP,km,km,kr1,kr2)
...
C GET -QII*TEMP=GAB IN GAB
	call MATMUL(RECIP,TEMP,GAB,k1,k1,k2,-one,km,km,km,km,kr1,kr2)
c
	DEALLOCATE(temp)
	RETURN
	END
