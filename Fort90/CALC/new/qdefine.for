...
c When kcon>0 the statenames are in statename(kstat+1,kstat+kcon)
...
c		new state #i corresponds to old state is0(i)
	   allocate(is0(k))	!size=new number of states
	   do i=1,k
		is0(i)=0
	   enddo
	   print 743
...
1121	   format(
     &	' Number of subunits = ')
	   ...
	   print 112
112	   format(
     &	' Number of states in each subunit = ')
	   call INPUTi(kstat0)
	   kstat=nsub*kstat0
...
c		kstat=kstat+kcon	! NO -keep separate
	   ...
	   print 1
1	   format(
     &	' Number of rate constants within each subunit = ')
	   call INPUTi(npar0)
	   npar=nsub*npar0
	   if(kcon.gt.0) then
		npar1=2*kcon
...
c	  Convention is that statname() contains names for subunit 1, followed
     ...
     &  ' (names have up to 10 characters)')
		...
		if(.not.altmod) then
		   ...
		   do i=1,ks0old	!ks0old=number defined in model being modified
			i1=i+(m-1)*kstat0
			cdata(i,1)=statname(i1)	!use old names for basic states
		   enddo
		   do i=1,ks0old,kstat0		!zero any undefined ones
			cdata(i,1)='          '
		   enddo
		endif
		call CLS() 	!clear screen
...
c   Call inwindc again to define names for concerted states, if any
      if(altmod.and.kcon.gt.0) then
	   print 833,kcon,kstat+1,kstat+kcon
833	   format(/,
     &   ' Now define names (eg A2R) of the ',i2,
     &	' concerted states (states #',i2,' - ',i2,')')
	   ...
	   iflagc=3		!show data
	   do i=1,kcon
		cdata(i,1)=statname(i+kstat)	!use old names for basic states
	   enddo
	   ni0=kcon	!rows
	   nj0=1		!columns
	   nidisp=kcon
     ...
     & 	nr1,nc1,iflagc,iconst,nd1,nd2,0)
	   do m=1,kcon
		statname(m+kstat)=cdata(m,1)(1:10)
	   enddo
	endif
c
...
c Now 'concerted trans' rates, npar1=2*kcon values
	   if(npar1.gt.0) then
		if(altmod) then
	         do i=1,npar1
			i1=i + nsub*2*npar0  !index in titlep of concerted rate #1, 2 ...
			cdata(i,1)(1:10)=titlep(i1)
	   	   enddo
		endif
		ni0=npar1		!rows
...
c		else if(altmod) then
c		   i0=irate(m)
c		   j0=jrate(m)
c		   if(mod(m,2).eq.0) then	!default
c===
c			i=j0
c			j=i0
c		   else
...
c Not normally possible to read in rates from an old model when new one
c has been defined so remove all of this
c	ans='Y'
c	if(newmod) ans='N'
c	print 301,vref,ans
c301	FORMAT(
c     &' Read rate constants (at Vref=',f4.0,'mV) from disc [',a2,'] ? ')
c	call INPUTa(ans)
c	readp=UC(ans).ne.'N'
c	call QZERO(QT,k)		!in case not read
c	if(.not.readp) goto 330
cC Read Q etc from disc	(default for read/write defined above as always
cc write but may not always read)
c304	continue
cc column headings
cc Modif 11/18/99 09:20am so prints only the sets of rates that correspond
cc to the specified model (imod0)
c	n=0
c	do i=1,nrecs
c	   if(jmod(i).eq.imod0) n=n+1
c	enddo
c	if(n.eq.0) then
c	   print 191
c191	   format(' No rate constants have been stored for this model')
...
c		   print 192,i,rtitles(i)
c192		   format(1x,i3,': ',a74)
c		   irec=i	!use as default
c		endif
c	   enddo
c	   print 15,irq
c15	   format(' File # for rates [',i2,'] = ')
c	   call INPUTi(irq)
c	   irec=jstart(irq)
cc	   imodsav=imod0
cc===	   jrecq=2*irecq-1
...
c      OPEN(unit=15,file=qfilem,status='UNKNOWN',
c     &	access='TRANSPARENT')
cc  NB read model number as imod1
c	read(unit=15,rec=irec) iver,
c     &	imod1,mtitle,k,kA,kB,kC,kD,rtitle,ilast,jlast,nrateq,
c     &	ncon,ncdep,nlig,chardef,boundef,ncyc,vref,nvdep,kmfast,
c     &	indmod,npar,nsetq,kstat,
c     &	((charmod(i,j),i=1,ilast),j=1,jlast),
c     &	(irate(i),i=1,nrateq),(jrate(i),i=1,nrateq),
...
c98	if(imod1.ne.imod0) then
c	   call BELL(2)
c	   print 157,imod1,imod0
c157	   format(
c     &  ' Model on disc (#',i3,') is not that requested (#',i3,')',/,
c     &  ' (1) Type in new rate constants',/,
c     &  ' (2) Try again (e.g.different disc)',/,
c     &  ' (3) Use anyway (may be OK, eg if model # have been changed',/,
c     &  ' Option number [2] = ')
c	   call INPUTi(i)
c	   if(i.le.0) i=2
c	   if(i.eq.1) then
