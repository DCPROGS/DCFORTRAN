	subroutine GETQD(QT,irate,jrate,nchan,dgamma,vkin,vhold,vref,
...
c	iver = version number =101 initially, 102 now
...
c If not call QCONVERT to look for old files and create qmwch.dat.
	if(.not.readini) qfilem='QMECHS.DAT'
	qfilmsav=qfilem
...
26	format(
     ...
     &  ' (5) List all the models in QMECHS.DAT (new version)',/,
     &  ' (6) Exit',/,
     &  ' Option number [',i2,'] = ')
	call INPUTi(iread)
c
	...
	if(iread.eq.1) then
	   if(.not.samefil) then
		...
		print 264
264		format(
     &  ' Cannot use values from .ini because different qmechs.dat',
     &  ' specified',/)
		iread=2
		goto 263
	   else
...
c Nodified for iver=102, and some old debugging stuff removed
	if(iread.eq.5) then
	   print 210,nrecs
	   if(discprt) write(8,210) nrecs
210	   format(
     &   ' QMECHS.DAT contains ',i4,' records of rate constants +model')
...
c parameters
     ...
     &	indmod,npar,nsetq,kstat
	   ...
	   if(iver.ne.102) then
		call BELL(1)
		print 78,iver
78	   format(' ERROR in GETQD: iver = ',i4,' (should be 102)')
	   endif
	   read(unit=15,rec=irec) iver,
     &	imod,mtitle,k, kA, kB, kC, kD,rtitle,ilast,jlast,nrateq,
     &	ncon,ncdep,nlig,chardef,boundef,ncyc,vref,nvdep,kmfast,
...
c GETREV now sets obeymr(i)=true if cycle #1 obeys MR
	call GETREV(ncyc,nsc,im,jm,ncyc1,nsc1,im1,jm1,useini)
...
c===this QD not used at present, but need to call QSETD here to set
...
c
	...
	nlvar=1
	if(nlig.gt.1.and.ncdep.gt.0) then
	   print 590,nlvar
590	   format(' Agonist (to calc EC50) is ligand number [',i2,'] = ')
	   call INPUTi(nlvar)
	endif
c 02/13/02 09:24pm argument plotcur added to eqec50 for scalcs; set true here
	call EQEC50(EC50,curinfs,monot,curmax,concmax,cur0,pop0,QT,
     & 	conc1,nlvar,vhold,vref,cfacd,dgamma,.true.,k,kdim)
	print 593,cur0,pop0
