...
c			if(discprt) write(8,141) j,ligname(j),
c     &			conc(j,1)*1.e6
141			format('  (conc of ligand #',i2,',  ',a10,' = ',
     &			g13.6,' micromolar)')
		   enddo
		   if(ifix.eq.1) then
			n=nlvar
			if(n.eq.0) n=1
			xs=sngl(ec50*1.d6)		!micromolar default
		   else
			n=nlvar2
...
271		   continue
c		   print 27,n, ligname(n)
c27		   format(/,
c     &    ' EC50 specified for ligand # ',i2,'(',a10,'), O.K.  [Y] ? ')
c		   call INPUTa(ans)
c		   if(ans.eq.'N') then
			print 28,nlig
28			format(
     &    	 ' Specify EC50 for ligand number (1 to ',i2,') = ')
			call INPUTi(n)
c			goto 271
c		   endif
		   print 292,ligname(n),xs
292		   format(
...
291		   format(
     ...
     & ' micromolar',/,'  at concentrations of other ligands: ')
		   do j=1,nlig
			if(j.ne.nlvar.and.j.ne.nlvar2) then
			   print 144,j,ligname(j),conc(j,1)*1.e6
			   if(discprt)write(8,144) j,ligname(j),
     &			   conc(j,1)*1.e6
...
c221				format(
c     & ' Element ',i2,',',i2,' is fixed by microscopic reversibility',/,
c     & ' (cycle #',i3,'). Do not constrain it with EC50! Start again.')
				goto 25
			   endif
			enddo
		   endif
