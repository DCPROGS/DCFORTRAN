...
c	real*8 covlog(200,200),varlog(200)							!for HMAT
...
c For ec50 constraint
	...
	real*4 xs
	real*8 ec50,xqlo,xqhi,ec50out,ec50out1,pmax
...
59	continue	!return after getqd2 with idest=59 to try another file
	call GETQD1(QT,irate,jrate,nchan,dgamma,vkin,vhold,vref,
     ...
     &  useprim,imodsav,dcmod,jcon,theta0,conc,jfix,jmic,nmr)
...
c when PRIM3 is used for MR
	if(.not.useprim) then
     ...
     &	theta0,obeymr,.true.,npar,km)
	   call SETEC50(readini,imod0,dcmod,imodsav,nlig,
     &	conc,titlep,IQ)
	endif
c
c Used fixed EC50 as constraint?
c Move ec50 constraints to subroutine SETEC50 so they can be set
c before MR (in getqd1 when PRIM3 is used for MR
c Now uses modec50 when dcmodel is true, ec50_hjc (model indep) otherwise
c so can fix ec50 whatever the model
c Modified 10/08/03 08:14am to allow ec50 to be fixed for two ligands, via
c	common/ec4/nfixec50,nlvar2,ec502,i502,j502,m502,
c     &     prtec502,xqlo2,xqhi2	 !for 2nd fixed ec50
c25	continue
...
cc     &' ------------------------------------------------------------')
c	   ans='N'
c	   if(readini.and.fixec50.and.imod0.eq.imodsav) ans='Y'
c	   call DCASK(' Specify EC50 (no block) as constraint',ans,ans)
c	   if(ans.eq.'N') then
c		fixec50=.false.
c		nfixec50=0		!number fixed by ec50 (0 or 1 or 2)
c	   else
c		fixec50=.true.
cc
c		if(dcmodel) then
...
c		   print 19,nmod,ans
c19		   format(
c     &  ' Check: is this model ',i3,' in DC''s QMECHS file [',a1,'] ? ')
c		   call INPUTa(ans)
c		   if(ans.eq.'N') dcmodel=.false.	!don't use modec50
c		endif
c		if(nlig.eq.1) then
c		   nlvar=1
c		   nfixec50=1		!number fixed by ec50 (0 or 1 or 2)
c		else if(nlig.gt.1) then
c		   ans='N'
c		   if(nfixec50.eq.2) ans='Y'	!not in .ini at present
c		   call DCASK('Constrain EC50 for TWO agonists',ans,ans)
c		   if(ans.eq.'N') then
c			nfixec50=1		!number fixed by ec50 (0 or 1 or 2)
c		   else
c			nfixec50=2		!number fixed by ec50 (0 or 1 or 2)
c		   endif
c		endif
cc
...
c			n=nlvar
c			if(n.eq.0) n=1
c			xs=sngl(ec50*1.d6)		!micromolar default
c		   else
c			n=nlvar2
c			if(n.eq.0) then  	!pick a value different from nlvar
c			   do j=1,nlig
c				if(j.ne.nlvar) then
c				   n=j
c				   goto 2712
c				endif
c			   enddo
c			endif
c2712			xs=sngl(ec502*1.d6)		!micromolar default
c		   endif
c271		   continue
cc		   print 27,n, ligname(n)
cc27		   format(/,
cc     &    ' EC50 specified for ligand # ',i2,'(',a10,'), O.K.  [Y] ? ')
cc		   call INPUTa(ans)
cc		   if(ans.eq.'N') then
c			print 28,nlig
c28			format(
c     &    	 ' Specify EC50 for ligand number (1 to ',i2,') = ')
c			call INPUTi(n)
cc			goto 271
cc		   endif
cc		   print 292,ligname(n),xs
c292		   format(
c     &  ' EC50 for ',a10,' (without block) (micromolar) [',f9.4,'] = ')
c		   call INPUTr(xs)
c		   if(ifix.eq.1) then
c			nlvar=n
c			ec50=dble(xs*1.e-6)		!molar
c		   else
c			nlvar2=n
c			ec502=dble(xs*1.e-6)		!molar
c		   endif
c		   print 291,n,ligname(n),xs
c		   if(discprt) write(8,291) n,ligname(n),xs
c91		   format(
c     & ' EC50 for ligand # ',i2,'(',a10,')  constrained to be ',g13.6,
c     & ' micromolar',/,'  at concentrations of other ligands: ')
c		   do j=1,nlig
c			if(j.ne.nlvar.and.j.ne.nlvar2) then
c			   print 144,j,ligname(j),conc(j,1)*1.e6
c			   if(discprt)write(8,144) j,ligname(j),
c     &			   conc(j,1)*1.e6
c144			   format('  (conc of ligand #',i2,',  ',a10,' = ',
c     &			 g13.6,' micromolar)')
c			endif
c		   enddo
cc
cc Now get i,j for the q(i,j) to be fixed by ec50
c		   if(ifix.eq.1) then
c			i=i50
c			j=j50
c		   else
...
c		   print 21,i,j
c21		   format(
c     & ' Use EC50 to constrain rate constant from state i to state j = '
c     & ,/,'   specify: i, j [',i3,',',i3,'] = ')
c		   call INPUT2i(i,j)
cc	Check this is not already fixed via mr, or constrained
c		   if(ncyc.gt.0) then
c			do m1=1,ncyc
c			   m=isetmr(m1)	!actual cycle number
c			   if(i.eq.im(m,1).and.j.eq.jm(m,1)) then
c				call BELL(1)
c				print 221,i,j,m
cc221				format(
cc     & ' Element ',i2,',',i2,' is fixed by microscopic reversibility',/,
cc     & ' (cycle #',i3,'). Do not constrain it with EC50! Start again.')
c				goto 25
c			   endif
c			enddo
c		   endif
c		   if(neq.gt.0) then		!check constraints
c			do n=1,neq
c			   if(i.eq.ie(n).and.j.eq.je(n)) then
c				call BELL(1)
c				print 222,i,j
c222				format(
c     & ' Element ',i2,',',i2,' is already constrained.',/,
c     & ' Do not constrain it with EC50!  Start again.')
c				goto 25
c			   endif
c			enddo
c		   endif
...
c			j502=j
c		   endif
cc         Set limits to constrained rate constant to prevent silly values
c		   if(ifix.eq.1) then
c			x1=sngl(xqlo)
c			x2=sngl(xqhi)
c		   else
...
c			x2=sngl(xqhi2)
c		   endif
c		   print 226,i,j,x1,x2
c226		   format(
c     & ' Lower and upper limits for q(',i2,',',i2,') [',
c     &  g11.4,',',g11.4,'] = ')
c		   call INPUT2r(x1,x2)
c		   if(ifix.eq.1) then
c			xqlo=dble(x1)
c			xqhi=dble(x2)
c			m50=IQ(i50,j50)		!OK so define param #
c			m5=m50            	!for print
c			xs=1.e6*sngl(ec50)      !for print
c		   else
c			xqlo2=dble(x1)
c			xqhi2=dble(x2)
c			m502=IQ(i502,j502)		!OK so define param #
c			m5=m502			!for print
c			xs=1.e6*sngl(ec502)      !for print
c		   endif
c		   print 22,m5,i,j,titlep(m5),xs
c22		   format(' rate ',
c     & i3,3x,' q(',i2,',',i2,') =',1x,a10,' is constrained to give',/,
c     & ' an EC50 = ',g16.6,' micromolar: O.K. [Y] ? ')
c		   ans='Y'
c		   call INPUTa(ans)
c		   if(ans.eq.'N') goto 25
c		enddo		!ifix=1,nfixec50
c
c		penfac=10.0d0
c		x1=sngl(penfac)
c		print 227,x1
c227		format(' Factor for penalty function [',f7.1,'] = ')
c		call INPUTr(x1)
c		penfac=dble(x1)
cc
c	   endif	!end of fixec50=true
c	endif		!end of ncdep.gt.0
c
...
c EC50 now printed AFTER the rate constants
	kdim=100
c	if(dcmodel) then
c	   call MODEC50(imod0,QT,npar,kdim,pmax,ec50out)
c	   call EC50_HJC(EC50out1,curinf,curmax,concmax,cur0,pop0,QT,
c     &      conc,k,kdim)
c	else
c	   call EC50_HJC(EC50out,curinf,curmax,concmax,cur0,pop0,QT,
c     &      conc,k,kdim)
c	endif
c	print 346,1.d6*ec50out
c	if(discprt) write(8,346) 1.d6*ec50out
c346	format(
c     & ' Final parameters correspond to EC50 = ',f9.3,' micromolar')
c	if(dcmodel) then
c	   print 3461,1.d6*ec50out1
c	   if(discprt) write(8,3461) 1.d6*ec50out1
c3461	   format(
c     & ' Check: EC50 by numerical calculation = ',f9.3,' micromolar')
c	endif
c	if(fixec50) then
c	   print 345,i50,j50,1.d6*ec50
c	   if(discprt) write(8,345) i50,j50,1.d6*ec50
c345	   format(
c     &'    Q(',i2,',',i2,') calculated from EC50 = ',f9.3,
c     &' micromolar')
c	   if(penalty) then
c	      print 344,penfunc
c            if(discprt) write(8,344) penfunc
c344	      format(' Penalty function in effect: ',f8.3,
c     &        ' log likelihood units')
c	   endif
c	endif
c
	if(.not.autosim) then
...
590		format('  Error in bisection for EC50: nerr = ',i2)
	   ...
	   print 593,ligname(nlv),cur0,pop0
	   if(discprt) write(8,593) ligname(nlv),cur0,pop0
593	   format(
     & ' At zero concentration of ',a10,', current (pA) = ',g12.5,
     &	', Popen = ',g12.5)
	   if(monot) then
		print 594,curinfs,pmax,ligname(nlv)(1:10),ec50out*1.e6
      	if(discprt) write(8,594) curinfs,pmax,ligname(nlv)(1:10),
     &		ec50out*1.e6
594		format(
...
592		format(
     ...
     &   ' --------------------------------------------------')
   	   ...
   	   if(dcmodel.and.nlig.eq.1) then
		print 3461,1.d6*ec50out1
		if(discprt) write(8,3461) 1.d6*ec50out1
3461		format(
     & ' Check: EC50 by explicit calculation = ',f9.3,' micromolar')
	   endif
...
c		   m=m502
		...
		if(discprt) write(8,345) i,j,ligname(n),1.d6*ec
345	      format(
     &'    Q(',i2,',',i2,') calculated from EC50 (',a10,') = ',f9.3,
     &' micromolar')
	   enddo		!ifix=1,nfixec50
	   if(penalty) then
	      print 344,penfunc
...
c For autosim need to fix to keep ec50s for 2 ligands
     ...
     &	  pmax,nlvar,QT,conc,k,kdim,nerr)
		if(nerr.ne.0) then
		   call BELL(1)
		   print 590,nerr
		   if(discprt) write(8,590) nerr
...
c and restore after errors calc
	   if(allocated(QTsav)) DEALLOCATE(QTsav,QDsav,thsav1)
...
c1215		   format('& (from EC50=',f8.3,' muM)')
	         ...
	         print 1215,1.d6*ec50out2
	         if(discprt) write(8,1215) 1.d6*ec50out2
c1215		   format('& (from EC50=',f8.3,' muM)')
		else if(jfix(m).eq.1) then
      	   print 1214
      	   if(discprt) write(8,1214)
...
301		format(/,'  FINAL VALUES WITH ERRORS FROM log(rate)',/,23x,
     &	'  initial         final         CV(%)      approx SD')
c     &	'  initial         final       approx SD      CV(%)')
	   else
      	print 311
      	if(discprt) write(8,311)
311		format(/,'  FINAL VALUES WITH ERRORS FROM log(rate)',/,27x,
     &	'  true        initial        final        approx CV(%)')
c     &	'  true        initial        final        approx SD')
	   endif
	   do m=1,npar
		i=irate(m)
		j=jrate(m)
