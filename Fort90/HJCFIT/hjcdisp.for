...
c
c Modif 11/21/06 03:57pm so can replot (especially shut time pdf) showing data
c curves for shut times beyond tcrit.  Normally when tcrit used (burst(j)=true)
c only shut times =< trit are binned, but when rescalex=.true. bin all the
c shut times up to new xmax

c Modif 08/08/04 02:50pm to plot Popen curves, and real*8 ec50out added
c as parameter, as guide to conc range to use
c
c MODIF 06/05/02 05:16pm for version that pre-defines open PERIODS and puts
c them into tint() etc, (in GETOPER) before any fitting done, so no need to
c find open periods here -already don
c
...
c	logical exass
	logical fitted,open,bad,bad1,nores,scale,setmax,btest
c	logical good
c	logical first
...
c	real*4 tau1(100),area(100),amo,ams,sd
c==from scvd1
...
c
	...
	real*8 time,tres,FTCT,ttop2,ptc,ucol(100,1)
	real*8 tresd(10)
...
c for data
	...
	real*4 syms(5)
c for calc curves
	integer ncal1(6),icurvc1(6),iline1(6)
...
c=	common/absave/nab,alpha2,jalpha,beta2,jbeta,aloglik
  	real*8 alpha2(kab),beta2(kab),aloglik(kab),sm
	common/absave/nab,jalpha,jbeta		!in hjclik, hjcdisp
c for fast block correction
	logical*4 fastblk
	real*4 aKB
...
2271	   format(/,
	...
	ipmax=7	!no of options
	i=iplot
	call INPUTi(i)
	if(i.ge.1.and.i.le.ipmax) iplot=i
	if(iplot.eq.ipmax)  goto 999
c Plots to be done, so specify which data set (if nset>1) and recalc values
c (apart from iplot=6 which is independent of set #)
c BUT if iplot=6 the set number DOES matter when nlig>1 (because conc of
c other ligands may effect ec50 etc), so ask for set number before goto 86
c when nlig>1.
c
...
62	   format(' Data set to be plotted (1 - ',i3,') [',i3,'] = ')
	   ...
	   if(iset.ne.isetlast) hjmcalc=.false.	!recalc means
	   tres=tresd(iset)
	   tres1=sngl(1.d3*tres)	!tres in msec
	   if(tres.eq.0.d0) tres1=0.0001
	   treso=tres1
...
c
199	continue	!return to get shut times up to new xmax when rescalex=.true.
c
...
c In this verion open periods have already been defined in tint(), ampl(),
c iprops() (in GETOPER).
c========new version
	topen=tint(in,iset)
	open=ampl(in,iset).ne.0.
	if(.not.open) pause 'Error 1 in hjcdisp'
	...
	iopen=in	!index in tint of the current opening
	inext=in+1	!index of shutting that ends open period

c=========problem -need iofst,iolast,isfst,islast
c========problem -although tint() etc now contain alternating open-period/shut
c========time values,  not all open periods will form part of the groups/clusters
c========that were fitted -some will have been omitted eg because of
c========bursts aborted by a bad shut time -so should pdf show all open periods?
c========or only the ones that were used (old version was latter)
cc=	top=0.	!to accum length of open period
c=	i1=in
c=	iofst=-1	!records index in tint() of 1st opening in the open period
c=	do 51 jn=i1,nint(iset)
c=	   open=ampl(jn,iset).ne.0.
c=	   good=.not.BTEST(iprops(jn,iset),3)	!tint(i) was unusable (bit 3='8' set)
c=	   if(open) then
c=		if(good) then
c=		   top=top+tint(jn,iset)
c=		   if(iofst.eq.-1) iofst=jn !index in tint of 1st opening in open period
c=		   iolast=jn		!index in tint of last opening in open period
c=		else		!bad opening, so end group
c=		   in=jn
c=		   if(burst(iset)) then
c=			goto 93	!abandon whole burst & look for next
c=		   else
c=			goto 91	!end group at end of prev shutting
c=		   endif
c=		endif
c=	   else
c=		inext=jn	!index of shutting that ends open period
c=		goto 52	!shut, so jump out
c=	   endif
c=51	continue
c=c 	Get here if last opening reached (and it is good), but in this case
c=c 	a gap is not found and inext not updated. Therefore set inext=nint+1 to
c=c 	signal (below) that end reached
c=	inext=nint(iset)+1
c=c
...
c skipped a bad burst in between), which will be so if ioopen=islast+1
	if((corprev.or.iplot.eq.4).and.
     &    islast.gt.0.and.iopen.eq.islast+1) then
...
c		if(nr.eq.nrspec) then
c		   print 80,nrspec,in,jn,tshut,topen
		   print 80,nr,in,jn,tshut,topen
...
c Check for 2 adjacent gaps, or bad gap
	...
	tshut=tint(in,iset)
c
c In BURST case, check if tshut> Tcrit
c When rescaling x axis for shut times (rescalex=.true.) want all shut times
c to be binned, not only those below tcrit)
...
c	 for correlation,in burst case
     ...
     &   .and.in.eq.iopen+1) then
c check this!
c     &   .and.in.eq.iolast+1) then
...
c if xmax increased then count all shut times, not just thos < trcrit
	   if(rescalex) then
		if(tshut.gt.xmax) goto 92	!end present group with prev opening
	   else
		goto 92
	   endif
	endif
...
c
	...
	if((cornext.or.iplot.eq.4).and.
     &	islast.eq.iopen+1) then
c     &	islast.eq.iolast+1) then
	   call GETRANGE(tshut,topen,ylo,yhi,nrange,sy1,syy1,ny1,
     &    sy2,syy2,ny2,sx1,sxx1,nx1,sx2,sxx2,nx2,deb,nr)
	   if(nr.eq.0) nr0=nr0+1	!tgap not in any range
	   if(idebug.eq.20) then	!debug
		nrspec=nr		!what is nrspec?
		if(nr.eq.nrspec) then
		   print 80,nrspec,in,jn,tshut,topen
		   if(discprt) write(8,80) nrspec,in,jn,tshut,topen
c80		   format(' tshut in range ',i5,/,
...
c (this could be simplified, now that open periods predefined in GETOPER)
...
c=	else if(iplot.eq.3.or.iplot.eq.5) then
	...
	xlo=-1.
c
c Separate section for iplot=4,6 here (use VPLOT)
86	continue
	autplt=.false.
	draft=.false.
	plotonly=.false.
...
c
c for Popen curves (iplot=6)
c  calc Popen at each conc, hjc in j=1, ideal in j=2, nH HJC in j=3
c nH ideal in j=4
c  need to put conc in conc(i,1) (original conc(i,j) saved)

c For ideal P(open) no need to calc whole pdf, just occs
	if(iplot.eq.6) then
	   ...
	   enddo
c
	   if(hjmcalc) goto 50	!ec50, ycal1() etc already calculated
c If more than one ligand, do ec50 for all, then choose which ligand to
c plot the curves for.
	   do nlv=1,nlig
		call EC50_HJ2(EC50,concmax,pop0,
     & 	  pmax,nlv,iset,QT,conc,k,nerr,monot,decline,
...
590		   format(
     &	'  Error in bisection for EC50 (EC50_HJ2): nerr = ',i2)
		endif
		print 593,ligname(nlv),pop0,p0hjc
		if(discprt) write(8,593) ligname(nlv),pop0,p0hjc
593		format(/,
     & ' At zero concentration of ',a10,/,
     &	', P(open) (ideal) = ',g12.5,', P(open) (HJC) = ',g12.5)
		if(monot) then
		   print 594,ligname(nlv)(1:10),pmax,pmaxhjc,
     &	   ligname(nlv)(1:10),ec50*1.e6,ec50hjc*1.e6
      	   if(discprt) write(8,594) ligname(nlv)(1:10),pmax,pmaxhjc,
     &	   ligname(nlv)(1:10),ec50*1.e6,ec50hjc*1.e6
594		   format(
     & ' Equilibrium response-concentration for ',a10,' (monotonic)',/,
     & ' Maximum P(open) (ideal) = ',g11.4,
     & ' Maximum Popen (HJC) = ',f9.6,/,
     & '    EC50 for ',a10,/,
     & '    EC50 (ideal) = ',g11.4,'    EC50 (HJC) = ',g11.4)
c     &   ' --------------------------------------------------')
c
c		   pause
		endif
		if(nlig.gt.1) then	!print conc of other ligands
		   do n=1,nlig
			if(n.ne.nlv) then
			   print 5941,ligname(n)(1:10),concsav(n,iset)*1.e6
			   if(discprt) write(8,5941)
     &			ligname(n)(1:10),concsav(n,iset)*1.e6
...
c
c	   nlvar=1			!set above
	   if(nlig.gt.1) then	!which ligand conc to be plotted
		print 108
c		if(discprt) write(8,108)
		do i=1,nlig
		   print 65,j,i,ligname(i),1.e6*conc(i,j)
c65	         format(' Set ',i2,': conc (muM) of ligand ',i2,' (',
c     &	      a10,') = ',g11.4)
		enddo
		print 502,nlvar
502		format(' Ligand number to plotted [',i2,'] = ')
		call INPUTi(nlvar)
		if(nlvar.ne.nlvlast) hjmcalc=.false.	!recalc means
		nlvlast=nlvar
	   endif
c print only if monotonic, for now
...
50	   print 591,iopth
591	   format(
     & ' (1) Plot P(open) against concentration',/,
     & ' (2) Plot Hill slope for (1) against concentration',/,
     & ' (3) Plot Hill slope for (1) against P(open)',/,
     & ' (4) Choose another plot',/,
     & ' Option number [',i2,'] = ')
	   call INPUTi(iopth)
c
c	   ncalc=256
	   ...
	   ALLOCATE(XVAL1(ndv1,ndimd1),YVAL1(ndv1,ndimd1),
     &	XCAL1(ndc1,ndimc1),YCAL1(ndc1,ndimc1))
	   ALLOCATE(weight(kwi,kwj),icurvw(ndimd1))	!for VPLOT5
	   do i=1,ndimd1
		icurvw(i)=-1	!no SD unless reset below
	   enddo
	   if(iopth.eq.1) then
c		titlex='Conc (micromolar)                       '
...
c Make QD for the specified concentration(s)
	   ...
	   x1=xmin*1.e6			!micromolar
	   x2=xmax*1.e6			!micromolar
	   ...
	   print 508,x1,x2
508	   format(
     &    ' Plot between conc (micromolar) [',f8.2,',',f8.2,'] = ')
	   call INPUT2r(x1,x2)
	   xmin=x1*1.e-6	!molar
	   xmax=x2*1.e-6
...
c
	   ...
	   if(.not.hjmcalc) then	!values not yet calc
		print 511
511		format(' Calculating HJC means  . . . . ')
		do i=1,ncalc
		   x=xmin*(10.**(float(i-1)*dx))	!molar
		   xcal1(i,1)=1.e6*x		!micromolar for axes
		   xcal1(i,2)=1.e6*x		!micromolar
c NB set number DOES matter if nlig>1 and conc of the other ligands
c differs from one set to another
c
c		   conc(nlvar,1)=x		!molar for calculations
c		   iset=1
		   conc(nlvar,iset)=x		!molar for calculations
		   call QNEW_HJC(QT,iset,conc,QD,ncdep,nlig,
     &		IL,IX,JX,k,km,km)
		   if(debug()) then
		      call DATYP(QT,'QT =    ',.false.,k,k,km,km)
		      call DATYP(QD,'QD =    ',.false.,k,k,km,km)
		   endif
c  use the current QD to get Popen at each conc (phiA, QEXPQA etc
c   are all recalculated locally with HJCMEAN, for the current QD)
		   call HJCMEAN(QD,tres,hmopen,hmshut,
     &  	     k,kA,kF,kAm,kFm,km)
		   ycal1(i,1)=sngl(hmopen/(hmopen+hmshut))	!HJC Popen
		   call EQOC_HJC(QD,peq1,k,km,km)
		   s=0.0
		   do m=1,kA
			s=s+ sngl(peq1(m))
		   enddo
		   ycal1(i,2)=s		!ideal Popen
c In fastblk case, correct Popen for unresolved block
c But this is already in QD
c		   if(fastblk.and.(nlvar.eq.iflig)) then
c			x1=1.0 + conc(iflig,iset)/aKB
c			ycal1(i,1)=ycal1(i,1)/x1	!HJC Popen
c			ycal1(i,2)=ycal1(i,2)/x1	!ideal Popen
c		   endif
		enddo
c Now calc Hill slope values in ycal1(i,j), j=3 (HJC), j=4 (ideal)
c  Recalculate pmax etc for the current ligand (#nlvar) [could also save, above
c the pmax etc for each ligand]
		call EC50_HJ2(EC50,concmax,pop0,
     & 	  pmax,nlvar,iset,QT,conc,k,nerr,monot,decline,
     & 	  tres,kA,kF,kAm,kFm,km,p0hjc,pmaxhjc,ec50hjc)
	      call HJC_Hill(xcal1,ycal1,ncalc,nchill,h50,h50hjc,
     &       decline,pmax,pop0,ec50,p0hjc,pmaxhjc,ec50hjc,ndc1,ndimc1)
		hjmcalc=.true.		!so calcs not repeated
	   endif
	   print 592,iset,h50,h50hjc
	   if(discprt) write(8,592) iset,h50,h50hjc
592	   format(' SET #',i2,
     &  ':  Hill slope at EC50 = ',f9.4,' (ideal), or ',f9.4,' (HJC)',/,
     &   ' --------------------------------------------------',/)
c	   pause
c
	   do i=1,nlig
...
c Fir iopt=3 must replace X axis with P(open)
	   ncal1(1)=ncalc
	   ncal1(2)=ncalc
	   ncal1(3)=nchill
	   ncal1(4)=nchill
...
c Add observed Popen values for each set as data?
	   ncurvd=0
	   ndat1(1)=0
	   isym(1)=-7	!circle for data
	   ijoin1(1)=-1	!not joined
c
	   if(iopth.ne.1) goto 504
c
	   ans='N'
	   call DCASK(' Add observed P(open) points to the graph',ans,ans)
	   if(ans.eq.'Y') then
		ip=2
		print 510,ip
510		format(/,
     &  ' (1) Read P(open) values from a CVFIT file',/,
     &  ' (2) Type in the P(open) values now',/,
     &  ' Option number [',i2,'] = ')
		call INPUTi(ip)
		if(ip.eq.1) then
		   call CVREAD(xval1,yval1,weight,icurvw,ndat1,
     &		ndv1,ndimd1,kwi,kwj)
		   ncurvd=1
		   icurvd1(1)=1
		   ...
		   print 505
505		   format(' Number of points to be added [0] = ')
		   call INPUTi(ndat1(1))
		   ncurvd=1
		   icurvd1(1)=1
		   print 5051
5051		   format(' Specify SD for each point [N] ? ')
		   ans='N'
		   call INPUTa(ans)
		   if(ans.eq.'Y') icurvw(1)=1
		   if(ndat1(1).gt.0) then
	     		ncurvd=1
			icurvd(1)=1
			do i=1,ndat1(1)
			   print 506,i
506			   format(
     &		' Point #',i3,' Concentration (micromolar) = ')
			   call INPUTr(xval1(i,1))
			   print 507
507			   format('     Observed P(open) = ')
			   call INPUTr(yval1(i,1))
			   if(icurvw(1).eq.1) then
				print 5071
5071				format('     SD for this point = ')
				call INPUTr(sd)
				if(sd.gt.1.e-18) then
				   weight(i,1)=1.0/(sd*sd)
				else
				   weight(i,1)=0.0
				endif
			   endif
	   ...
	   endif	!end of entering observed Popen
504	   print 509
509	   format(/,
     &  ' Curve 1 (blue): HJC (exact) prediction of P(open)',/,
     &  ' Curve 2 (red): ideal (zero resolution) prediction of P(open)')
	endif	!end if iplot.eq.6
c
861	continue
	if(iplot.eq.4.or.iplot.eq.6) then	!call vplot
	   ilog=1		!y vs log(x)
	   iscal=1		!scale internally
	   itrace=0		!not multiple traces
	   if(nset.gt.1) then
		itit=1
		call INTCONV(iset,cnum)
		title1(1:6)='Set '//cnum(1:2)
	   endif
...
c Call with thetas() and kmax=kth (not used because ifitype=0)
     ...
     &   isetcol,itrace,ndv1,ndc1,weight,kwi,kwj,icurvw,kth,iver)
	   goto 994			!where next?
	endif		!end of diversion for iplot=4 and 6
c
...
13	   format(
     ...
     & ' (2) Show exact AND asymptotic distributions ',/,
     & ' (3) Show asymptotic distribution and its components also ',/,
     & ' Option number [',i2,'] = ')
	   i=icomp
	   call INPUTi(i)
	   if(i.ge.1.and.i.le.3) icomp=i
c Now allow NORES for any icomp (if requested, zero res always in j=2)
c	   if(icomp.eq.1.or.icomp.eq.2) then
...
c
c====fix scaling of calc curves, and reset of xmax to include all obs====
...
c	   tcrit2=dble(tcrit(iset))*1.d-3	!in sec
	   ttop2=dble(tcrit(iset))*1.d-3	!in sec
	   if(rescalex) then
c if xmax used here must take antilog because log(xmax taken above at
c	if(logt) then ...   xmin=0.010001
c
	     ttop2=dble(10.**xmax)*1.d-3	!in sec (NB log(xmax) taken above)
	   endif
c	   Ptc=FTCt(tcrit2,tres,k,kA,kF,ucol,
	   Ptc=FTCt(ttop2,tres,k,kA,kF,ucol,
...
c	   print 603,tcrit(iset),ptc
c	   if(discprt) write(8,603) tcrit(iset),ptc
	   print 603,ttop,ptc
	   if(discprt) write(8,603) ttop,ptc
...
c  BUT the asymptotic distn is defined only for 'excess time', so
c should not use below tres
c Allocation to xcal, ycal
c (1) icomp=1
c	j=1 for exact
c	j=2 for zero res (if nores=true)
c (2) icomp=2
c	j=1 for exact
c	j=2 for asymptotic
c	j=3 for zero res (if nores=true)
c (3) icomp=3
c	j=1 for exact
c	j=2 for asymptotic
c	j=3,..,ncomp+2 for components of assymptotic
c	j=ncomp+3 for zero res (if nores=true)
c
c  Define columns
	if(iplot.eq.1.or.iplot.eq.2) then
	   isetcol=1
...
c Modif 08/03/04 07:58am do both open and shut are calculated each time
c so Popen can be calculated and printed
c These calcs now moved to HJCLIK so printed once after
c "distributions for set #"
	   if(iplot.eq.1) then
	 	kX=kA
	 	call PDFopen(QD,phio,area,tau1,kA,km)
c		call PDFshut(QD,phio,area,tau1,kF,km)
c		call PDFOUTs('Shut time pdf with zero resolution',
c     &	-1,-1,area,tau1,kF,ams,sd,km,.false.,.false.,.false.)
c	 	call PDFopen(QD,phio,area,tau1,kA,km)
c	 	print 108
c		if(discprt) write(8,108)
c		call PDFOUTs('Open time pdf with zero resolution',
c     &	-1,-1,area,tau1,kA,amo,sd,km,.false.,.false.,discprt)
	   else if(iplot.eq.2) then
		kX=kF
		call PDFshut(QD,phio,area,tau1,kF,km)
c	 	call PDFopen(QD,phio,area,tau1,kA,km)
c		call PDFOUTs('Open time pdf with zero resolution',
c     &	-1,-1,area,tau1,kA,amo,sd,km,.false.,.false.,.false.)
cc
c		call PDFshut(QD,phio,area,tau1,kF,km)
c		print 108
c		if(discprt) write(8,108)
c		call PDFOUTs('Shut time pdf with zero resolution',
c     &	-1,-1,area,tau1,kF,ams,sd,km,.false.,.false.,discprt)
	   endif
c
	   fac1=fac
	   if(scale) then
...
c ncurvc, icurvc, ncal, iline all set above for iplot=1.2
	if(iplot.eq.1.or.iplot.eq.2) then
c	   isetcol=0
	else if(iplot.eq.3) then
...
c Call with thetas() and kmax=kth (not used because ifitype=0)
	...
	rescalex=.false.	!in case it was switched on
c
c where next?
994	continue
	if(iplot.ne.4.and.iplot.ne.6) then
	   iopt=3
	   if(iplot.eq.2.and.abs(xmax-xmaxsav).gt.0.0001) then !xmax and xmaxsav in log units if logt
		rescalex=.true.
		iopt=1	!always recalculate after xmax changed
	   endif
	   print 18,iopt
18	   format(
...
c
	...
	if(iopt.eq.1) then
	   ...
	   rescalex=.true.
c	   if(xmax.ne.xmaxsav) then
	   if(abs(xmax-xmaxsav).gt.0.0001) then !xmax and xmaxsav in log units if logt
		if(logt) xmax=10.**xmax
	      goto 199		!rebin including slonger shut times
	   else
c===		if(logt) xmax=10.**xmax	!no, after label 19 expect xmax to be log units
...
c set defaults for next plot
	if(iplot.ne.6) then
	   ...
	   if(iopth.lt.4) then
		iopth=4		!skip iopth=2,3 by default
		iplot=6
	      if(nlvar.lt.nlig) then
		   nlvar=nlvar+1		!next ligand, same set
	      else if(nlvar.eq.nlig) then
	   	   if(iset.lt.nset) then
			iset=iset+1		!next set, 1st ligand
			nlvar=1
	   	   else if(iset.eq.nset) then
			iplot=7		!all sets and ligands done
			goto 226
		   endif
		endif
	   endif
	   goto 86
c This goes to next sort of Popen plot, but with default iopth=4 so if default
c is accepted, go straight to 226 for next plot type, but can still get
c iopth=2,3 (rarer) by overriding the default
	endif
	if(iplot.gt.ipmax) iplot=ipmax
	goto 226
c
999	continue
	if(allocated(thetas)) deallocate(thetas)
