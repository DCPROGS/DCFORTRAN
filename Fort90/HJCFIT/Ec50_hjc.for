...
c NB uses only conc(i,1).
c Modif 09/04/04 02:14pm by adding jset as parameter, and using
c conc(i,jset).  This matters when nlig>1 because conc of other ligands
c may affect the pmax and EC50
c
...
c
c	ALLOCATE(QD(kdim,kdim),pinf(kdim))
...
c 09/04/04 02:16pm jset is now a parameter
c	jset=1
	do i=1,nlig
...
c no need to correct for fast block when conc=0
c
...
c
	...
	curmax=sngl(Ymax)
	curinfs=sngl(Yinf)
c  Debug amd checking
c	Popen=0.0d0
c	current=0.0d0
c	do i=1,kA
c	   current=current+cfacd*dgamma(i)*pinf(i)
c	   Popen=Popen+pinf(i)
c	enddo
c	f1=(Popen-Popen0)/(pmax-Popen0)
c	f2=(Yout-Y0)/(Ymax-Y0)
c end of debug
c
	DEALLOCATE(QD,pinf)
	do i=1,nlig
