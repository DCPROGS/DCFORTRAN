	subroutine QDEFINE(iread,QD,PI,pinf,QT,IQ,titlep,titles,
...
c
	real*8 QT(100,100),QD(100,100),P0(100),Pinf(100),pzero(100)
	real*8 Peq(100),dgamma(100)	!use Peq as arg for EQOCC
	real*8 Qdum(100,100)
...
c	character*74 rtitle(40)	!title to describe rate constants
	...
	character*2 charmod(25,40),charmod1(25,34)
	character qfilem*40
	real*4 PI(100,100)
...
c calc determinants in EQOCC
	...
	character*11 cnum0,cnum1,cnum2
	logical chardef,boundef
c
	REAL*4 PSTAR(4)		!KM2 OCCS
	integer IC(2,200)
	integer NSC(50),IM(50,100),JM(50,100),IX(100),JX(100),IL(100)
	integer NSC1(50),IM1(50,100),JM1(50,100),IX1(100),
     &	JX1(100),IL1(100)
	integer kmcon(9)
	character*1 ans,UC
	LOGICAL READP,KMFAST,newmod,altmod,blank,sameq,vjump,cjump
c	logical ranrate
...
1	   format(
     &	' Number of underlying rate constants in the mechanism = ')
	   call INPUTi(npar)
	   if(npar.ge.nrateq) then
		call BELL(2)
		print 2
2		format(' ERROR:',/,
     & ' For independent models number of underlying rate constants',/,
     & ' is less than number of q(i,j).                            ',/,
...
c
	   ...
	   JFQ(35)=2
	   JFQ(36)=2
	   do n=1,36	!must also define irate, jrate, for debug
		i=ieq(n)
		j=jeq(n)
...
c
	call GETCHAN(nchan,dgamma,readp,sameq,idest)
	if(pon()) write(7,320) NCHAN,kA,(1.0d12*dgamma(i),i=1,kA)
      if(discprt) write(8,320) NCHAN,kA,(1.0d12*dgamma(i),i=1,kA)
...
C NOW GET AGONIST CONC.
	if((.not.kmfast).and.ncdep.eq.0) goto 504	!conc not needed
24	continue
311	continue
	if(nlig.eq.1) then
	   xA=conc(1)	!default
...
16	   FORMAT(' Concentration (micromolar) = ')
	   ...
	   XA=XA*1.0E-6		!=MOLAR
	   conc(1)=xA
         if(discprt) write(8,163) 1,xa,ligname(1)
163	   FORMAT(' Ligand #',i2,': concentration (micromolar) = ',
     &	g13.6,'(',a20,')')
	else
	   do n=1,nlig
		x=conc(n)	!default
		print 161,n,ligname(n)
161		FORMAT(1x,i2,'. Concentration of ',a20,' (micromolar) = ')
		call INPUTr(x)
		X=X*1.0E-6		!=MOLAR
		conc(n)=x
      	if(discprt) write(8,163) n,x,ligname(n)
	   enddo
	endif
	xB=conc(2)		!for consistency with earlier versions
	if(nlig.lt.1.or.nlig.gt.10) print 162
162	format(' ERROR: not yet written for more than 10 ligands')
c
241	continue
c
c	IF(KMFAST) CALL KM2OCC(XA)
	IF(KMFAST) CALL KM2OCC(conc(1))
504	CONTINUE
...
C CALC CURRENT FOR THIS (or these) conc (now all in SLIFED1)
	call SLIFED2(QD,Peq,.false.,.false.,kmax,kmax) !screen only
	CUR=0.0
	DO 319 I=1,kA
319	CUR=CUR+sngl(Peq(I)*dgamma(I))
	CUR=CUR*CFkin
...
400	FORMAT( ' Type Q and PI matrices [Y]? ')
	...
	CALL ATYPD(QD,'    QD  ',K,K,KMAX,KMAX)		!TYPE Q
	CALL ATYPS(PI,'    PI  ',K,K,KMAX,KMAX)
45	continue
	call flush(7)
C
...
c NB iw=1,2 options abandoned for new file format
cc If iw=2 then all existing models with numbers => imod0 must be read in
cc and moved up one place. Also update imod() which records which model
cc has data (rates etc) on each disc
c	if(iw.eq.2) then
c	   do i=nmods-1,imod0,-1	!nmods already inc so nmods-1=orig nmods
c		mtits(i+1)=mtits(i)
c		read(14,rec=i+1) charmod1,ilast1,jlast1,ncdep1,ix1,jx1,il1,
c     &      ncyc1,nsc1,im1,jm1,kA1,kB1,kC1,kD1,titlep1,ncon1
c		write(14,rec=i+2) charmod1,ilast1,jlast1,ncdep1,ix1,jx1,il1,
c     &      ncyc1,nsc1,im1,jm1,kA1,kB1,kC1,kD1,titlep1,ncon1
c	   enddo
c	   mtits(imod0)=mtitle		!insert new title after others moved up
c	   do 51 i=1,10
c		if(jmod(i).ge.imod0) jmod(i)=jmod(i)+1
c51	   continue
c	endif
c
c	write(14,rec=1) nmodc
c	if(nmodc.gt.41) then	!write mtits() to qtitles.dat
c         OPEN(unit=15,file='QTITLE.DAT',status='UNKNOWN',
c     &     access='DIRECT',form='UNFORMATTED',recl=10240)
c	   write(15,rec=1) nmodc,(mtits(i),i=1,nmodc)
c         CLOSE(unit=15)
c	else
c	   write(14,rec=1) nmodc,(mtits(i),i=1,nmodc)
c	endif
c	write(14,rec=imod0+1) charmod,ilast,jlast,ncdep,ix,jx,il,
c     & ncyc,nsc,im,jm,kA,kB,kC,kD,titlep,ncon
cc NB recl is in bytes (4*value for PDP)
c      CLOSE(unit=14)
c	if(nmodc.le.41) then
c         print 8031,imod0
c         if(discprt) write(8,8031) imod0
c8031     format(
c     &  ' Details of model stored, as model #',i3,
c     &  ' , on disk (in QMODEL.DAT)',/)
c	else
c         print 8032,imod0
c         if(discprt) write(8,8032) imod0
c8032     format(
c     &  ' Details of model stored, as model #',i3,
c     &  ' , on disk (in QMODEL.DAT and QTITLE.DAT)',/)
c	endif
c5	continue	!end of storage of model in qmodel.dat
cc
cc SAVE RATES: write Q etc back to disc
c	call DCASK('Save rate constants on disc','n',ans)
c      if(UC(ans).eq.'N') goto 9
c       OPEN(unit=18,file='QDAT.DAT',status='UNKNOWN',
c     &  access='DIRECT',form='UNFORMATTED',recl=2048)
c	print 811
c811	format(
c     & ' Choose file number to write rates to:',/,
c     & ' (1) List rate constant files for current model only',/,
c     & ' (2) List all rate',/,
c     & ' Option number [1] = ')
c	iopt=1
c	call INPUTi(iopt)
...
c	if(iopt.eq.1) then
c	  if(.not.newmod.and.nrecq.ge.1) then
c	   print 190,imod0,mtits(imod0)(1:74)
cc190	   format(' For model #',i3,': ',a74,/,
cc     &   ' the following rate constants have been stored:',/,
cc     &   '   file #:                      Name of rate file')
c	   do i=1,nrecq
c		if(jmod(i).eq.imod0) then
c		   print 192,i,rtitle(i)
cc192		   format(1x,i3,': ',a74)
c		   irecq=i	!use as default
c		endif
c	   enddo
c	  endif
c	else
...
c		   print 11
cc11		   format(' Hit space bar for more')
c		   call ANYKEY
...
c	  enddo
c	endif
c	print 151,nrecq+1,irecq
c151	format(' Next vacant rate file # = ',i4,/,
c     & ' File # for rates [',i2,'] = ')
c	call INPUTi(irecq)
c	call TITENT0(' Enter title for rates',rtitle(irecq),74,.false.)
cc Write the data to chosen file (pair of records) in QDAT
cc NB In record 1 single channel progs (SCBST etc) read only up to MTITLE; the
cc rest of the values in recd #1 are needed in for noise/jumps
c       OPEN(unit=18,file='QDAT.DAT',status='UNKNOWN',
c     &  access='DIRECT',form='UNFORMATTED',recl=2048)
c	jrecq=2*irecq-1	!record # for FIRST record of data
cc NB in this version, xa0,xa1 not distinguished, cur0, P0() not defined
cc and vjump, cjump not defined, but simpler to leave them in here
cc to avoid altering read statements in other progs. Likewise in 2nd
cc record pzero not defined
c	vjump=.false.
c	cjump=.false.
c	write(18,rec=jrecq) QD,PINF,K,ka,kb,kc,kd,nmod,imod0,mtitle,
c     &   xa,xa,xb,xb,nlig,P0,cur0,dgamma,kmfast,pstar,vhold,vkin,
c     &   nchan,vjump,cjump,titles,ncdep,IL,IX,JX
c	ibflag=1001		!to signal that valid nbound() has been written
c	write(18,rec=jrecq+1) QT,Pinf,nmod,imod0,vref,Vkin,PZERO,PSTAR,
c     &   KMFAST,KMCON,HPAR,IV,JV,NVDEP,nchan,dgamma,rtitle(irecq),
c     &   ibflag,nbound
c	jreclast=jrecq
c	CLOSE(unit=18)
cc
c      print 14,imod0,irecq,rtitle(irecq)
c      if(pon()) write(7,14) imod0,irecq,rtitle(irecq)
c      if(discprt) write(8,14) imod0,irecq,rtitle(irecq)
c14	format(/,
c     & ' Rate constants for model # ',i3,' stored in file # ',i3,
c     & ' of QDAT.DAT with title:',/,1x,a74)
cc
cc Record in QGEN.INI which data was used last
c	jmod(irecq)=imod0
c	if(irecq.gt.nrecq) nrecq=irecq
c      OPEN(unit=17,file='QGEN.INI',status='UNKNOWN',
c     & access='DIRECT',form='UNFORMATTED',recl=512)
c	write(17,rec=1) nrecq,jreclast,jmod
c	CLOSE(unit=17)
c9	continue
c      if(pon()) write(7,10) imod0,irecq,rtitle(irecq)
c      if(discprt) write(8,10) imod0,irecq,rtitle(irecq)
c10	format(/,
c    & ' Rate constants for model # ',i3,' from file # ',i3,
c    & ' of QDAT.DAT with title:',/,1x,a74)
cc
99	continue
...
c
	...
	subroutine GETCHAN(nchan,dgamma,readp,sameq,idest)
	REAL*8 dgamma(100)
	logical readp,sameq
	character*1 ans,UC
	COMMON/KBLK/kA,kB,kC,kD
c
	...
	print 320,NCHAN,IR,(1.0d12*dgamma(I),I=1,IR)
320	FORMAT(' Number of channels = ',i10,/,
     & 3X,I3,' open states, with gamma (pS)=', 5G13.6)
	if(sameq) goto 402
	print 43
