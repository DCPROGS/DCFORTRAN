...
5911	   	   format(/,
     ...
     & ' Maximum occupancy = ',g11.4,/,
     & '    Conc for 50% of this maximum (EC50) = ',g11.4,/)
		endif
...
5921		   format(/,
     ...
     &    '   Response at conc -> infinity = ',g12.5,' pA',/)
	   ...
	   yinfeq=abs(curinfs)	!maximum resonse/binding (for Hill slopes)
...
593	   		format(/,
     ...
     & ' Maximum response (pA) = ',g11.4,/,
     & '    Conc for 50% of this maximum (EC50) = ',g11.4,' (muM)',/)
		   else
...
5931	   	   format(/,
     ...
     & ' Maximum occupancy = ',g11.4,/,
     & '    Conc for 50% of this maximum = ',g11.4,' (muM)',/)
		endif
	    else
		if(plotcur) then
...
c	   endif			!end of peakec50
	   y00=0.0			!minimum response/binding (for Hill slopes)
c	   yinfp=abs(curinfs)	!maximum response/binding (for Hill slopes)
	   yinfp=abs(curmax)	!maximum response/binding (for Hill slopes)
...
c skip points above 99.9% of max (gets lost in rounding errors)
	   do i=1,ncalc-1
		if((ycal(i,1).gt.1.e-10.and.xcal(i,1).gt.1.e-10).and.
     &	  ((yinfeq-ycal(i+1,1))/yinfeq.gt.0.001)) then
		   n=n+1
		   y1=alog((ycal(i,1)-y00)/(yinfeq-ycal(i,1)))
		   y2=alog((ycal(i+1,1)-y00)/(yinfeq-ycal(i+1,1)))
		   x1=xcal(i,1)
		   x2=xcal(i+1,1)
		   if(x1.le.ec50eq.and.x2.ge.ec50eq) i50=i
		   x1=alog(x1)
		   x2=alog(x2)
...
133		format(/,
     & ' Hill slope at equilibrium EC50 (',g11.4,' muM) = ',g13.6)
	   endif
...
c skip points above 99.9% of max (gets lost in rounding errors)
     ...
     &	  ((yinfp-ycal(i+1,2))/yinfp.gt.0.001)) then
		   n=n+1
...
1331		format(/,
     & ' Hill slope at EC50 for peak (',g11.4,' muM) = ',g13.6)
	   endif
c
