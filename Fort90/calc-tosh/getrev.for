	subroutine GETREV(ncyc,nsc,im,jm,ncyc1,nsc1,im1,jm1,titlep,
...
c
c 02/14/04 06:44pm add as parameters: npar,irate,jrate
c
...
c Don't try 'same again' unless valid isetmr() is read from .ini
	if(ncyc.gt.0.and.useini.and.
     &	isetmr(1).ge.1.and.isetmr(1).le.ncyc) then
	   call BELL(1)
	   print 143
143	   format(/,
     & ' Last run set the following rates by microscopic reversibility')
	   do n=1,ncyc
		...
		if(obeymr(n)) then
		   print 142,n,i1,j1,titlep(m)
142		   format(
     &  ' Route ',i2,': q(',i3,',',i3,') = ',a10,' set by MR')
		else
		   print 145,n,i1,j1,titlep(m)
145		   format(
     &  ' Route ',i2,': q(',i3,',',i3,') = ',a10,' NOT set by MR')
	   ...
	   if(ans.eq.'Y') then
		do n=1,ncyc
		   ...
		   enddo
		enddo
c print settings to disk
		do i=1,ncyc
		   isetmr(i)=i	!can set in original order if no cubes
		   call PRTCYC(i,ncyc,nsc,im,jm,icube,titlep,
     &	      npar,irate,jrate,1,0)
		enddo
		goto 99	!skip asking
	   endif
	endif
	if(useini) then
	   allmr=.true.
	   do i=1,ncyc
		if(.not.obeymr(i)) allmr=.false.	!default
	   enddo
	   do i=1,ndim
		automr(i)=.false.
...
21	   format(
     & ' Number of shared edges in cycle # ',i3,' = ',i3,/)
	enddo
...
c Altered 10/15/03 12:03pm to omit question about automr.  I think this could be
c true only for 'outer cycles' that have already been eliminated, apart from
c the case of cubes, for which the 'outer cycle' has the same number of states
c as the other cycles and has therefore not been eliminated
c		if(ncyc.lt.4) then	!automr not possible
		if(ncube.eq.0) then	!automr not possible
		   ans='Y'
		   if(useini.and.(.not.obeymr(i))) ans='N'
		   print 65,i,ans
65		   format(
     & ' Should cycle #',i2,' be constrained by micro rev [',a1,'] ? ')
		   call INPUTa(ans)
		   obeymr(i)=ans.eq.'Y'
		   automr=.false.
		else
...
c		if(obeymr(i).and.(.not.automr(i))) then	!set default
c		   ians=1
c		else if(automr(i)) then
c		   ians=2
c		else
c		   ians=3
c		endif
c As above, now set automr=false for all cases except cubes
c		if(ncyc.lt.4) then	!automr not possible
		   print 65,i
c65		   format(
...
cc651		   format(
cc     &    ' Cycle ',i3,':',/,
cc     &    '  (1) set to obey micro rev',/,
cc     &    '  (2) obeys micro rev via constraints on adjacent cycles ',/,
cc     &    '  (3) does NOT obey micro rev',/,
cc     &    '  Option number [',i2,'] = ')
c		   call INPUTi(ians)
c		   if(ians.eq.1) then
c			obeymr=.true.
c			automr=.false.
c		   else if(ians.eq.2) then
c			obeymr=.true.
c			automr=.true.
...
cc
	...
	subroutine PRTCYC(icyc,ncyc,nsc,im,jm,icube,titlep,
     &	npar,irate,jrate,iscr,imode)
...
c
c isc=0 to print to disk only (if discprt)
c isc=1 to print to screen and disk
c
c for definitions of cubes
	integer nsc(50),im(50,100),jm(50,100)
	integer*4 cubedef(100,8),cubecyc(100,6),cubext(100)
...
1271		   format(' Micro rev guaranteed by adjacent cycles',/,
     &		2(5(i3,'-',i3,4x),/))
		   ...
		   j1=jm(n,1)
		   call GETM(i1,j1,m,npar,irate,jrate)
		   if(iscr.eq.1) print 1311,i1,j1,titlep(m)
		   if(discprt) write(8,1311) i1,j1,titlep(m)
1311		   format(' q(',i3,',',i3,') = ',a10,' set by micro rev')
		   if(iscr.eq.1) print 130,(im(n,m),jm(n,m),m=2,nsc(n))
		   if(discprt) write(8,130)(im(n,m),jm(n,m),m=2,nsc(n))
130		   format(2(5(i3,'-',i3,4x),/))
		endif
	   else
		if(iscr.eq.1) print 1312,im(n,1),jm(n,1)
		if(discprt) write(8,1312)im(n,1),jm(n,1)
1312		format(i3,'-',i3,'  (no micro rev constraint)')
		if(iscr.eq.1) print 130,(im(n,m),jm(n,m),m=2,nsc(n))
		if(discprt) write(8,130)(im(n,m),jm(n,m),m=2,nsc(n))
c130		format(2(5(i3,'-',i3,4x),/))
	   endif
c==	enddo		!end of L=1,ncyc
c
	RETURN
