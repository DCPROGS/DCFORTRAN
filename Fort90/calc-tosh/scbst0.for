...
C    DECLARE INTEGER ARGUMENTS FOR SUBMAT
	...
	LOGICAL OPEN,GAP,DOKR,DOK,DOCOND,SHORT,DONE,pon,debug,log
	character*1 ans,UC
...
3	format(
	...
	print 398
398	format(
     &' This program uses the Q matrix generated by QGEN. ',/)
c
c
...
c106	continue
	...
	INQUIRE(file='SCBST.INI',exist=present,flen=nlen)
	if(present.and.nlen.gt.0) then
	   readini=.true.
	   if(nlen.eq.1024) then		!old.ini
      	OPEN(unit=17,file='SCBST.INI',status='UNKNOWN',
...
c   can't write back in new format until nsc(), ncyc know -write on exit
	     	...
	     	OPEN(unit=16,file='SCBST.INI',status='UNKNOWN',
...
c==		read(16,rec=1)irecq,nvdep,conc,imodold,ncyc,
     ...
     &	qfilem,useprim
		...
		if(suffix.ne.'mec') qfilem='qmechs.mec'
...
146			format('  (conc of ligand #',i2,',  ',a10,' = ',g13.6,
     ...
     &   conc,nlvar,vhold,vref,cfacd,dgamma,.true.,k,kdim)
	   print 593,cur0,pop0
...
343	continue
	...
	call SLIFED2(QM,Peq,pon(),discprt,km,km) !print it
...
C
	DOCOND=.FALSE.
	IF(SHORT.OR.(ITCOR.GT.0).OR.KA.EQ.1) GOTO 3512
	print 3511
3511	FORMAT(
     & ' Type distributions conditional on starting state in A [N] ? ')
	ans='N'
	call INPUTa(ans)
	if(UC(ans).EQ.'Y') DOCOND=.TRUE.
3512	CONTINUE
C
...
322	CONTINUE	!END OF M LOOP
	...
	if(.not.short.and.pon()) write(7,324)
...
C AGAIN FOR OVERALL (ITYPE=3)
	DOKR=.FALSE.
	DOK=.FALSE.
	IF(SHORT.OR.KA.EQ.1) GOTO 50	!PDF SAME FOR ALL K,R
	print 51
51	FORMAT(
     & ' Type f(k,r) and f(k) (kth event in burst with r ops) [N] ? ')
	ans='N'
	call INPUTa(ans)
	if(UC(ans).ne.'Y') GOTO 5201
	DOKR=.TRUE.
	DOK=.TRUE.
	GOTO 50
5201	print 52
52	FORMAT(' Type f(k) (conditional on k only) [N] ? ')
	ans='N'
	call INPUTa(ans)
	if(UC(ans).EQ.'Y') DOK=.TRUE.
50	CONTINUE
C
...
1041	format(' For event #',i3, ' in a burst with',i3,' opening(s)')
	if(open) call PDFOUTs('     f(open)',ik1,ir1,area,tau,kA,am,sd,
     & km,pon(),.true.,discprt)
	if(gap) call PDFOUTs('     f(shut)',ik1,ir1,area,tau,kB,am,sd,
     & km,pon(),.true.,discprt)
...
1051	format(
	...
	if(open) call PDFOUTs('     f(open)',ik1,-1,area,tau,kA,am,sd,
     & km,pon(),.true.,discprt)
	if(gap) call PDFOUTs('     f(shut)',ik1,-1,area,tau,kB,am,sd,
     & km,pon(),.true.,discprt)
...
c	   topen=s
	...
	if(gap) call PDFOUTs('     f(shut)',-1,-1,area,tau,kB,am,sd,
     & km,pon(),.true.,discprt)
...
53	format(/,' Check: distribution of burst length')
...
c	print 7,M,EIG,-1.0E3/EIG
c	if(pon()) write(7,7)M,EIG,-1.0E3/EIG
c      if(discprt) write(8,7)M,EIG,-1.0E3/EIG
847	CONTINUE
...
205		format(/,' Use t(crit) (ms) = ')
     ...
     &    ob,stb,otb,bl,pon(),discprt,noprint,imode)
...
c rename it as scbst0.ini
	INQUIRE(file='SCBST.INI',exist=present,flen=nlen)
	if(present.and.nlen.eq.1024) then
	   call RENAME('scbst.ini','scbst0.ini')
	   print 13
	   if(discprt) write(8,13)
13	   format(/,
     &' Your original SCBST.INI has been renamed SCBST0.INI, and',/,
     &' replaced with the new, larger, scbst.ini')
	endif
	imodold=imod0
