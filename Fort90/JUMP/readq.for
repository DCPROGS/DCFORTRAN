...
c
	...
	read(11,rec=krn) iptype,ndv2,ndimd2,ndc2,ndimc2,itit,title1(1:44),
...
c now, so that if text, lines etc added in VPLOT they will have a default colour
	call SETCLS(mono,icol,.false.,.false.,1)
c
...
365	continue
	do j=1,ncurvc
	   if(ncal(j).gt.ndc1) ndcf=ncal(j) 
	enddo
	if(ndcf.gt.ndc1) then
	   deallocate(xcal,ycal)
	   allocate(xcal(ndfc,ndimc),ycal(ndfc,ndimc))
	endif
