...
c    idatyp=1 for new SCAN.SCN
c    idatyp=2 for original SCANDAT.DAT
c    idatyp=3	special data from YVALIN
...
c
	...
	logical fastf,caplock,present,readini,nopoint
...
c for cjump
	logical cjump
	allocatable::jumps,jumporig,kjumps0,kjumps,jumpomit,jomit1
c===next 2 lines 2D?
c	integer*2 jumps(:),jumpomit(:),jomit(:)
c	integer*4 kjumps0(:),kjumps(:)
	integer*2 jumps(:,:),jumpomit(:,:),jomit1(:,:),jumporig(:,:)
...
c
	...
	common/aafunc/trise	!for aamax, afunc (see ptyp1)
c===need arrays in common/rblck?
	common/rblck/treso,tresg,acrit,avamp	!for RESINT and STABPLOT
c	real*4 Yval(81920,10)	!better make allocatable and put in all calls (explik etc)
	real*4 Yval(500000,10)	!better make allocatable and put in all calls (explik etc)
...
c	ndimy=81920		!size of yval (but can't use var dimension
	ndimy=500000		!size of yval (but can't use var dimension
c				!in common/fitblk so dimension must be set as here
c				!in all subroutines where common used:
c				"explik,gaulik,gaufit1,oblik, mlfits and EKDIST (above)
...
450	format(' Read defaults from .ini file on disc [Y] ? ')
	...
	call INPUTa(ans)
     ...
     &    'Name for .ini file:',inifile,40,.false.)
	   INQUIRE(file=inifile,exist=present,flen=len,err=452)
...
c
c NB the inifile contains BOTH pfile and pfileb (unlike hjcfit) -does not need
c both so use ONLY pfile (the first one), but read it back as pfileb (as in
c hjcfit), so can tell whether expts are same as the last run.
c Same for nfile(b) and kfile(b) (and in error contains nfile twice in 2nd line!
c but too complicated to change ini file structure now)
c NB the duplicate appearence of nfile  in 2nd line gives 40 bytes that can
c be used for something else.  For iniver=102, this is replaced by
c nocor,yhio,yhig,idum where idum is dummy array to make read up to
c 40 bytes.
		OPEN(unit=19,file=inifile,status='UNKNOWN',
     &	access='TRANSPARENT')		!new .ini
c	      read(19,rec=1) iniver,
c     &	ndev,nfile,nset,nfile,kfile,pfile,iydat,
c     &	nmax,ntot,nswpmax,nomit,cjump
	      read(19,rec=1) iniver,
     &	ndev,nfileb,nset,nfileb,kfileb,pfileb,iydat,
     &	nmax,ntot,nswpmax,nomit,cjump
		if(nswpmax.lt.1) nswpmax=1
c Now we have nset etc can allocate jomit() and read the rest
c Read pfile as pfilen, nfileb (2nd pfileb is superfluous)
c Likewise do not need both tresolo and tresolob in the ini file -read
c back tresolo as tresolob here and ignore tresolb
c BUT must also read back what is written as pfileb, as pfile (not used,
c but if read as pfileb it overwrites the earlier one -same for tresolo)
		ALLOCATE(jomit1(nswpmax,nset))
		if(iniver.eq.101) then
	         read(19,rec=1) iniver,
     &	   ndev,nfileb,nset,nfileb,kfileb,pfileb,iydat,
     &	   nmax,ntot,nswpmax,nomit,cjump,concA,concB,
     &	   nbad1,isbad,iebad,nfile,kfile,pfile,idiskq,
     &	   tresolob,tresolgb,tresolo,tresolg,
     ...
     &	   ((jomit1(i,j),i=1,nomit(j)),j=1,nset)
		else if(iniver.eq.102) then
		   ALLOCATE(idum(7))	!so nocor,yhio,yhig,idum=40 bytes
c	         read(19,rec=1) iniver,
c     &	   ndev,nfileb,nset,nfileb,kfileb,pfileb,iydat,
	         read(19,rec=1) iniver,
     &	  ndev,nfileb,nset,nocor,yhio,yhig,idum,kfileb,pfileb,iydat,
     &	   nmax,ntot,nswpmax,nomit,cjump,concA,concB,
     &	   nbad1,isbad,iebad,nfile,kfile,pfile,idiskq,
     &	   tresolob,tresolgb,tresolo,tresolg,
     &	   onetcrit,tcvals,tcrit0,qfile,nvalsav,ylo,yhi,
...
c     	OPEN(unit=20,file=inifile,status='UNKNOWN',
c     &	access='DIRECT',form='UNFORMATTED',recl=3072)
		OPEN(unit=20,file=inifile,status='UNKNOWN',
...
c iniver=102 uses the 40 bytes freed by the duplicate appearence of nfile
c in 2nd line to keep nocor
c		iniver=101
		iniver=102
		ALLOCATE(idum(7))	!so nocor,yhio,yhig,idum=40 bytes
c	      write(19,rec=1) iniver,
c     &	ndev,nfile,nset,nfile,kfile,pfile,iydat,
	      write(19,rec=1) iniver,
     &	ndev,nfile,nset,nocor,yhio,yhig,idum,kfile,pfile,iydat,
     &	nmax,ntot,nswpmax,nomit,cjump,concA,concB,
...
12	   format(/,
     ...
     & ' Option number [',i2,']  = ')
	   call INPUTi(iydat)
	   idatyp=iydat
c	   if(iydat.eq.1.or.iydat.eq.2) idatyp=iydat
	endif
c
	if(iydat.eq.3) then
	   call YVALIN(titlex,yval,nyval,.false.,thist,obhist,
     &     ahist,idtype,ndimy)
...
c	nsetini=nset		!save input value in case changed in diskin1
...
c
c Write pfile etc to .ini straight after diskin1
	OPEN(unit=19,file=inifile,status='UNKNOWN',
     & access='TRANSPARENT')		!new .ini
c	iniver=101
	iniver=102
...
c	 write(19,rec=1) iniver,
c     & ndev,nfile,nset,nfile,kfile,pfile,iydat,
c     &  nmax,ntot,nswpmax,nomit,cjump,concA,concB
	  write(19,rec=1) iniver,
     &	ndev,nfile,nset,nocor,yhio,yhig,idum,kfile,pfile,iydat,
     &	nmax,ntot,nswpmax,nomit,cjump,concA,concB
	DEALLOCATE(idum)
...
c
c If nset changed from the value from ini file then need to re-allocate
c jumpomit() -do anyway since nswpmax not known
c=	if(first.and.cjump.and.nset.ne.nsetini) then
	if(first.and.cjump) then
	   ...
	   do j=1,nset
		if(newini) then
		   do i=1,nomit(j)
			jumpomit(i,j)=jomit1(i,j)
		   enddo
		else
		   do i=1,nomit(j)
			jumpomit(i,j)=jomit0(i)
		   enddo
		endif
	   enddo
	   if(allocated(jomit0)) DEALLOCATE(jomit0)
	   if(allocated(jomit1)) DEALLOCATE(jomit1)
	endif
	ALLOCATE(jumps(nswpmax,nset),jumporig(nswpmax,nset),
     & kjumps0(nswpmax,nset),kjumps(nswpmax,nset))
c
c Read data into these arrays
	call DISKIN2(idatyp,tint0,iampl0,ampl0,iprops0,nintt,
	...
	if(allocated(jfreq3)) then
	   DEALLOCATE(jfreq3)	!no longer needed
c	   DEALLOCATE(jfreq3,STAT=IERR)	!no longer needed
c	   PRINT*,IERR
	endif
	if(allocated(jfreq2)) then
	   DEALLOCATE(jfreq2)	!no longer needed
	endif
	if(allocated(jfreq1)) then
	   DEALLOCATE(jfreq1)	!no longer needed
	endif
c
c Now do checks on jump data
	if(cjump) then
	   call JUMPCHK(nset,nsweep,jumps,jumporig,kjumps0,
...
c		ihjc=-1	!so no questions after 1st set done
	   ...
	   do j=1,nset
	      if(first.or.(.not.sres)) then
		   print 63,tresolo(j)*1000.,tresolob(j)*1000.,tresolo(j)*1000.
c       treso,tresg in ms
63		   format(/,
...
c		  treso=tresolo(j)	!default resolution from .ini in microsec
c		  tresg=tresolg(j)	!default resolution from .ini in microsec
		  call RESINT61(tint0(1,j),tint(1,j),ampl0(1,j),ampl(1,j),
     &	  iprops0(1,j),iprops(1,j),nint(j),nintt(j),
     & 	  imode,ffilt,fc,rms,trise,nmax,nd1,index(1,j),
     & 	  cjump,nsweep(j),kjumps0(1,j),kjumps(1,j))
...
702		   format(' ERROR in RESINT:',3i9,2g13.6,i5)
		endif
	   else
...
9		continue
	...
	enddo		!end of j=1,nset
c
c Insert here a test section to round intervals in tint() to integer
c multiples of a fixed length, eg sample interval, to mimic SKM etc
	ans='N'
	call DCASK('Discretise all time intervals',ans,ans)
	if(ans.eq.'Y') then
	   print 71
71	   format(' sample interval (microseconds) = ')
	   call INPUTr(tsam)
c e.g tint(i,j), j=1,..,nset; i=1,..,nfile(j)
	   deb=debug()
	   do j=1,nset
		do i=1,nint(j)
		   t=1000.*tint(i,j)	!in microseconds e.g. 91.5
		   x=ROUND(t/tsam,0)	!e.g. round(91.5/25)=round(3.66)=4
		   t1=x*tsam		!e.g.=100 microsec
		   if(deb) print 72,i,j,t,t1
72		   format(2i8,f12.2,3x,f12.2)
		   tint(i,j)=t1*0.001	!e.g.=0.1 msec
		enddo
	   enddo
	endif		!end of discretisation
c===start moved bit
...
c        check file names but may have different paths so care needed
	   ...
	   if((.not.samex).and.first) then
		print 70
c           if(discprt) write(8,70)
70	      format(' The bad sections defined in ekdist.ini are for',
...
37		format(' NOTE: ',i3,
     ...
     &	    nmax,nbad,isbad(1,j),iebad(1,j),index(1,j),.false.)
		   cutdone=.true.
		else
...
100	     format(
     ...
     & ' Option number [',i2,'] = ')
	     ...
	     if(iopt.lt.4.or.iopt.gt.6) iopt=6
	     print 103,nset,iopt
103	     format(
     & '        MENU OF DISTRIBUTION TYPES ',/,
...
c91		format(/,' DATA SET # ',i4)
     ...
     &   nfile(jset),nset,jset,nmax,yhio,yhig)
...
c NB call with yval1(:,:) for the VHIST data
	...
	call SCVDU(xaxis,xmin,xmax,freq,nbin,fmin,fmax,flo,fhi,
     ...
     & YVAL,nyval)
c YVAL() added to args (10/22/02 06:24pm) for randomistaion tests
c (known as yval0 within SCVDU)
...
c==		   idest=0
     ...
     & 		   cjump,yvalt,nfitt,ihtype,nset,j,idest1)   !don't change idest
		...
		else if(idest.eq.0) then	!randomisation test done, not fit
		   goto 705		!redisplay from 1st set (jset is set in scvdu)
		endif
	   else if(fitted) then
		if(idest.eq.0) then
...
c	kmax=k
c
cc============temp write to .ini here
c	OPEN(unit=19,file=inifile,status='UNKNOWN',
c     & access='TRANSPARENT')		!new .ini
cc	iniver=101
c	iniver=102
c	ALLOCATE(idum(7))	!so nocor,yhio,yhig,idum=40 bytes
cc	 write(19,rec=1) iniver,
cc     & ndev,nfile,nset,nfile,kfile,pfile,iydat,
cc     &  nmax,ntot,nswpmax,nomit,cjump,concA,concB
c	  write(19,rec=1) iniver,
c     &	ndev,nfile,nset,nocor,yhio,yhig,idum,kfile,pfile,iydat,
c     &	nmax,ntot,nswpmax,nomit,cjump,concA,concB,
c     &	nbad1,isbad,iebad,nfileb,kfileb,pfileb,idiskq,
c     &	tresolo,tresolg,tresolob,tresolgb,
c     &	onetcrit,tcvals,tcrit0,qfile,nvalsav,ylo,yhi,
c     &	tcseq,opendown,oneshut,adcfil1,
c     & 	ylogap,yhigap,nvalgap,yloop,yhiop,nvalop,
c     &	neq,ieq,jeq,ieq0,jeq0,ncomp,
c     &	((jumpomit(i,j),i=1,nomit(j)),j=1,nset)
cc     &	((jumpomit(i,j),i=1,nomit(j)),j=1,nset),dummy
c	DEALLOCATE(idum)
c	CLOSE(unit=19)
cc
cc============end of temp write
...
c
	if(prefit) call GETPAR(AMEAN(1,1),AREA(1,1),SD(1,1),IDtype,
     & ANTRUE(1),AHIST,THIST,OBHIST,Ylow(1),yhigh(1),Ncomp(1),
     & gamdst,isg)		!ID=IDTYPE
	if(prefit) GOTO 204		!TO DISPLAY FITTED CURVE
c
c======fix arrays (antrue, nfit etc) in ekerr
	if(nset.eq.1) then
	   if(errors) call EKERR(YLOW,YHIGH,ncomp(1),nfit(1),
     &    ERRORS,antrue(1),gamdst,isg,idtype,ghist,titlep,
     &    excreg,yexclow(1),yexchigh(1),isdcon,anfit,
     &    nsweep(1),nnull,nfitt)
...
4		continue
     ...
     &     ACCESS='APPEND',FORM='FORMATTED',CARRIAGE CONTROL='FORTRAN')
		print 911,prtfil
		write(8,911) prtfil
911		format(' Print file, EKDIST.PRT, renamed as ',a40)
		CLOSE(unit=8)
	   endif
	endif
	END
