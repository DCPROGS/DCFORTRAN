...
c
c Modif 02/23/01 06:19pm Yinf added as paramater for Popen curves
c   i.e. nmod=8-11, 13-15 (nmod=12 already has a maximum parameter)
c
c Modif 01/11/97 06:49am Change method for fixing parameters.  Up to now
...
c
c For Popen curves have no automatic guesses but can set guess for Ymax
c for the first set here
	if(mset.eq.1) then
	   if(Popens.and.jfix(1).ne.1) then
		j=juse(1)
      	ymax2=-1.e37
      	do i=1,nj(j)
         	   if(Yobs(i,j).gt.ymax2) then
			ymax2=Yobs(i,j)
		   endif
		enddo
		theta(1)=ymax2
...
c	   call DO_AoV(xobs,yobs,w,jset,nj,niobs,njset,
c     &  	kmax,theta,setx,kfit)
c For sep fit of Popen curves, after first fit set these values as
c defaults for subsequent fits, except that Yinf set to max(Y), if it has
c not been fixed (would be fixed=1 for real Popen curves). Here set
c thetgues(i,j) for rest of sets to be fitted (use this to set theta() for
c the current set below.  Fixed values will be copied too.
	   if(Popens.and.mset.eq.1) then
		do i=1,kmax
		   do m=1,nsfit
			j=juse(m)
			thetgues(i,j)=theta(i)
		   enddo
		enddo
	   endif
c
...
c these are separate fits that FOLLOW ifitmode=4,5)
	   if(mset.gt.nsfit) then
		if(ifitlast.le.3) then		!previous ifitmode
...
c For popen curves now set Yinf=theta(1) to max(Y) for the current set, if it
c has not been fixed (would be fixed=1 for real Popen curves).
	   if(Popens.and.jfix(1).ne.1) then
      	ymax2=-1.e37
      	do i=1,nj(jset)
         	   if(Yobs(i,jset).gt.ymax2) then
			ymax2=Yobs(i,jset)
		   endif
		enddo
		theta(1)=ymax2
