...
c
c Normalise wrt fitted maximum -option for Hill/Langmuir and Popen curves
c NB after separate fits (ifitmode=2), the guesses in thetgues() have been
c replaced (at end of CVSIMP) by the fitted params for each set, for use here
	...
	if(fitted.or.nodata) then
     ...
     &	 ifitmode.eq.2.and.iflag.eq.3) then	   !separate fits displayed -now normalise?
		if(alpha) then
		   call DCASK(
...
c for ymax k=2,4,6...for Langmuir, k=2,5,8 for Hill, for each component in one set
			if(nmod.eq.26.or.nmod.eq.27) then
			   ...
			   ymaxj(j)=thetgues(1,j)	!Ymax=theta(1) for Popen curves
			endif
		   enddo
