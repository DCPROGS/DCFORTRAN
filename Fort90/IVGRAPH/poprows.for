	subroutine POProws(ixlo,iylo,iyhi,strings,n,ic,icf,icup,ibk,
...
c 	   define help strings
	   ...
	   helpst(nhelp+2)='  Move bar with arrows and Hit Enter, or'
	   helpst(nhelp+3)='  click with Left mouse button '
	   helpst(nhelp+4)='CANCEL OPTION:'
	   helpst(nhelp+5)='  Hit Del to or click Central mouse button'
	   helpst(nhelp+6)='LEAVE MENU:'
	   helpst(nhelp+7)='  Hit ESC or Right mouse button'
...
c1	call mouse_update(ibutton,ix,iy)
	if(kbhit()) then
	   ...
	   else if(ival.eq.13) then		!ESC or ENTER
		if(m.gt.1) then
		do j=1,m-1
		   if(iline.eq.kline(j)) goto 1
		enddo
		endif
		if(m.le.n) then
		   kline(m)=iline
	   ...
	   else if(ktype.eq.8.and.ival.eq.83) then
		if(m.gt.1) then
		 j=1
		 do while (iline.ne.kline(j))
		   j=j+1
		   if(j.eq.m) goto 1
		 enddo
		 if(j.lt.(m-1)) then
	   ...
	   else if(ival.eq.27) then		!ESC or ENTER
	  	goto 98
	   else
...
c highlight line on which mouse clicked
	  ...
	  if (ix.gt.ixlo+5.and.ix.lt.ixhi-5.and.iy.gt.iylo1+16.
     &           and.iy.lt.iyc0+16) then		!click on a line
		if(m.gt.1) then
