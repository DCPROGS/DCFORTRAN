	subroutine POPadd(ixlo,iylo,iyhi,strings,n,ic,icf,icup,ibk,
     & title,helps,nhelp,iline,charout,ival,kline,addstr)
...
c
	...
	if (add) then
	   ...
	   nadd=NBLANK1(addstr)
	   do i=1,n
		newstr(i)=strings(i)(1:lenmax)//'  '//addstr(1:nadd)
	   enddo
	else
	   do i=1,n
		newstr(i)=strings(i)
...
c with ixc0=-1 instead
	if(add) then
	   call windim(ixlo,iylo,ixhi,iyhi,idimen,newstr,n,ixc0,iyc0)
	else
...
1	continue	!return to look for next keyboard/mouse hit
	if(kbhit()) then
	   ...
	   else if(ktype.eq.2.and.ival.eq.32) then  ! Space bar
		if(m.gt.1) then
		  do j=1,m-1
     ...
     &                                  ic)
		   call ISETSTRING(ixc0,jy,newstr(iline),ic,
     &	   		icup,ibk,chararray(i))
		   m=m+1
		   call rdgwin1(ixlo,iylo,ixhi,iyhi,array2)
	  	   goto 1
...
6	   	   	   if(i.lt.n) then
     ...
     &				ic,chararray(i))
		   ...
		   if(m.gt.1) then
			do j=1,m-1
			   if(i.eq.kline(j)) then
				call FILLWIN(ixlo+5,jy,ixhi-5,jy+16,ic)
				call ISETSTRING(ixc0,jy,newstr(i),ibk,invc,
     &				ic,chararray(i))
			   endif
			enddo
		   endif
...
c           highlight line on which mouse clicked or delete
     ...
     &		chararray(k))
		...
		call ISETSTRING(ixc0,ky,newstr(k),ibk,invc,ic,chararray(k))
		kline(m)=iline
		if(m.lt.n) m=m+1
111		continue
		call rdgwin1(ixlo,iylo,ixhi,iyhi,array2)
