...
c
c Modif 09/22/97 05:34pm to remove 'zoom' and to fix rescale so it
c works like in old version!
...
c
	...
	dimension zseq(10)
	character*1 ans,UC
	character*33 oldfile,printfil,metafil,printer
	character*40 qfile
...
c	logical bigplot,allpnt
	logical sqrty,slock,bad,off,vert,present,plotcols
	logical doframe,autplt,draft,landplot,landscap,blank
	logical caplock,debug,mono,monsav,redrawn,meta,out
...
c      character*1 ch, fndcur   !fndcur is now integer
	...
	common/DMENU/ifonb,csizem,ifont2,nboxlast,nblast
...
c	icb=0	!background colour for dialog box 1
...
c	ict2=12	!text colour for dialog box 2
...
c	csizb=1.7   !character size for boxes
	cbig=3.0
...
c (2) Initialisation ONLY when AUTPLT=true
	if(autplt) then
	   ...
	   thick=thbig
	   fitted=ncurvc.gt.0
c	   allpnt=.true.		!plot all points if ndat>2048
	   if(ntext.gt.0) then
	      do i=ntext+11,30
	   ...
	   if(size(10).eq.0.0) size(10)=csize		!title
	   if(size(6).eq.0.0) size(6)=0.8*csize		!param text
	   if(size(7).eq.0.0) size(7)=csize
	   if(size(8).eq.0.0) size(8)=csize
	   if(size(9).eq.0.0) size(9)=csize
	   if(size(10).lt.4.) then
		do i=1,30
		   size(i)=1.4*size(i)
c		   wpots(i)=size(i)*4.		!size in points=4*size
		enddo
	   endif
	   size(7) =18./4.	!18 point
	   size(8) =20./4.	!20 point
	   size(9) =20./4.
	   size(10)=16./4.
c	   do i=1,30
c		wpots(i)=size(i)*4.
c	   enddo
c	   if(wpots(10).lt.16.) then	!10=title
c		 do i=1,30
c		    size(i)=1.4*size(i)
c		    wpots(i)=size(i)*4.
c		 enddo
...
c	mono=.true.			!monochrome display (except DCMENU)
	...
	if(ifont.lt.0) ifont=1	!simplex (on 0-11 scale)
	ifont2=0	!copy for common/dmenu/
...
c	ifsav=ifont		!save input value (if there is one)
	...
	thick=thbig
	thfacsml=0.6		!line thickness factor for small plots
c	s=0.8*csize !default symbol size
c	if(syms(1).le.0.) then
c	   do i=1,ndimd
c		syms(i)=s
c	   enddo
c	endif
...
c	rlt=0.20		!default thickness, unless reset
	rlt=0.5	      !default thickness, -start thicker
	do i=1,100
	  rlth(i)=rlt
	enddo
 	rlth(22)=0.3     !frame
c	rlth(26)=0.3	!param value text
c
c 	RLTH(23)=0.5		!labels
c 	RLTH(24)=0.5		!numbers
c 	RLTH(25)=0.5		!title
c 	RLTH(26)=0.25	!param value text
	do i=31,50		!new text
		rlth(i)=0.5
	enddo
	if(isval.eq.1) then
...
c	Jflag=0
	...
	out=xv.lt.xmin.or.xv.gt.xmax.or.yv.lt.ymin.or.yv.gt.ymax
	if(k.eq.1.or.out) then
...
c	if(xv.lt.x1.or.xv.gt.x2) goto 313
...
c
c	goto(11,140,40,13,221,991,155,301,999,306,401,401,152,152,154)ikey
	goto(11,151,40,13,221,991,155,301,999,306,401,401,152,152,154)ikey
c
c MORE OPTIONS
154	continue
...
c	   xtic=xtsav
c	   ytic=ytsav
c	else if(ikey.eq.1) then
c	   if(.not.zoomed) then
cc		xminsav=xmin
cc		xmaxsav=xmax
cc		yminsav=ymin
cc		ymaxsav=ymax
...
c147	   continue
c	   call WDIALOG(1,
c     &      'Mark BOTTOM LEFT corner of new display with cursors',ict)
c	   call CURPOS(xlo,ylo)
c	   call CURSOR(ikey,x1,y1)
c	   ch=char(ikey)
c         call spagra(x1,y1,xmin,ymin)
c	   XMIN1z=XMIN
c	   ymin1z=ymin
c	   if(logx) xmin1z=10**xmin
c	   if(logy) ymin1z=10**ymin
c	   if(sqrty) ymin1z=ymin*ymin
c	   xmin2=xmin
c	   ymin2=ymin
c	   call jSYMBOL(x1,y1,4,csize,12,idev)   !mark bottom left with red diamond
c	   call WDIALOG(1,
c     &      'Mark TOP RIGHT corner of new display with cursors',ict)
c	   x1=x1+200
c	   y1=y1+150
c	   if(x1.gt.xhi) x1=xhi
c	   if(y1.gt.yhi) y1=yhi
c	   call CURPOS(x1,y1)
c	   call CURSOR(ikey,x1,y1)
c	   ch=char(ikey)
c         call spagra(x1,y1,xmax,ymax)
c	   XMax1z=XMax
c	   ymax1z=ymax
c	   if(logx) xmax1z=10**(xmax)
c	   if(logy) ymax1z=10**(ymax)
c	   xmax2=xmax
c	   ymax2=ymax
c	   call jSYMBOL(x1,y1,4,csize,12,idev)   !mark bottom left with red diamond
c	   if(xmin.gt.xmax.or.ymin.gt.ymax) then
c
c		call BELL(2)
c	      call WDIALOG(1,
c     &      'Maximum x,y must be greater than minimum -try again',12)
c		goto 147
c	   endif
cc	Fix tic length only (xminz etc not used)
c	   call FIXAX(xmin,xmax,xminz,xmaxz,xtic,ilog)
c	   call FIXAX(ymin,ymax,yminz,ymaxz,ytic,ilog)
c	   if(xcross.lt.xmin) xcross=xmin
c	   if(xcross.gt.xmax) xcross=xmax
c	   if(ycross.lt.ymin) ycross=ymin
c	   if(ycross.gt.ymax) ycross=ymax
c	else
c	   goto 140		!no valid key
c	endif
c	if (idraw(6).ne.0) idraw(6)=-2       !Parameter values drawn at default posn
c	if (idraw(10).ne.0) idraw(10)=-2	!so title, if present, drawn at default posn
c	if (idraw(8).ne.0) idraw(8)=-2       !X axis label drawn at default posn
c	if (idraw(9).ne.0) idraw(9)=-2       !Y axis label drawn at default posn
c	do 341 i=31,80
c341	idraw(i)=-2		!all numbers at default posn
c	goto 306		!redraw
cc
...
c
3041	continue
c	call DEFOLT2r(xmin1z,xmax1z,defolt)
c	call QDIALOG(1,'Xmin, Xmax',defolt,ict,cans)
c	call GETINP2r(cans,xmin1z,xmax1z)
c	XMIN=XMIN1z
c	XMAX=XMAX1z
c	if(logx) then
cc=======what is next line for??!
cc====	   if (xmin1z.le.0) xmin1z=0.01*(xmax1z-xmin1z)
c	   if(xmin1z.lt.1.e-30.or.xmax1z.lt.1.e-30) goto 3041
c	   XMIN=alog10(XMIN1z)
c	   XMAX=alog10(XMAX1z)
c	endif
c	if(xmax.le.xmin) goto 3041
c	call DCMENU(-1,4,Lb,text,0,0)	!delete box 1 only
c	Lb(1)=-1
c	call DCMENU(-1,4,Lb,text,icol0,icol2)	!draw box 1 only (italic)
cc	Lb(1)=-1
cc	call DELBOX1(1)	!delete single box
cc	call DRAWBOX1(-1,text(1),icol0,icol2)	!redraw single box (italic)
c	goto 112
c	goto 111
c209	format(2g13.6)
c17	continue
c3051	continue
c	call DEFOLT2r(ymin1z,ymax1z,defolt)
c	call QDIALOG(1,'Ymin, Ymax',defolt,ict,cans)
c	call GETINP2r(cans,ymin1z,ymax1z)
c	YMIN=YMIN1z
c	YMAX=YMAX1z
c	if(ymax.le.ymin) then
c	   call BELL(1)
c	   call OPENDIALOG(2,icf2,.true.)	!draw dialog box #2
c	   call WDIALOG(2,'Ymin<Ymax!',12)
c	   goto 3051
c	endif
c	if(logy) then
cc=======what is next line for??!
cc====	   if (xmin1z.le.0) ymin1z=0.01*(ymax1z-ymin1z)
c	   if(ymin1z.lt.1.e-30.or.ymax1z.lt.1.e-30) goto 3051
c	   YMIN=alog10(YMIN1z)
c	   YMAX=alog10(YMAX1z)
c	endif
c	if(sqrty) then
c	   ymin=sqrt(ymin1z)
c	   ymax=sqrt(ymax1z)
...
c=============
	if(logx) then
	   xmin1=10**(xmin)	!non-log units
	   xmax1=10**(xmax)	!non-log units
	endif
	call DEFOLT2r(xmin1,xmax1,defolt)
	call QDIALOG(1,'Xmin, Xmax',defolt,ict,cans)
	call GETINP2r(cans,xmin1,xmax1)
	if(xmax1.le.xmin1) goto 3041
	if(logx) then
...
3051	continue
	if(logy) then
	   ymin1=10.**(ymin)	!non-log units
	   ymax1=10.**(ymax)	!non-log units
c	else if(sqrty) then
c	   ymin1=ymin**2
c	   ymax1=ymax**2
	endif
	call DEFOLT2r(ymin1,ymax1,defolt)
	call QDIALOG(1,'Ymin, Ymax',defolt,ict,cans)
	call GETINP2r(cans,ymin1,ymax1)
	if(ymax1.le.ymin1) then
	   call BELL(1)
	   call OPENDIALOG(2,icf2,.true.)	!draw dialog box #2
	...
	if(logy) then
	   if(ymin1.lt.1.e-30.or.ymax1.lt.1.e-30) goto 3051
	   ymin=alog10(ymin1)
	   ymax=alog10(ymax1)
c	else if(sqrty) then
c	   if(ymin1.lt.0.0.or.ymax1.lt.0.0) goto 3051
c	   ymin=sqrt(ymin1)
c	   ymax=sqrt(ymax1)
	else
	   ymin=ymin1
...
c start of arrow- need to store start/end points, and number of arrows,
c so they can be reproduced when graph redrawn
	if(ikey.eq.1.or.ikey.eq.6.or.ikey.eq.11.or.ikey.eq.12) then
	   if(ikey.eq.12) then
     ...
     &	defolt,ict,cans)
	   ...
	   call draw_arrow(ikey,narrow,xe,xb,ye,yb,ixfix,iyfix,xv,yv,
     &  csize,idev,xabs1,yabs1,xabs2,yabs2,rlth,.false.,iver,ihead)
	call broken(0)
...
654      continue
	   ...
	   call WDIALOG
     &(1,'Sorry this option is not available for computers with less
     &then 32MB ',ict)
	   goto 777
c=	     wmeta=.true.
c=	     idev=1
...
9876	continue
	...
	scalfac=1.0
	call papenq(xp,yp,itype)
c     call setcls(.false.,icol,.false.,.false.,0)
	xlo=xlo2		!restore screen GRAPHBOUNDARY
	xhi=xhi2
	ylo=ylo2
	yhi=yhi2
