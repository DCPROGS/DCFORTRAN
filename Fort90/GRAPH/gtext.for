      subroutine gtext(parval,xtitle,ytitle,title1,cnumx,cnumy,
     &  cexpx,cexpy,newtext,numbx,numby,ntext,inumx,inumy,logx,logy,
     &  rx,ry,ijus,icol,rotate,ifnt,idraw,csize,ibk,imd,rlth,
...
c*************************************************************************
	...
	real    	   RX(100),RY(100),rotate(100),csize(30),rlth(100)
...
c	      str(k+4+numbx+numby)= nwtext(1:nreal(k))
	      ...
	      thick(k+4+numbx+numby)=rlth(k+30)
...
c			    if(jdraw(i).ne.-1) then
c			       call lincols(ibk,idev)
c			       call FBOX1(xg(i),yg(i),str(i),ANGLE(i),
c     &                  SIZE(i),JUS(i),xbox,ybox,1)
c			    endif
			    iline=8
			    goto 777
			endif
...
444	            call QDIALOG(1,'Color [F1=HELP]',defolt,11,cans)
     ...
     &			SIZE(i),JUS(i),xbox,ybox,1)
			      ...
			      call linwid(thick(i))
				call justifystring(xg(i),yg(i),
     &			str(i)(1:nl),angle0,size(i),jus(i))
			first=0
			goto 25
			endif
			call wrgwin1(ixlo,iylo,ixhi,iyhi,array0) !display
...
c	 ry(10)=yg(4)
	 ...
	 csize(7)=size(5)
	 do k=1,numbx
	   	if((.not.logx).or.(logx.and.inumx.ne.1))  then
		   cnumx(k)=str(k+4)
		endif
...
c		ry(k+30)=yg(k+4)
	 ...
	 enddo
	 do k=1,numby
	   	if((.not.logy).or.(logy.and.inumy.ne.1))  then
