...
c
	...
	logical sqrty,slock,bad,off,vert,bigplot,present
...
c	      call LOADVTB('2'//'W'//char(rlth(j)))
	...
	if(.not.mono) call lincols(icol(j))		!colour for jth histo bars
...
c=	if(off.or.bad) goto 20		!omit LO bin if Y off scale
	if(.not.mono) call lincols(icol(10))		!colour end bins
...
c	call qdialog(1,'OPTIONS:1.Black&White  2.Color/Grey shades',
c     &defolt,ict,cans)
c	call GETINPi(cans,iopt)
c	if(iopt.lt.1.or.iopt.gt.2) iopt=1
c	if(iopt.eq.1) then
c         call setcls(.true.,icol,autplt,plot,isetcol)
c	endif
...
c	if(bigplot.and.(.not.allpnt)) then
c	   call INTCONV(ndelt,cnum1)
c	   ans='Y'
c	   call DEFOLTa(ans,defolt)
c	   call QDIALOG(1,'Every nth point displayed, n = '
c     &	  //CHARNB(cnum1)//'; plot ALL points',defolt,ict,cans)
c	   call GETINPa(cans,ans)
c	   if(ans.ne.'N') allpnt=.true.
c	endif
	call WDIALOG(1,
...
1042	continue
	...
	xlo=xloq*xpp/xp	! for GRAPHBOUNDARY- same for all
	xhi=xhiq*xpp/xp
	ylo=yloq*ypp/yp
	yhi=yhiq*ypp/yp
	vxlo=vxloq*xpp/xp	! for GRAPHBOUNDARY- same for all
	vxhi=vxhiq*xpp/xp
	vylo=vyloq*ypp/yp
	vyhi=vyhiq*ypp/yp
	call GRAPHBOUNDARY(xlo,xhi,ylo,yhi)
