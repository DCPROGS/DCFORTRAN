...
c---For scale factors (see SCALCURV)
	...
	logical xdone,ydone,meta,plotrue
c///
c for interpolation
	...
	logical plotonly,mulpage,help,present
	real theta(kmax)
	dimension zseq(10)
...
c	real*4 yhline1(10)		!record y value
c	real*4 xhlb1(10),xhle1(10)     !start/end of HLINE
c	real*4 xvline1(10)		!record x value
c	real*4 yvlb1(10),yvle1(10)     !start/end of VLINE
c=====      integer fndcur
...
c	RLTH(1) to RLTH(20) = 0.2 draw
	RLTH(21)=0.3	!axis
 	RLTH(22)=0.3	!frame
c	RLTH(23)=0.2		!labels
c	RLTH(24)=0.2		!numbers
	RLTH(25)=0.3		!title
c	RLTH(26)=0.10	!param value text
...
1083	if(landscap) then
	   xlo=0.15*xp		! set
	   if(calbarY) xlo=0.05*xp	!make wider
	   xhi=0.85*xp		! display
	   ylo=0.2*yp! location
	   yhi=0.75*yp		! screen
	else
...
c	if(.not.mono)
      call lincols(icol(25))		!colour for title
	call graspa(rx(10),ry(10),xsc,ysc)
...
c	if(.not.mono)
      call lincols(icol(26))		!colour for params
	call graspa(rx(6),ry(6),xsc,ysc)
...
c	   if(.not.mono)
         call lincols(icol(i+30))		!colour for text(i)
...
c	      if(.not.mono)
            call lincols(icol(28))		!colour for c-logo
...
c	      if(.not.mono)
            call lincols(icol(29))		!colour for v-logo
...
c	   if(.not.mono)
         call lincols(icol(j0))	 !col for jth data set: symbols+line+SD
...
c        Put do 215 loop + interp drawing into DRAWDAT, to draw curve # j
     ...
     &	 weight,kwi,kwj,icw)
	   ...
	   if(.not.plot) then
	      ...
	      call jSYMBOL(xsps+8.,ysps+1.,isym(j),syms(j),icol(j0))     !symbol if isym>0
	   endif
	   ysps=ysps-6.
...
c	if(.not.mono)
      call linCOLs(icol(j0+10))		!colour for jth calc curve
...
c	if(ikey.gt.10) goto 154
	...
	if(ikey.eq.1) then
	   mono=.not.mono
	   if(mono) call lincols(15)		!bright white
...
c	if(.not.mono)
	...
	if(itit.eq.1) then	!delete existing title first
	   call lincols(icol(71))		!colour for title
	   call graspa(rx(10),ry(10),xsc,ysc)
...
401	continue
	...
	if(ikey.eq.11.or.ikey.eq.12) then	!x,y axis labels
	   ...
	   x=(rx(n))
	   y=(ry(n))
	   call lincols(icol(71))	!delete existing title
	   nlx=NBLANK1(oldtext)
	   call setfnt(n)
...
406	if(ikey.eq.1) then
     ...
     &      'Mark TOP RIGHT corner of new graph with cursors',ict)
	   ...
	   rxlo=xlo/100.	!as percent
	   rxhi=xhi/100.	!as percent
         rylo=ylo/70.	!as percent
	   ryhi=yhi/70.	!as percent
...
2351  continue
	...
	if(ikey.eq.1.or.ikey.eq.2) then
 	...
 	text(1)='<-,->: Left,Right '
 	text(2)='Home,End: Colour'
	text(3)='/\,\/: Up,Down'
	text(4)='+,- : Inc,Dec size'
	text(5)='x,/ : Inc,Dec font'
	text(6)='PgUp: Inc angle'
	text(7)='PgDn: Dec angle'
	text(8)='Ins : Sensitivity'
	text(9)='Del : Delete'
	text(10)='b : Boxes'
	call DCMENU(10,5,Lb,text,icol1,icol2)

      call GINONUMB(cnumx,cnumy,cexpx,cexpy,numbx,numby,inumx,inumy,
...
1551	continue
	...
	text(9)=' 9. PLOT PORTRAIT  '
 	text(10)='10. CANCEL PLOT '
...
1552	call CKEY(ch,ikey)
	...
	if(ikey.lt.1.or.ikey.gt.10) goto 1552	!no valid key
      call DCMENU(-ikey,5,Lb,text,0,0)	!delete
	Lb(ikey)=-1
 	call DCMENU(-ikey,5,Lb,text,icol0,icol2)	!redraw (not italic)
c	plot=.true.
	call DEFOLTr(scalfac,defolt)
	call QDIALOG(1,'Scale up/down ?: scale factor [0.8 to 1.2]',
     &  defolt,ict,cans)
	...
	iopt=1
	call defolti(iopt,defolt)
	call qdialog(1,'OPTIONS:1.Black&White  2.Color/Grey shades',
     &defolt,ict,cans)
	call GETINPi(cans,iopt)
	if(iopt.lt.1.or.iopt.gt.2) iopt=1
	if(iopt.eq.1) then
c	   mono=.true.
c         call setcls(mono,icol,autplt,plot,0)
	...
	if(ikey.ge.2.and.ikey.le.5) goto 72
...
71	continue	!whole page
	...
	if(Lb(i).eq.-1) then
	  Lb(i)=1
	  call DCMENU(-i,5,Lb,text,0,0)		!delete box i only
	  call DCMENU(-i,5,Lb,text,icol0,icol2)	!draw box 2 only (italic)
	endif
711	continue
...
72	continue	!quarter page positions
	do i=1,7
	if(Lb(i).eq.-1.and.i.ne.ikey) then
	  Lb(i)=1
	  call DCMENU(-i,5,Lb,text,0,0)		!delete box i only
	  call DCMENU(-i,5,Lb,text,icol0,icol2)	!draw box 2 only (italic)
	endif
	enddo
c	call DCMENU(-ikey,5,Lb,text,0,0)		!delete box 1 only
	csize=csize*cfacsml
...
c ==============================6=Define position of plot on screen
73	continue
...
c 	AND ABOUT 1250 UNITS/INCH IN PORTRAIT. Thus landscape gives better resolution
	...
	Lb(6)=-1
	call DCMENU(-6,5,Lb,text,0,0)		!delete box #6 only
...
75	continue
 	...
 	if(iopt.eq.1) then
     		...
     		call setcls(.false.,icol,.false.,.true.,2)
 	endif
...
c	vylo=0.
c	vyhi=ypp
c	xlo=s1*xpp	! for GRAPHBOUNDARY- same for all
c	xhi=s2*xpp
c	dyp=((s2-s1)*xpp*yp)/(ypp*xp)
c	ylo=((1-dyp)/2)*ypp
c	yhi=ylo+dyp*ypp
c	xpp=297.
c	ypp=210.
	xhi=xhiq*xpp/xp
	yhi=yhiq*ypp/yp
c	xlo=xloq*xpp/xp	! for GRAPHBOUNDARY- same for all
c	ylo=yloq*ypp/yp
...
c	vxlo=vxloq*xpp/xp	! for GRAPHBOUNDARY- same for all
c	vylo=vyloq*ypp/yp
	call GRAPHBOUNDARY(xlo,xhi,ylo,yhi)
	if(.not.sqrty) call gSCALE(xmin,xmax,ymin,ymax,is)
	if(sqrty) call gSCALE(xmin,xmax,sqrt(ymin),sqrt(ymax),is)
