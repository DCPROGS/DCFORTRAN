      subroutine VHIST5(XVAL,YVAL,NDAT,icurvd,ncurvd,ijoin,ndimd,
...
c
c Modif 11/14/97 10:44am VHIST5 has extra parameters for declared dimensions
c for arrays
c	real XVAL(0:ndv1,ndimd),YVAL(0:ndv1,ndimd)
c	real XCAL(ndc1,ndimc),YCAL(ndc1,ndimc)
c	real theta(kmax)
c rather than
c	real XVAL(0:511,ndimd),YVAL(0:511,ndimd)
c	real XCAL(2048,ndimc),YCAL(2048,ndimc)
c	real theta(20)
c Modified as in VPLOT to make use of xmin, xmin1 etc more tidy
...
c
	...
	LOGICAL PLOT,logx,logy,down,pon,fitted,croset,equal
...
c
	ihtype=2	!tells VHELP that call was from VHIST
c
c Initialisations:
...
c	angle(8)=0			!0 set above
	...
	do i=1,30
	   size(i)=size(i)*csize
	enddo
	size(6) =10./4.	!12 point for param values
	size(8) =18./4.	!18 point for axis labels
	size(9) =18./4.	!ditto for axis number
...
c==	call SETATTR(112)		!'inverse' attrib seems nec while in graphics
c
	idev=0			!screen
...
c	if(iscal.ne.0.and.ilog.ne.0) goto 133	!scale for logs first
	if(iscal.ne.0) then
	   if(ilog.ne.0) then
...
c     Fill background colour
	...
	if(ilabel.lt.0) then
	   ...
	   if(logy) titley='log(number) '
c/	   if(ilog.eq.4) titley='log((Y-Y0)/(Ymax-Y)) '
	endif
...
c calculated internally.
	...
	call LAXES2(xmin,xmax,ymin,ymax,xtic,ytic,xcross,ycross,
     ...
     & rlth,inumx,inumy,xwbase,ilog,doframe,mono,icol,xc,yc,
...
1361	continue
	...
	if(idraw(6).eq.-2) then
	    idraw(6)=1	!so not drawn with box by default
...
c is out of range (so won't draw right to edge).
	...
	if(jflag.eq.0) then
	   ...
	   call  clipping(xmin,ymin,xmax,ymax,xv0,yv0,xv,yv,iflag)
	   if(iflag.eq.0) then
		xv0=xv
		yv0=yv
		goto 220
...
C====SET FOR REDUCED SIZE PLOT
	   ...
	   INQUIRE(file='vhist.ini',exist=present,flen=len)
         OPEN(unit=17,file='VHIST.INI',status='UNKNOWN',
...
3041	continue
	...
	if(ncurvc.gt.0) then
	   do i=1,ncurvc
		j=icurvc(i)
		if(xmin.gt.xminsav) x1calc(j)=xmin2		!non-log
		if(xmax.lt.xmaxsav) x2calc(j)=xmax2		!non-log
	   enddo
	endif
...
c so they can be reproduced when graph redrawn
	if(ikey.eq.1.or.ikey.eq.6.or.ikey.eq.11.or.ikey.eq.12) then
     ...
     &	defolt,ict,cans)
	   ...
	   call draw_arr(ikey,narrow,xe,xb,ye,yb,ixfix,iyfix,xv,yv,
...
c===following stuff replaced by FIXCURV (this should never be reached
c   with ikey =6,7 (symbol type/size) which are used only in VPLOT)
c	j1=1
c	if(ikey.eq.6.or.ikey.eq.7.or.ikey.eq.8) then	!data
c	   if(ncurvd.gt.1) then
c		j1=0
c		call INTCONV(ncurvd,cnum1)
c		call QDIALOG(1,'Histogram data number (1 to '
c     &	  //CHARNB(cnum1)//') [all] ',' ',ict,cans)
c		call GETINPi(cans,j1)
c	   endif
c	   if(j1.gt.0) then
c		j2=icurvd(j1)	!selected set
c	   else
c		j2=icurvd(1)	!use 1st set plotted for 'present value'
c	   endif
c	else if(ikey.eq.9) then		!calc curve
c	   if(ncurvc.gt.1) then
c		j1=0
c		call INTCONV(ncurvc,cnum1)
c		call QDIALOG(1,'Curve number (1 to '
c     &	  //CHARNB(cnum1)//') [all] ',' ',ict,cans)
c		call GETINPi(cans,j1)
c	   endif
c	   if(j1.gt.0) then
c		j2=icurvc(j1)	!selected set
c	   else
...
c	if(ikey.eq.8) then
c	   i=ijoin(j2)
c	   call DEFOLTi(i,defolt)
c	   call QDIALOG(1,'Line type (0 to 8)',defolt,ict,cans)
c	   call GETINPi(cans,i)
c	   if(j1.gt.0) then
c		ijoin(j2)=i
c	   else
c		do 1191 j2=1,ncurvd
c		j=icurvd(j2)
c1191		ijoin(j)=i
c	   endif
c	else if(ikey.eq.9) then
c	   i=iline(j2)
c	   call DEFOLTi(i,defolt)
c	   call QDIALOG(1,'Line type (0-5, 10-18)',defolt,ict,cans)
c	   call GETINPi(cans,i)
c	   if(j1.gt.0) then
c		iline(j2)=i
c	   else
c		do 1201 j2=1,ncurvc
c		j=icurvc(j2)
c1201		iline(j)=i
c	   endif
c	endif
c	call DCMENU(-ikey,5,Lb,text,0,0)	!delete box #ikey only
c	Lb(ikey)=-1
c	call DCMENU(-ikey,5,Lb,text,icol0,icol2)	!redraw (italic)
c	goto 221	!back to menu
c
c##########################################################################
...
301	continue
c vhsq5 now has ndv1, ndc1 and kmax added to paameters
	call VHSQ5(xval,yval,xcal,ycal,ndimd,ndimc,
