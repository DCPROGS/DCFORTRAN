...
c Next line added to prevent error if try to delete boxes before any drawn
	...
	en=float(nbline)
	w=(vxhi-vxlo)/(1.04*en)
c	h=(ymenu-vylo)/(1.04*nline)
	h=(ymenu-vylo)/(1.14*3)
	gh=0.04*w		     		!horizontal gap between boxes
	gv=0.08*h			!vertical gap between boxes
	gh2=vxlo+gh/2
	gv2=vylo+gv/2
	ti1=h/4.0		!start point for text
c	ti2=w/8.0		!start point for text
	ti2=w/16.0		!start point for text
	imax=0
	do i=1,nbox
...
c	if(imax.gt.12) imax=12
	do i=1,imax
	 if(Lbox1(i).ne.0) then
	    if(Lbox1(i).gt.0) then
		 call setfnt(ifont)    	!set font for text
