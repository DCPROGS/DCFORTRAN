...
c Modif 03/26/97 02:28pm to calculate suitable size for datcop
	...
	if(ncurvd.gt.0) then
	   do j1=1,ncurvd
		j=icurvd(j1)
		ndim=ndim+ndat(j)*8	!for bytes for xval, 4 bytes for yval
	   enddo
	   if(icurvw(j).ge.0) then
		ndim=ndim+4*ndat(j)	!for weight()
	   endif
	endif
	if(ncurvc.gt.0) then
	   do j1=1,ncurvc
		j=icurvc(j1)
		ndim=ndim+ncal(j)*8	!for bytes for xcal, 4 bytes for ycal
	   enddo
	endif
	ndim=ndim+2048+10000
c
c=	ALLOCATE(datcop(46960),int2cop(1000),charcop(100))
c=	ALLOCATE(datcop(200000),int2cop(1000),charcop(100))
...
c==	call QDIALOG(1,
c==     & 'Change name of plot queue file (''N''= abandon): O.K.',
c==     &   defolt,ict,cans)
c==	call GETINPa(cans,ans)
c==	if(UC(ans).ne.'Y') goto 45		!abandon
	if(iopt.eq.1) then
	   call INTCONV(iplot,cnum1)
	   call QDIALOG(1,
...
c	istrec=(jstrec(iplot))	!start record for plot to be overwritten
