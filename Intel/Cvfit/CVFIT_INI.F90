subroutine cvfit_ini(main,initwin,intoggle,itcall,itogbutton,pfilem,initwin_Text2)
...
! Set up complex dialogue box initwin child of imecform1
 initwin = gmCreateComplexDialogueBox(main,190,120,421,540, GALL, 'Welcome ', &
              	gmhpos=gleft,gmvpos=gtop,gmIconFormat=GDLLICON,gmIconFile='MBIG1052')

! Create main panel for form
   initwin_MainPanel=gmCreatePanel(initwin, 0, 0, 421, 540, gmHpos=GCENTRE, gmVpos=GMIDDLE, gmExpand=GON, gmType=GNOBOUNDARY, &
...
! Create text widget initialw_Text1 child of initialw_Panel1
   initialw_Text1 = gmCreateTextEntry(initwin_mainPanel, 150, 10, 300, 25,'CVFIT', 255, GDISPLAY, gmBack1Col=0, gmBack2Col=12, &
...
! Create text widget initialw_Text2 child of initialw_Panel1
   initialw_Text2 = gmCreateTextEntry(initwin_mainPanel, 160, 40, 90, 25,'Version 1.00 ', 255, GDISPLAY, gmBack1Col=0, gmBack2Col=12, &
...
! Create toggle button initwin_Toggle3 child of initwin_Radio2
   inToggle(11) = gmCreateToggleButton(initwin_Radio2, 70, 10, 230, 25, 'Append to existing print file', 1, gmType=G3DRADIO, &
...
initwin_Panel6 = gmCreatePanel(initwin_MainPanel, 0,285 , 420, 100, gmTitle='File:', gmType=GPROJECTED, &
      gmLineBorder=GOUTEREDGE, gmFillCol=140, gmFillBorder=GOUTEREDGE, gmScrollMode=GNOBARS, gmVpos=GTOP)
initwin_Radio3 = gmCreateRadioBox(initwin_Panel6, 20, 20, 380, 70, gmType=GFREEFORM, gmBorderType=GINVERSECHISEL, &
      gmLineBorder=GOUTEREDGE, gmFillCol=141, gmFillBorder=GOUTEREDGE, gmVpos=GTOP)

! Create toggle button initwin_Toggle3 child of initwin_Radio2
   inToggle(14) = gmCreateToggleButton(initwin_Radio3, 70, 10, 230, 25, 'Read existing file', 1, gmType=G3DRADIO, &
      gmVpos=GTOP,gmcallback=9001)

! Create toggle button initwin_Toggle4 child of initwin_Radio2
   intoggle(15) = gmCreateToggleButton(initwin_Radio3, 70, 40, 230, 25, 'Create new file file', 0, gmType=G3DRADIO, &
      gmVpos=GTOP,gmcallback=9002)




initwin_Panel3 = gmCreatePanel(initwin_MainPanel, 0,385 , 420, 130, gmTitle='Setup for this computer', gmType=GPROJECTED, &
      gmLineBorder=GOUTEREDGE, gmFillCol=140, gmFillBorder=GOUTEREDGE, gmScrollMode=GNOBARS, gmVpos=GTOP)
initwin_Panel5 = gmCreatePanel(initwin_Panel3, 20,20 , 380, 100, gmTitle='', gmType=GPROJECTED, &
      gmLineBorder=GOUTEREDGE, gmFillCol=141, gmFillBorder=GOUTEREDGE, gmScrollMode=GNOBARS, gmVpos=GTOP)

! Create text widget initwin_Text2 child of initwin_Panel2
 iniTextq = gmCreateTextEntry(initwin_Panel5, 70, 10, 230, 25,'Computer : '//text1, 255, Gdisplay, gmBack1Col=0, gmBack2Col=0, &
      gmTextCol=16 ,gmScrollMode=Ghorizontal, gmVpos=GTOP)
  
 iniTextq = gmCreateTextEntry(initwin_Panel5, 70, 40, 230, 25,'User : '//text2, 255, GDIsplay, gmBack1Col=0, gmBack2Col=0, &
      gmTextCol=16 ,gmScrollMode=Ghorizontal, gmVpos=GTOP)
 
 iniTextq = gmCreateTextEntry(initwin_Panel5, 70, 70, 230, 25,'Drives : '//drives, 255, GDIsplay, gmBack1Col=0, gmBack2Col=0, &
      gmTextCol=16 ,gmScrollMode=Ghorizontal, gmVpos=GTOP)

! Create button initwin_Button6 child of initwin_MainPanel
   itogButton = gmCreatePushButton(initwin_MainPanel, 210, 515, 210, 25, 'Continue', &
   gmVpos=GTOP,gmType=GDEFAULTBUTTON,gmcallback=itcall) !-40)
  call gmSetWidgetStatus(itogbutton, GSELECTABLE)
   itogButton = gmCreatePushButton(initwin_MainPanel, 0, 515, 210, 25, 'Exit', &
