...
**
** 14/May/98  GPS   Define USE_NO_HANDLE for windows use with all alloc
**                  done by malloc and free. This allows you to use the debug
**                  system to track horrible memory problems. We should really
**                  illiminate the old methods as soon as possible.
**
...
*****************************************************************************/
    ...
    typedef short BOOLEAN;
       ...
       #ifdef USE_NO_HANDLE
       #define M_AllocMem(x)     malloc(x)
       #define M_AllocClear(x)   malloc(x) /* needs memfil() adding */
       #define M_FreeMem(x)      free(x)
       #define M_LockMem(x)      (x)
       #define M_MoveLockMem(x)  (x)
       #define M_UnlockMem(x)    1
       #define M_NewMemSize(x,y) (x = realloc(x,y))
       #define M_GetMemSize(x)   HELP!!! /* needs illiminating from code */
       #else
