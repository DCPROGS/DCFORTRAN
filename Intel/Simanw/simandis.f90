subroutine simandis(main,thetval,nintval,ixval,iyval,izval,elmval,elmset,nevals,ec50val,&
		nsims,npar,nset,imod0,ndimd,ndimc,titlep,simfiles,thtrue,id,iomit,nomit,icallsim,&
		newrecords,igraph,combo1_1,COMBO1_2,nfile)
...
!	include '\intel\common_files\graphics_definitions.f90'
	...
	ALLOCATABLE:: XVAL1,YVAL1,XCAL1,YCAL1,weight,w,icurvw
	real*4 XVAL1(:,:),YVAL1(:,:)
	real*4 XCAL1(:,:),YCAL1(:,:)
	real*4 weight(:,:),w(:,:)
	integer*4 icurvw(:),nj(20)
	integer ndat1(20),icurvd1(20),ijoin1(20),isym(20),jopen(1000),iline1(20)
	real*4 syms(20)
...
logical newfile
integer :: itogButton,callid,ifstat(200),COMBO1_1,COMBO1_2
...
common/pixpos/ixgrid,iygrid,ixpix,iypix,wid,landscape
	...
	enddo
	nsims0=nsims
	OPEN(unit=10,file=simfiles(1),status='UNKNOWN',access='DIRECT', form='BINARY',RECL=1)
	read(unit=10,rec=1) iver1,nsims1
	close(unit=10)
	nsims=0
	do lk=1,nfile
	OPEN(unit=10,file=simfiles(lk),status='UNKNOWN',access='DIRECT', form='BINARY',RECL=1)
	read(unit=10,rec=1) iver1,nsims1
	nsims=nsims+nsims1
	if(iver1.le.101) then
	   ...
	   read(unit=10,rec=1) iver,nsims1,npar,nset,treso,cDATEW,ctimew,mtitle1,simfile,k,kfit,imod0,nlig,&
		 ...
		 (titlep(i),i=1,npar),((thetval(i,j),i=1,npar),j=n1,nsims),((conc(i,j),i=1,nlig),j=1,nset),&
         ((nintval(i,j),i=1,nset),j=n1,nsims1),(ixval(i),i=n1,nsims),&
         (iyval(i),i=n1,nsims),(izval(i),i=n1,nsims),(elmval(i),i=n1,nsims)
	   ...
	   read(unit=10,rec=1) iver,nsims1,npar,nset,imods,mtitles1,npars,treso,cDATEW,ctimew,mtitle1,&
         ...
         ((thetval(i,j),i=1,npar),j=n1,nsims),((conc(i,j),i=1,nlig),j=1,nset),&
         ((nintval(i,j),i=1,nset),j=n1,nsims),(ixval(i),i=n1,nsims),(iyval(i),i=n1,nsims),&
         (izval(i),i=n1,nsims),(elmval(i),i=n1,nsims),((elmset(i,j),i=1,nset),j=n1,nsims),&
	   ...
	   read(unit=10,rec=1) iver,nsims1,npar,nset,imods,mtitles1,npars,treso,cDATEW,ctimew,mtitle1,simfile,&
	     ...
	     (thtrue(i),i=1,npars),(titlep(i),i=1,npar),((thetval(i,j),i=1,npar),j=n1,nsims),&
         ((conc(i,j),i=1,nlig),j=1,nset),((nintval(i,j),i=1,nset),j=n1,nsims),(ixval(i),i=n1,nsims),&
         (iyval(i),i=n1,nsims),(izval(i),i=n1,nsims),(elmval(i),i=n1,nsims),&
         ((elmset(i,j),i=1,nset),j=n1,nsims),ix1,iy1,iz1,(nevals(i),i=n1,nsims)
		mtitles=mtitles1
	    ...
	    read(unit=10,rec=1) iver,nsims1,npar,nset,imods,mtitles,npars,treso,cDATEW,ctimew,mtitle1,simfile ,&
	     ...
	     (thtrue(i),i=1,npars) ,(titlep(i),i=1,npar) ,((thetval(i,j),i=1,npar),j=n1,nsims) ,&
      	 ((conc(i,j),i=1,nlig),j=1,nset),((nintval(i,j),i=1,nset),j=n1,nsims),(ixval(i),i=n1,nsims),&
     	 (iyval(i),i=n1,nsims),(izval(i),i=n1,nsims),(elmval(i),i=n1,nsims),((elmset(i,j),i=1,nset),j=n1,nsims),&
         ix1,iy1,iz1,(nevals(i),i=n1,nsims),(ec50val(i),i=n1,nsims)
	    ...
	    if(iver.le.103.and.imod0.eq.29) then 	!fudge! (imods not kept)
		    ...
		    if(discprt) write(7,80) thtrue(3),thtrue(4)
80		    format(' Model used for simulation had desensitised state',/,&
     	    ' k(+d) = ',g13.6,' k(-d) = ',g13.6)
		    n=0
	    ...
	    if(iver.ge.104) then
		    ...
		    n=0
		    do ij=1,npars		!npars=16 for mod 39
		        thtrsav(ij)=thtrue(ij)		!keep original for print
		        if(ij.ne.3.and.ij.ne.4) then
			    n=n+1
			    thtrue(n)=thtrue(ij)
		        endif
		    enddo
	        else
		    do ij=1,npars		!npars=16 for mod 39 and mod 36
		        thtrsav(ij)=thtrue(ij)		!keep original for print
		    enddo
	        endif
	    endif
		...
		do j=1,i-1	!check all earlier seeds
		   if(ix.eq.ixval(j)) then
			if(iy.eq.iyval(j).and.iz.eq.izval(j)) then
			   ...
			   iomit(i1)=1
			   nomit=nomit+1
			   write(string,fmt='(a34,i8)') 'Run omitted (duplicate seeds) = ',i1
			   CALL GMSETTEXTSETTING(ITTY,string)
			   if(discprt) write(7,27) i1
		   ...
		   sdm=sd/dsqrt(dfloat(n))
			...
			case(9040) ! ok second window plot distributions
				...
				if(imes2.eq.1) then
					...
					n=NBLANK1(title1)	!must exclude ascii 0
					n11=NBLANK1(titlep(ipex))
...
34					format(' Exclude ',i3,' fits that were ABORTWed')
				...
				if(nyval.eq.0) then
					...
					if(ASSOC(ip1,id,imod0)) then
						n=NBLANK1(titlep(ip1))	!must exclude ascii 0
					...
					n=NBLANK1(titlep(ip2))	!must exclude ascii 0
...
11		format('  Plot only values for which parameter ',i3,', '&
	   ...
	   if(nABORTW.gt.0) then
		...
		!	kwj=1							!ditto
			...
			if(ASSOC(ip1,id,imod0)) then
			nb=NBLANK1(titlep(ip1))	!must exclude ascii 0
	   ...
	   if(ASSOC(ip2,id,imod0)) then
	      nb=NBLANK1(titlep(ip2))	!must exclude ascii 0
	...
	if(itipe.eq.1) then
	   if(ipp.le.npar) then
		...
		if(ipp.eq.8+id.or.ipp.eq.10+id.or.ipp.eq.12+id.or.ipp.eq.14+id) then
		   nb=NBLANK1(titlep(ipp))	!must exclude ascii 0
