   ...
   if(ipos.le.1) then
	...
	if(ixposv.eq.-1) then
		...
		ivalp1 = gmCreateValueEntry(ipanel1, 1, 2, 3, 1,0, 20,6, Gedit, &
              	...
              	gmBack1Col=0, gmBack2Col=0, gmTextCol=0, &
              	gmVpos=GTOP, gmExpand=GOFF)
        iradiop1 = gmCreateRadioBox(iPanel1, 0, 3, 5, 2, gmType=GFREEFORM, gmBorderType=Gnoboundary, &
			gmFillCol=123,gmLineBorder=0, gmFillBorder=0, gmTitle='', gmVpos=GTOP)

		igtog(1) = gmCreateToggleButton(iRadiop1 , 1, 0, 3, 1,'move', 0, &
              	gmType=G3Dradio,gmVpos=GTOP, gmExpand=GOFF, gmCallBAck=151)
        igtog(2) = gmCreateToggleButton(iRadiop1 , 1, 1, 3, 1,'ok', 1, &
              	gmType=G3Dradio,gmVpos=GTOP, gmExpand=GOFF, gmCallBAck=152)
		ipanel2=gmCreatePanel(iGraphPanel,0 , 6, 5,6, &
              	gmHpos=Gright, gmVpos=GTOP, gmExpand=GON, gmType=GINVERSECHISEL, &
              	gmLineCol=0, gmLineBorder=0, gmFillCol=123, gmFillBorder=0)
		itextt= gmCreateTextEntry(ipanel2, 1, 1, 3, 1,'Y shut',10, Gdisplay, &
              	gmType=GSTANDARD, gmFont=GDEFAULT, gmJustify=GLEFT, &
              	gmBack1Col=0, gmBack2Col=0, gmTextCol=0, &
              	gmVpos=gtop, gmExpand=GOFF)
		ivalp2 = gmCreateValueEntry(ipanel2, 1, 2, 3, 1,0, 20,6, Gedit, &
              	gmType=GSTANDARD, gmhpos=GLEFT, &
              	gmBack1Col=0, gmBack2Col=0, gmTextCol=0, &
              	gmVpos=GTOP, gmExpand=GOFF)
	iradiop2 = gmCreateRadioBox(iPanel2, 0, 3, 5, 2, gmType=GFREEFORM, gmBorderType=Gnoboundary, &
			gmFillCol=123,gmLineBorder=GOUTEREDGE, gmFillBorder=GOUTEREDGE, gmTitle='', gmVpos=GTOP)

		igtog(3) = gmCreateToggleButton(iRadiop2 , 1, 0, 3, 1,'move', 0, &
              	gmType=G3Dradio,gmVpos=GTOP, gmExpand=GOFF, gmCallBAck=153)
        igtog(4) = gmCreateToggleButton(iRadiop2 , 1, 1, 3, 1,'ok', 1, &
              	gmType=G3Dradio,gmVpos=GTOP, gmExpand=GOFF, gmCallBAck=154)
		ipanel3=gmCreatePanel(iGraphPanel,0 , 12, 5,7, &
              	gmHpos=Gright, gmVpos=GTOP, gmExpand=GON, gmType=GINVERSECHISEL, &
              	gmLineCol=0, gmLineBorder=0, gmFillCol=123, gmFillBorder=0)

		itextt= gmCreateTextEntry(ipanel3, 1, 1, 3, 1,'X beg',10, Gdisplay, &
              	gmType=GSTANDARD, gmFont=GDEFAULT, gmJustify=GLEFT, &
              	gmBack1Col=0, gmBack2Col=0, gmTextCol=0, &
              	gmVpos=gtop, gmExpand=GOFF)	
		ivalp3 = gmCreateValueEntry(ipanel3, 1, 2, 3, 1,0, 20,6, Gedit, &
              	gmType=GSTANDARD, gmhpos=GLEFT, &
              	gmBack1Col=0, gmBack2Col=0, gmTextCol=0, &
              	gmVpos=GTOP, gmExpand=GOFF)
		iradiop3 = gmCreateRadioBox(iPanel3, 0, 3, 5, 2, gmType=GFREEFORM, gmBorderType=Gnoboundary, &
			gmFillCol=123,gmLineBorder=GOUTEREDGE, gmFillBorder=GOUTEREDGE, gmTitle='', gmVpos=GTOP)

		igtog(5) = gmCreateToggleButton(iRadiop3 , 1, 0, 3, 1,'move', 0, &
              	gmType=G3Dradio,gmVpos=GTOP, gmExpand=GOFF, gmCallBAck=155)
		igtog(6) = gmCreateToggleButton(iRadiop3 , 1, 1, 3, 1,'zoom', 1, &
              	gmType=G3Dradio,gmVpos=GTOP, gmExpand=GOFF, gmCallBAck=156)
        igtog(7) = gmCreateToggleButton(iRadiop3 , 1, 2, 3, 1,'ok', 1, &
              	gmType=G3Dradio,gmVpos=GTOP, gmExpand=GOFF, gmCallBAck=157)
		
		ipanel4=gmCreatePanel(iGraphPanel,0 , 19, 5,7, &
              	gmHpos=Gright, gmVpos=GTOP, gmExpand=GON, gmType=GINVERSECHISEL, &
              	gmLineCol=0, gmLineBorder=0, gmFillCol=123, gmFillBorder=0)
		itextt= gmCreateTextEntry(ipanel4, 1, 1, 3, 1,'X end',10, Gdisplay, &
              	gmType=GSTANDARD, gmFont=GDEFAULT, gmJustify=GLEFT, &
              	gmBack1Col=0, gmBack2Col=0, gmTextCol=0, &
              	gmVpos=gtop, gmExpand=GOFF)
		ivalp4 = gmCreateValueEntry(ipanel4, 1, 2, 3, 1,0, 20,6, Gedit, &
              	gmType=GSTANDARD, gmhpos=GLEFT, &
              	gmBack1Col=0, gmBack2Col=0, gmTextCol=0, &
              	gmVpos=GTOP, gmExpand=GOFF)
		iradiop4 = gmCreateRadioBox(iPanel4, 0, 3, 5, 2, gmType=GFREEFORM, gmBorderType=Gnoboundary, &
			gmFillCol=123,gmLineBorder=GOUTEREDGE, gmFillBorder=GOUTEREDGE, gmTitle='', gmVpos=GTOP)

		igtog(8) = gmCreateToggleButton(iRadiop4 , 1, 0, 3, 1,'move', 0, &
              	gmType=G3Dradio,gmVpos=GTOP, gmExpand=GOFF, gmCallBAck=158)
		igtog(9) = gmCreateToggleButton(iRadiop4 , 1, 1, 3, 1,'zoom', 1, &
              	gmType=G3Dradio,gmVpos=GTOP, gmExpand=GOFF, gmCallBAck=159)
        igtog(10) = gmCreateToggleButton(iRadiop4 , 1, 2, 3, 1,'ok', 1, &
              	gmType=G3Dradio,gmVpos=GTOP, gmExpand=GOFF, gmCallBAck=160)
		...
		Icon0g = gmCreateIcon(GraphMainPanel1_1(iplot),0,0,2,1 ,'Msml1021',  &
    			GDLLICON,0,&
           gmHpos=Gright, gmVpos=GTOP,gmHFlag = GBUBBLEANDSTATUSBAR,&
			gmHelp = 'Continue',gmcallback = 192)
