c FILE NAME-was HJCLIK1.FOR but now renamed HJCLIK.FOR
	...
	real*8 function HJCLIK(iasympPanel,kfit,theta,
     ...
     & kAm,kFm,km,irt,npar,IQf,irate1,jrate1,nlig)
...
c
	use menu_f90
c	PARAMETER 60=60 	!max number of open states for following
      allocatable :: olik,nopen
c=	real*4 theta(20)
	real*8 theta(200)
c
	real*8 eigAAsav(60,10),eigFFsav(100,10)
...
c	common/qexpblk/QEXPQA,QEXPQF		!now parameter
	...
	real*8 QAF(60,100),QFA(100,60),QAA(60,60),QFF(100,100)
	real*8 EXPQA(60,60),EXPQF(100,100)
	real*8 WA(60,60),WF(100,100)
...
c	real*8 s1Asav(kAm,10),s2Asav(kAm,10),s1Fsav(kFm,10),s2Fsav(kFm,10)
	real*8 rootAsav(60,10),rootFsav(100,10)
	real*8 s1Asav(60,10),s2Asav(60,10),s1Fsav(100,10),s2Fsav(100,10)
c Array sizes
c Z00A(kA,kF,k)
c Z00F(kF,kA,k)
c Z10A(kA,kF,k)
...
c
	...
	FA=51
	FF=55
	call SUBMAT(QD,AA,QAA,km,km,60,60)		!QAA in QAA etc
	call SUBMAT(QD,AF,QAF,km,km,60,km)		!QAF
	call SUBMAT(QD,FA,QFA,km,km,km,60)		!QFA
...
cccccccccccccccccccccc
	call HJCASYMP(iasympPanel,QD,k,kA,kF,phiA,phiF,QEXPQA,QEXPQf,XAF,
     ...
     & s1A,s2A,s1F,s2F,first,kAm,kFm,irt)
	if(abort) then
...
cp	   print 6,nerr
	imess=gmdisplaymessagebox('','Error in HJCASYMP: all theta reset'
     &	,gstop,gok)
...
cp            print 79,(phibst(1,j),j=1,kA)
            if(discprt) write(7,79) (phibst(1,j),j=1,kA)
79	      format(' Initial CHS vector for burst = ',20g13.6)
	   endif
	   if(first.or.idebug.eq.8) then
cp      	print 81,(endbst(i,1),i=1,kA)
      	if(discprt) write(7,81) (endbst(i,1),i=1,kA)
...
c
	call HJCEXACT(QD,k,kA,kF,EXPQF,QEXPQF,EXPQA,QEXPQA,QAF,QFA,
     & Z00A,Z10A,Z11A,Z00F,Z10F,Z11F,eigen,kAm,kFm,60,ibad,eig1)
	if(ibad.ne.0) then
cp	   print 1501
...
cp	   call BELL(3)
cp         print 911,ng+1,nopen(ng+1)		!ng not updated yet
	imess=gmdisplaymessagebox('',
...
cp	   call BELL(5)
cp	   print 991
	imes=gmdisplaymessagebox('','NO GROUPS FOUND',gstop,gok)
...
cp		   print 807,i,nopen(i),jset,olik(i)
     ...
     &	'): Lik = ',olik(i)
		   ...
		   goto 3333
!		   imes=gmdisplaymessagebox('','Problems with group'//
 !    &		cstring(1:3)//
 !    &	   ':  lik='//cstring1,ginformation,gok)		   	
		   if(dprt) write(7,807)i,nopen(i),jset,olik(i)
807		   format(
