...
c		fix0=.true.
	   ...
	   do m=1,nsfit
		...
		if(guesdone) then
		 ...
		 do i=1,nj(j)
c150202		   if(Yobs(i,j).gt.0.1*ymax1.and.Yobs(i,j).lt.0.9*Ymax1)then
		 if (Yobs(i,j).gt.0.05*ymax1.and.Yobs(i,j).lt.0.95*Ymax1) then
...
96		continue
	   ...
	   if(ifitmode.eq.3) then
		   ...
		   do m=1,nsfit
			j=juse(m)		!set # used
			k=k+1
			theta(k)=thetgues(2,j)  !K value for each set
		   enddo
		   kmax=k
	   endif
c
