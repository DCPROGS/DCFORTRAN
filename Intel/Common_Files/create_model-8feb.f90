...
!	common/model/imod0,charmod,jlast,ilast,mtitle,imodold
	...
	common/ini_pag/ ini,ini1,ini_panel1,ini_panel1_1,ini_Text1_1
...
333			format('  conductance of state ',i3,2x,a12,'(pS) = ',g13.6)
	...
	if(mod_create.lt.1.or.(mod_create.eq.1.and.(nxmodel.eq.0.or.nymodel.eq.0))) then
		if(models(jgraph)%indmod) then
			...
			else if(models(jgraph)%n.lt.64) then
	...
	if(mod_create.ne.2) then
		...
		nxmodel,nymodel,mtype,mtitle,mod_create)
		iswicon=0
		ipbar_Progress2=0
	endif
!	dxs=dxs0
!	dys=dys0
	...
	if(mod_create.ge.1) then
		...
		do ind_m=1,models(jgraph)%n
			...
			call lincols1(models(jgraph).colour(ind_m))
	...
	if(mod_create.ge.1) then
		...
		iwcell(i)=110
		enddo
		nl1=len_trim(ligname(1))
		nl2=len_trim(ligname(2))
		if(models(jgraph)%nlig.eq.1) then
			ncols=5
			xtext='State name '//char(124)//'Conductance'//char(124)//'Nr links'//&
			char(124)//'Link to'//char(124)//models(jgraph)%ligname(1)(1:nl1)//' bound'
		else if(models(jgraph)%nlig.eq.2) then
			xtext='State name '//char(124)//'Conductance'//char(124)//'Nr links'//&
			char(124)//'Link to'//char(124)//models(jgraph)%ligname(1)(1:nl1)//' bound'//&
			char(124)//models(jgraph)%ligname(2)(1:nl2)//' bound'
			ncols=6
		else
			xtext='State name '//char(124)//'Conductance'//char(124)//'Nr links'//&
		char(124)//'Link to'
