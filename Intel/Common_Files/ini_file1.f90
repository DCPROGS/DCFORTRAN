...
text1(2)=' (2) Simulate a one or more scan.scn file'
text1(3)=' (3) Read data from old SCANDAT.DAT disc'
...
text2(2)=' (2) Change path only'
text3(3)=' (3) Use different experiments'
	xtext='Set 1 '//char(124)//' '
	nseta=2
	if(nset.gt.1) then
		nseta=nset
		xtext='Set 1'//char(124)
		do i=2,nset-1
			nb=nblank1(xtext)
			call intconv(i,cnum)
			xtext=xtext(1:nb)//'Set '//cnum//char(124)
		enddo
	    nb=nblank1(xtext)
	    call intconv(i,cnum)
		xtext=xtext(1:nb)//'Set '//cnum
	endif
   ini_TxtArray5 = gmCreateTextArray(ini_Panel5, 5, 90, 250, 140, nseta, 10, gmAxisW=30, gmAxisH=25, gmXtext=xtext, &
...
'DEFINE VALUES FOR SIMULATED DATA'
' Number of different data sets '
