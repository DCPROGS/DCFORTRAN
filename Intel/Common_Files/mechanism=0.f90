...
irect,iarca,indrec1,idestf,readmec)
...
COMMON/MPAR1/NCYC1,NSC1,IM1,JM1
	...
	common/mech_param/k,kf,nsub,kstat,ncon1,statname,icspec,icout,&
		...
		common/irs/irates,jrates,inion
	...
	!!!!! calculate nmr again
		...
		newmr=0
			...
			do i=1,ncyc
				...
				indnmrj=0
				do k=1,nsc(isetmr(i))
				  if(ir1(i).eq.im(isetmr(i),k)) then
			
					indnmr=k
				  endif
				  if(jr1(i).eq.im(isetmr(i),k)) then
				...
				if(good) then
					...
					do j=1,nsc(isetmr(i))
					   im2(isetmr(i),j) = im(isetmr(i),nsc(isetmr(i))-j+1)
					enddo
					 else
					do j=1,nsc(isetmr(i))
					   im2(isetmr(i),j) = im(isetmr(i),j)
					enddo  
					endif
					
					
					do j=1,nsc(isetmr(i))-1
					   jm2(isetmr(i),j) = im2(isetmr(i),j+1)
					enddo
					jm2(isetmr(i),nsc(isetmr(i))) = im2(isetmr(i),1)
				
				
					indnmr=0
					indnmrj=0
					do k=1,nsc(isetmr(i))
					if(ir1(i).eq.im2(isetmr(i),k)) then
			
					indnmr=k
					endif
					if(jr1(i).eq.im2(isetmr(i),k)) then
				...
				ll=2
				ijk=indnmr+1
				if(ijk.gt.nsc(isetmr(i))) indnmr=0
				do k=indnmr+1,nsc(isetmr(i))
					im(isetmr(i),ll)=im2(isetmr(i),k)
