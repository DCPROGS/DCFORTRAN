...
c bigger
	...
	allocatable::last,nod,nsc1,ind,melim,mcyc
	integer NSC(:),icyc(:),icycsav(:,:)
	integer nsi(:),ns(:,:),nck(:),nc(:,:)	!for search for cycles
	integer last(:),nod(:),nsc1(:),ind(:),melim(:),mcyc(:)	!ditto
...
c	n0=500
	...
	ALLOCATE(last(n3),nod(n3),nsc1(n2),ind(n2),melim(n2),mcyc(n2))
c
...
300	format(' State # ',i5,' has ',i5,' connections to states:',/,4i5)
34	continue
...
301	format(1x,i3,' states have ',i5,' connections,viz states:',/,10i5)
36	continue
c
...
c nc(k1,2),...,nc(k1,nck(k1)) (2 states the 1st time when k1=ks)
	do 51 nd=1,200
51	last(nd)=0	!=last exit route tried for node #nd
...
c322	   format(' Node found: nd,nod(nd),last(nd),icyc(mc) = ',4i5)
	   if(nck(k1).gt.last(nd)) then
	      do 42 m1=last(nd)+1,nck(k1)	!check all ways out not yet used
		mw=m1
		inext=nc(k1,m1)
...
c earliest node (or, if already at first node=starting state, then finish)
	   nd=nd-1
	   do 421 i2=nd+1,200
421	   last(i2)=0	!reset all exits for all later nodes usable again
...
c     & ' ICYC= ',20i5)
	   do 45 i2=2,mc-1
45	   if(icyc(i2).eq.icyc(mc)) goto 44	!repeat: return to prev node
...
400	continue		!loop for jc=2,3,4
c
c
90	continue
...
c take only the n-1 smallest cycles.
	if(ncyc.le.1) goto 91	!no elim needed
c
	do 410 jc=2,4		!outer loop for 2,3,4-con states
	if(nsi(jc).eq.0) goto 410       !no jc-con states -try next
c
	do 411 i1=1,nsi(jc)	!go through the jc-con start states,jc=2,3,4
	ks=ns(i1,jc)		!the state #
c how many cycles does each state #ks occur in
c
	njc=0		!number of cycles that the jc-con state occurs in
	do 412 n1=1,ncyc
	do 413 j1=1,nsc(n1)
413 	if(icycsav(n1,j1).eq.ks) goto 414
	goto 412	!next cycle
414	njc=njc+1
	nsc1(njc)=nsc(n1)	!keep # of states in each cycle with current jc-con state
	mcyc(njc)=n1	!cycle (row) number in ditto
412	continue
c
	if(njc.le.jc-1) goto 411	!correct # of cycles so no action needed
c
c Sort the sizes in nsc1. IND=index for sorted array.
c No need to go through the jc-1 smallest ones at all- just eliminate
c the jc,jc+1,..,njc largest ones from ICYCSAV so at end latter contains
c only the required cycles
	call SORTINDI(nsc1,ind,njc,n2)
c Remove any superfluous cycles from ICYCSAV before next cycle
c -move all cycles above that removed down one place in icycsav (and nsc)
	nelim=njc-jc+1
	do 451 i2=jc,njc
	   m=mcyc(ind(i2))
	   melim(i2-jc+1)=m	!store cycle # to be removed
c	   print 452,m,(icycsav(m,j1),j1=1,nsc1(i2))
452	   format(/' Superfluous cycle #',i3,' eliminated: ',/,20i3)
451	continue
c remove icycsav(m,*) (not necessary if m=last row=row #ncyc- just decrement
c ncyc in this case
c	print 456
456	format(' ICYCSAV before elimination')
c	do 457 i3=1,ncyc
c457	print 458,(icycsav(i3,j1),j1=1,nsc(i3))
c458	format(1x,20i3)
c
c care needed with elimination- if rows 2,4 to be elim then after row
c 2 is elim, row 4 will have become row 3, but if 4,2 elim in that
c order then there is no problem, so reorder the rows to be eliminated,
c stored in melim(), in descending order
	call SORTI(melim,nelim,.false.,n2)
	do 461 i2=1,nelim
	 m=melim(i2)
	 if(m.lt.ncyc) then
	   do 453 m1=m,ncyc-1
		nsc(m1)=nsc(m1+1)
	   do 453 j1=1,nsc(m1)
		 icycsav(m1,j1)=icycsav(m1+1,j1)
453	   continue
	endif
	 ncyc=ncyc-1
461	continue
c
c check icycsav OK after elim of superfluous cycles
c	  print 4561
c4561	  format(' ICYCSAV after elimination')
c	  do 4571 i3=1,ncyc
c4571	  print 458,(icycsav(i3,j1),j1=1,nsc(i3))
c
411	continue	!i1 loop to go through each jc-con state
410	continue	!loop for jc=2,3,4
c
91	continue
c
c Set final values, and get param to be fixed by micro rev (these must
c be the 1st two states in icyc, and in im,jm (see QSETD,QDEF1,GETCYC)
c define im,jm
...
c	  print 302,ncyc
	...
	do 52 m=1,ncyc
	do 47 j1=1,nsc(m)-1		!nsc1 already sorted
	   im(m,j1)=icycsav(m,j1)
	   jm(m,j1)=icycsav(m,j1+1)
47	continue
c last im,jm
	im(m,nsc(m))=icycsav(m,nsc(m))
	jm(m,nsc(m))=icycsav(m,1)
52	continue
c
c	print 326
...
303	continue
c put nsc() in array of same dimension (=8) as in QDEF1 for output
	do 3031 i=1,ncyc
3031	nsc0(i)=nsc(i)
c
999	continue
	...
	DEALLOCATE(last,nod,nsc1,ind,melim,mcyc)
	RETURN
	end
