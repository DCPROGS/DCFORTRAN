...
c
	...
	character ndev*2,adcf1*30,cDATEW*11,adctimew*8,title1*64
...
c
	...
	if(useconsam) then
     ...
     & 	 srate,ymin,ymax,calfac,ioff,calfac2,cDATEW,adctimew,
     &	 adcf1,base,njump
...
c If very low gain (nA) currents queued in consam, calfac2 may be >1
     ...
     &	   istart,iend,srate,calfac,calfac2,ioff,nrec1,cDATEW,adctimew
		  
      	   if(discprt) write(7,7) calfac2
7		   format(
     ...
     &	  (dt(i),i=1,ntrans-1),(stepamp(i),i=1,ntrans)
	   ...
	   if(iptype.eq.3) then		!no calc curve
	      read(11,rec=krn) idum,itit,title2,isdfst,ndisp1,ndev,
     &   istart,iend,srate,calfac,ioff,calfac2,nrec1,cDATEW,adctimew,
     &	 adcf1,base,njump
c		if(ioff.gt.5000.or.calfac2.lt.1.e-6) then
c		if(calfac2.lt.1.e-6.or.calfac2.gt.1.) then
c If very low gain (nA) currents queued in consam, calfac2 may be >1
