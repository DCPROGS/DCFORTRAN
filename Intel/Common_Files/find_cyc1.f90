...
n0=1000
	do nc(1)=1,n
		do nc(2)=1,n
			if((link(nc(1),nc(2)).eq.1).and.(nc(1).ne.nc(2))) then
			mc=mc+1
			call cyc(n,mc,nc,icycsav,nsc,im)
				do nc(mc)=1,n
					if((link(nc(mc),nc(mc-1)).eq.1).and.(k.ne.j).and.(k.ne.i)) then
						do l=1,n
...
do nc(mc)=1,n
	if((link(nc(mc),nc(mc-1)).eq.1).and.(nc(mc).ne.).and.(k.ne.i)) then
						do l=1,n
						if(link(k,l).eq.1.and.(k.ne.l).and.(l.ne.j)) then
							if(l.eq.i) then
								icyc(1)=i
