	subroutine write_plotqred(iqplot,iplotype,xval,yval,xcal,ycal,ndimd,&
...
! new arrays for horizontal lines
...
! new form for weights in VPLOT5
	real*4 weight(kwi,kwj)
	integer*4 icurvw(ndimd),jmiss(20)
! integer*2 copies to save disc space
	integer*2 IJUS1(100)
	integer*2 idraw1(100),icol1(100)
	COMMON/TPOS/IDRAW(250),ICOL(250),THICK(250),C_THICK(250),ITYPE(250),ifont(100),&
     rANGLE(100),IJUS(100),SIZEtext(100),rRXBOX(4,100),rRYBOX(4,100),&
     rrX(100),rRY(100),NARROW,NLINE,NHLINE,NVLINE, XBEG(50),YBEG(50),&
     XEND(50),YEND(50),nTEXT,NEWTEXT,CNUMX,CNUMY,CNUMZ,CEXPX,CEXPY,CEXPZ,&
...
67	continue
	   ...
	   nplot1=iplot
		...
		if(iqplot.eq.nplot+1) then
			...
			if(iptype0.ne.23.and.iptype0.ne.16) then 
				...
				iplot=nplot+1		!next number
				nplot1=iplot
				imes=gmdisplaymessagebox('','Cannot override record. New record created',ginformation,gok)
			endif
			nplot1=nplot
		endif
	endif
	...
	if(iptype.eq.16) then
	    ioffset=2048
		write(11,rec=istrec) iptype,ndv1,ndimd,ndc1,ndimc,kwi,kwj,kmax,itit,title1,&
     	xmin,xmax,ymin,ymax,xcross,ycross,xtic,ytic,xlo,xhi,ylo,yhi,&
     	ifont0,ilog,iscal,doframe,titlex,titley,ilabel,ncurvd,&
     	ncurvc,inumx,inumy,sval,ifitype,ncomp,numbx,numby,ifnt,ntext,k1,k2,k3,k4,k5,&
		 ...
		 write(11,rec=istrec) iptype,ndimd,ndv1,ndimc,ndc1,kmax,&
          ...
          jcol,mono,ioffset,(ameant(i),i=1,ncomp),(areat(i),i=1,ncomp)
	
	endif	

! Now write NEWTEXT,CHARCOP,INT2COP,INT4COP and DATCOP to disk
...
! UpDATEW rec #1 (after data written safely!)
