	subroutine CYCQ(k,ncon,ic,ncyc,nsc0,im,jm,ncmax)
...
c
c	do 410 jc=2,4		!outer loop for 2,3,4-con states
	do jc=2,10		!outer loop for 2,3,4, . . .,10 -con states
	   if(nsi(jc).eq.0) goto 410       !no jc-con states -try next
c
	   do i1=1,nsi(jc)	!go through the jc-con start states,jc=2,3,4
...
c
		...
		do n1=1,ncyc
		   do j1=1,nsc(n1)
	 		if(icycsav(n1,j1).eq.ks) goto 414
		   enddo
		   goto 412	!next cycle
414		   njc=njc+1
