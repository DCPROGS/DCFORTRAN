subroutine fitting(callid,igraph2,main,imainpanel,ivwin9,iynwin,Form1,Form1_TxtArray1,theta,jfix1,&
    tint,ampl,iprops,&
	 ...
	 iconv,ipl1,ipl2,ipl3,ipl4,ipl5,ipl6,icprev,isw,titpfree,iptitreal,EXPQF,EXPQA,&
	 qt,qd,liksurf,simwascii)

! subroutine to deal with fitting
! Modified 21-04-07 remove var, covar, cor, den, badpar from arguments -not used
! callid 4011 to 4499
	use DFLIB
!	use dfwin
	use menu_f90
...
!	real*8 var(200),covar(200,200),elmax,cor,den			!for HMAT
	...
	real*8 eigAAsav(20,10),eigFFsav(100,10)
	character*60 vtext(60),ctimews,ctimewf
	...
	integer nint(10),ival(50),itypeval(50)
	real*4 conc(10,10),tresol(10),vhold,vref		!to hold concentration ith ligand, jth set
...
common/root/rootA,rootF
	real*8 rootAsav(20,10),rootFsav(100,10)
	real*8 s1Asav(20,10),s2Asav(20,10),s1Fsav(100,10),s2Fsav(100,10)
...
!	common/rootsav/rootAsav,rootFsav
	...
	logical newfile,discprt,plot,readini,fixec50,endsim,ABORTW,penalty
	...
	real*8 ec50,pmax,ec501,xqlo,xqhi,ec5
	 real*8 ratemax,ec50out,ec50out1,ec50out2
...
character*60 :: edir='.'
	character*120 :: efilt
	character*60 :: efile
!DC add DATEW in case of fit running overnight
	character cDATEWs*11,qDATEWs*9
	character cDATEWf*11,qDATEWf*9
...
1420	   format(/,' EQUILIBRIUM CALCULATIONS FOR INITIAL GUESSES')
	       call PRNTEC50(QT,conc,k,nlig,nset,npar,ncdep,ndisp0,itogrec)
		 idebug=0
...
1351	format(/,' Fit ',i3,' parameters: initial theta() = ',20(/,5g13.6))
		...
		if(nerr.ne.0) then
			imes=gmdisplaymessagebox('','Error. Likelihood set to 0; try new params',gquestion,&
			gyesno)
	...
	!		ival=gmCreateValueEntry(iasymppanel,7, irt, 5, 1, nscal(j), 6, 0, gdisplay,gmVpos=GTOP)
			...
			call dbletoch(fcomp(j),cnum(2),11)
...
!	if(imes1.eq.gyesbutton) 	logsav=.true.
	...
	call val_table(Main,ivwin9,n,vtext,ival,val,icall,itypeval,iplot_Toggle0)
	do i=1,10
		itypeval(i)=0
	enddo
...
111 format(/,' Initial step size factor=',f15.9,/,' Simplex contraction factor (0-1)=',f10.6,/,&
	...
	first=.true.
	idebug=0
!	if(.not.silent) call gmsettextsetting(itty,' F1 key to ABORTW')
	call DATE(qDATEWs)	!DATEW when fit started
    cDATEWs=qDATEWs(1:7)//'20'//qDATEWs(8:9)
	call TIME(ctimews)
    call gmsettextsetting(itty,'Simplex started at:'//ctimews)
	if(discprt) write(7,131) ctimews
...
case(4300:4320)
	...
	call SIMPHJC(main,kfit,THETA,stpfac,errfac,nev,nevm,&
	  ...
	  irt,npar,IQf,irate1,jrate1,nlig,dprt,titpfree,iptit,EXPQF,EXPQA,qt,qd)
	!call gmflushcallbackqueue()
	if(logfit) then
	  do i=1,kfit
...
!duration of fit
	call DATE(qDATEWf)	!DATEW when fit ended
    cDATEWf=qDATEWf(1:7)//'20'//qDATEWf(8:9)
	 call TIME(ctimewf)
	 call gmsettextsetting(itty,' Fitting finished at:'//ctimewf)
! work out how long the fit took. NB CHTOINT expects 11 character input
	 call CHTOINT(ctimews(7:8)//'         ',isecs)
	 call CHTOINT(ctimews(4:5)//'         ',imins)
	 call CHTOINT(ctimews(1:2)//'         ',ihrs)
	 call CHTOINT(qDATEWs(1:2)//'         ',idays)
	 call CHTOINT(ctimewf(7:8)//'         ',isecf)
	 call CHTOINT(ctimewf(4:5)//'         ',iminf)
	 call CHTOINT(ctimewf(1:2)//'         ',ihrf)
	 call CHTOINT(qDATEWf(1:2)//'         ',idayf)
	 if(idayf.lt.idays) then	!fit straddles end of month!
		if(qDATEWs(4:6).eq.'FEB') then
			idayf=idayf+28		!not allowing for leap years
		else if(qDATEWs(4:6).eq.'APR'.or.qDATEWs(4:6).eq.'JUN'.or. &
		     qDATEWs(4:6).eq.'SEP'.or.qDATEWs(4:6).eq.'NOV') then
			idayf=idayf+30
		else
			idayf=idayf+31
		endif
	 endif
	 isecs=isecs+60*imins+3600*ihrs+(24*3600)*idays
	 isecf=isecf+60*iminf+3600*ihrf+(24*3600)*idayf
	 idur=isecf-isecs		!duration if fit in seconds
	 isec=mod(idur,60)
	 imin=mod(idur-isec,3600)/60
	 ihr=mod(idur-isec-60*imin,24*3600)/3600
	 iday=(idur-isec-60*imin-3600*ihr)/(24*3600)
	 WRITE(STRING,fmt='(a18,i2,a7,i3,a7,i3,a6,i3,a4)') &
 ' Duration of fit: ',iday,' days  ',ihr,' hours ',imin, ' min  ',isec,' sec'
	 call gmsettextsetting(itty,string)
	 if(discprt) write(7,132) ctimewf,' Duration of fit: ',iday,' days  ',ihr,&
	 ' hours ',imin, ' min  ',isec,' sec'
132 format(/,'  Simplex finished at:',a10,/,&
	a18,i2,a7,i3,a7,i3,a6,i3,a4)
...
23	format(&
	...
	if(istab.eq.0) then
   	    call gmsettextsetting(itty,'(Fit NOT ''stable'' at time of convergence)')	
	    if(discprt) write(7,231)
231		format(' (Fit NOT ''stable'' at time of convergence)')
	else if(istab.eq.1) then
   	    call gmsettextsetting(itty,'(Fit ''stable'' at time of convergence)')	  
	    if(discprt) write(7,232)
232		format(' (Fit ''stable'' at time of convergence)')
	else if(istab.eq.2) then
   	    call gmsettextsetting(itty,'(Fit ''very stable'' at time of convergence)')	
...
case(4400) ! prepare plot
    jset=1
	call QSET_HJC(-1,jset,theta,QT,QD,kfit,k,irt,npar,IQf,irate1,jrate1,nlig,itogrec )	!QT,QD no longer in common/qblk
...
138	format(' ------------------------------------------------------------')
	if(ncdep.gt.0) then
	 do ifix=1,nfixec50
	  ...
	  nlv=nlv0
         if(discprt) write(7,142) nlv0,ligname(nlv0)
...
143			format('  (conc of ligand #',i2,',  ',a10,' = ',g13.6,' micromolar)')
	  ...
	  if(itogrec.eq.5) then
	   ...
	   if(dcmodel.and.nlig.eq.1) then
	        ...
	        iset=1
		    call EC50_HJC(EC50out,curinfs,curmax,concmax,cur0,pop0,pmax,nlv0,QT,iset,conc,k,kdim,nerr)
	   endif
	   nlv=nlv0
	   if(nerr.ne.0) then
...
case(4023)
	...
	if(autosim) then
	   ...
	   if(ABORTW) then
	    endsim=.true.
		imesg=gmdisplaymessagebox('',' Save the last (ABORTWed) fit ?',gquestion,gyesno)
	...
	!	iz1sav=izr1
	   ...
	   !  if(.not.simwascii) then
        ...
        cDATEW,ctimew,apfile,tress1,mtitle1,nsim,&
        ix1sav,iy1sav,iz1sav,nint,nev,elmax,theta0,&
        thetval,nintval,ixval,iyval,izval,elmval,elmset,ABORTW,&
        imods,npars,mtitles,nevals,ec50out,penfunc,ec50val,nsims0,qfilem,nofit)
    !  endif
513	    nsims=nsims+1
	    if(nsims.le.nsim+nsims0) then
	    if(endsim) then
		idestf=0
		do j=1,nset
!		   tresol(j)=1.e3*treso	!keep res in mus for .ini
		...
		call RANDSKv1(ixr1,iyr1,izr1,iwrite,repeat,main)
	    else
		ik=0
		do m=1,npar
		 if(jfix(m).eq.0.and.jcon(m).eq.0.and.jmic(m).eq.0) then
...
!		elmax=-smin                                                            
	...
	nrr=5
	  ...
	  call text_array(igraph2,Main,Form1,Form1_TxtArray1,nrr,npar,iwcell,ihcell,xtext,isw,&
			...
			thsav,titlem,ratcons(indk)%titlep,NBOUND,ligname,nlig,&
			ratcons(indk)%micro,ratcons(indk)%ligant,jfix,theta0,iformText)
	if(autosim) then
	imes=gmdisplaymessagebox('','Analyse first the simulated fits?',gquestion,gyesno)
	if(imes.eq.gyesbutton) then
		idestf=10
		goto 99
	endif
...
!		call gmDefineKeyselectCallback(13,4401)
	endif
!	   callid=4401
!	   goto 2	
case(4401,4403)
