...
!	common/np1/npar
	...
	real*8 qval,ec50out,x1s,x2s,fx1,fx2
...
common/switch/iswicon,igraphText(100),ibutton1
	km=100
    dxs=0.6
	dys=0.8
	imod=imodel
...
case(1301:1325)	!state properties	2101:2600
	...
	case(1330)
		...
		n1=nblank1(ligname(1))
		n2=nblank1(ligname(2))	
		xtext='State name '//char(124)//'Conductance'//char(124)//'Nr links'//&
		char(124)//'Link to'//char(124)//ligname(1)(1:n1)//' bound'
			else if(models(igraph2)%nlig.eq.2) then
		xtext='State name '//char(124)//'Conductance'//char(124)//'Nr links'//&
		char(124)//'Link to'//char(124)//ligname(1)(1:n1)//' bound'//&
		char(124)//ligname(2)(1:n2)//' bound'
		ncols=6
		else
		ncols=4
		xtext='State name '//char(124)//'Conductance'//char(124)//'Nr links'//&
...
111			continue
 ...
 case(1601:1625) ! modify old model
	...
	itypeval(1)=0
    jgraph2=igraph2
	call value_table(Main,ivvwin,n,vtext,ival,val,1626,itypeval)
	do i=1,10
		itypeval(i)=0
	enddo		
	case(1626)
	...
	if(models(jgraph2)%nlig.gt.0) then
				...
				enddo
				call text_table(Main,iqwin,'Name for ligands',models(jgraph2)%nlig,textiq1,iqt1,textiqt1,1627)
		...
		newmr=0
			...
			do i=1,ncyc
				...
				do k=1,nsc(i)-1
					...
					jm(isetmr(i),k)=im(isetmr(i),k+1)
				enddo	 
				jm(isetmr(i),nsc(i))=im(isetmr(i),1)
...
445			continue	
		...
		do l=1,ncyc
		do m=1,nsc(l)-1
		ratcons(indk)%icyc(l,m)=im(l,m)
		ratcons(indk)%icyc(l,m+1)=jm(l,m)
		enddo
		ratcons(indk)%icyc(l,nsc(l))=im(l,nsc(l))
		ratcons(indk)%icyc(l,1)=jm(l,nsc(l))
		enddo
...
!		imess=gmdisplaymessagebox('','Now start fitting (if you already opened the file)',ginformation,gok)
	...
	case(2051,2052)
		...
		call gmRemoveWindow(Form1(igraph2,1))
		ibkta=0
		if(mod_create.ne.1) call gmRemoveWindow(iphelp)
		callid=1800+igraph2
		goto 2
    case(2053)
		if(autosim) then
