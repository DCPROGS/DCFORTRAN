subroutine write400(imodel,models,indrat,ratcons,irecm0,qfilem,pfilem)
use menu_f90
REAL*8 dgamma(100)
	...
	real*8 dgammas(100)
	character*60 :: qdir='.'
	character*60 :: qfilt='*.dat'
	integer::Button7(4),RECORDS
...
if(altmod) then
	mtitle=models(imodel)%title_model
	do ir=1,nrecs
...
!imod=imodel
	...
	chardef=models(imodel)%chardef
	mtitle=models(imodel)%title_model
	rtitle=ratcons(indk)%title
	vref=models(imodel)%vref
	nchan=models(imodel)%nchan
	
	j=1
do k=1,models(imodel)%n
	dgamma(k)=1.0d-12*(models(imodel)%dgamma(k))
...
if(indmod) then
	do j=1,kcon
		statname(j)=models(imodel)%con_states(j)
	enddo
	do i=1,nsub
	...
	i=1
	do j=1,kstat-kcon
	statname(kcon+j)=models(imodel)%sub_states(i,j)
	enddo
	endif
endif
...
do i=1,nlig
			...
			do j=1,models(imodel)%n
				nbound(j,i)=models(imodel)%nbound(j,i)
...
17		continue	
	...
	if(iver1.eq.200) then
		...
		if(imodel.gt.300) then
			...
			ilast=models(imodel)%ilast
			jlast=models(imodel)%jlast
		endif
		do in=1,ilast
		do jn=1,jlast
			charmod(in,jn)=models(imodel)%charmod(in,jn)
		enddo
		enddo
		...
		((ic(i,j),i=1,2),j=1,ncon),(models(imodel)%name(i),i=1,k),&
		(models(imodel)%colour(i),i=1,k),(models(imodel)%x(i),i=1,k),&
		(models(imodel)%y(i),i=1,k),models(imodel)%nwidth,models(imodel)%nheight
