...
j2=nset
nset1=nset
if(inipage.eq.-100.aND.NS.NE.0) then
	j1=ns
	j2=ns
	nset1=1
endif
...
ihex=400
iwid1=iwid
if(iwid.gt.1200) iwid1=1200
if(inipage.eq.-100) then
	ihe1=150	!.aND.NS.NE.0
    ihe=450
	ihex=500
endif
! Set up master window group
   igroup = gmCreateComplexDialogueBox(Main,120, 100, iwid1, ihex, Gall, 'Groups', gmIconFormat=GDLLICON, &
      gmIconFile='Gee',gmvpos=gtop)


! Create panel group_Panel1 child of group_MainPanel
   group_Panel1 = gmCreatePanel(igroup, 0, 0,iwid, ihe, gmTitle='DEFINE GROUPS OF OPENINGS', gmType=GINVERSECHISEL, &
...
! Create text widget group_Static6 child of group_Panel1
   group_Static6 = gmCreateTextEntry(group_Panel1, 10, 220, 200, 20,'Set long open times ''bad'' (ms) ', 255, GDISPLAY, &
      gmBack1Col=0, gmBack2Col=12, gmTextCol=16 ,gmScrollMode=GVERTICAL, gmVpos=GTOP)
if(inipage.eq.-100) then !.aND.NS.NE.0
	group_Static8 = gmCreateTextEntry(group_Panel1, 10, 255, 200, 20,'Resolution (microsec) ', 255, GDISPLAY, &
      gmBack1Col=0, gmBack2Col=12, gmTextCol=16 ,gmScrollMode=GVERTICAL, gmVpos=GTOP)
! Create text widget group_Static5 child of group_Panel1
	group_Static7 = gmCreateTextEntry(group_Panel1, 10, 290, 200, 25,'Concentration (micromolar) ', 255, GDISPLAY, &
      gmBack1Col=0, gmBack2Col=12, gmTextCol=16 ,gmScrollMode=GVERTICAL, gmVpos=GTOP)
...
! Create toggle button group_Toggle4 child of group_Panel2
 ...
 !  vgroup1(j)=3.1536e10	
   ...
   group_Editval1(j) = gmCreateValueEntry(group_Panel3(j), 10, 10, 110, 25, vgroup1(j), 10, 1, GEDIT, &
    gmType=Gdecimal, gmBack1Col=0, gmBack2Col=0, gmTextCol=0, gmVpos=GTOP)	!tcrit(j)
...
! Create toggle button group_Toggle19 child of group_Panel3
   ...
   group_Editval1(j) = gmCreateValueEntry(group_Panel3(j), 10, 10, 110, 25, vgroup1(j), 10, 1, GEDIT, &
   gmType=Gdecimal, gmBack1Col=0, gmBack2Col=0, gmTextCol=0, gmVpos=GTOP)	!tcrit(j)
call gmsetvaluesetting(group_Editval1(j),vgroup1(j))
   ...
   if(chsvec(j)) then
	...
	group_Toggle4(j) = gmCreateToggleButton(group_Panel3(j), 40, 40, 40, 25, '', 0, &
    gmType=G3DCHECKBOX, gmVpos=GTOP,gmcallback=3140+j)
	call gmSettoggleswitch(group_toggle4(j), Goff)
!	call gmSetWidgetStatus(group_toggle4(j), Gselectable)
  endif
! Create toggle button group_Toggle19 child of group_Panel3
  if(badend(j)) then
   ...
   gmType=G3DCHECKBOX, gmVpos=GTOP,gmcallback=3150+j)
  ! call gmSetWidgetStatus(group_toggle5(j), Gselectable)
  call gmSettoggleswitch(group_toggle5(j), Goff)
  endif
endif
...
! Create value entry group_Editval13 child of group_Panel3
   group_Editval2(j) = gmCreateValueEntry(group_Panel2(k), 40, 170, 100, 25, vgroup2(j), 12, 0, GEDIT, &
       gmType=Gdecimal, gmBack1Col=0, gmBack2Col=0, gmTextCol=0, gmVpos=GTOP) !tcbad(1,j) (shut)
!	
call gmsetvaluesetting(group_Editval2(j),vgroup2(j))
...
! Create value entry group_Editval14 child of group_Panel3
   group_Editval3(j) = gmCreateValueEntry(group_Panel2(k), 40, 200, 100, 25, vgroup3(j), 12, 0, GEDIT, &
       gmType=Gdecimal, gmBack1Col=0, gmBack2Col=0, gmTextCol=0, gmVpos=GTOP)	!tcbad(2,j) (open)
!	
call gmsetvaluesetting(group_Editval3(j),vgroup3(j))
...
if(setbad(1,j)) then
	call gmSettoggleswitch(group_toggle2(j),gon)
else
call gmSettoggleswitch(group_toggle2(j),goff)
endif
if(setbad(2,j)) then
call gmSettoggleswitch(group_toggle3(j),gon)
else
call gmSettoggleswitch(group_toggle3(j),goff)
endif
if(inipage.eq.-100) then	!.aND.NS.NE.0

 group_Editval4(j) = gmCreateValueEntry(group_Panel2(k), 40, 270, 100, 25, vgroup4(j), 12, 4, GEDIT, &
       gmType=Gdecimal, gmBack1Col=0, gmBack2Col=0, gmTextCol=0, gmVpos=GTOP)	!conc(1,j)
call gmsetvaluesetting(group_Editval4(j),vgroup4(j))
 group_Editval5(j) = gmCreateValueEntry(group_Panel2(k), 40, 240, 100, 25, vgroup5(j), 12, 4, GEDIT,  &
       gmType=Gdecimal, gmBack1Col=0, gmBack2Col=0, gmTextCol=0, gmVpos=GTOP)	!tresol(j)
if(ns.ne.0) then
igroup_text = gmCreateTextEntry(group_Panel2(k), 10, 300, 130, 25,ligname(1), 255, Gedit, &
      gmBack1Col=0, gmBack2Col=12, gmTextCol=16 ,gmScrollMode=GVERTICAL, gmVpos=GTOP)
if(nlig.eq.2) then
group_Editval4_1(j) = gmCreateValueEntry(group_Panel2(k), 40, 330, 100, 25, vgroup4_1(j), 12, 4, GEDIT, &
       gmType=Gdecimal, gmBack1Col=0, gmBack2Col=0, gmTextCol=0, gmVpos=GTOP)	!conc(2,j)
call gmsetvaluesetting(group_Editval4_1(j),vgroup4_1(j))
...
! Create button group_Button1 child of group_Panel1
   group_Button1 = gmCreatePushButton(group_Panel1, 180, 25, 75, 25, 'Help', gmVpos=Gbottom,&
   gmhpos=gleft,gmcallback=3162)
! Create button group_Button2 child of group_Panel1
   
   group_Button2 = gmCreatePushButton(group_Panel1, 95, 25, 75, 25, 'Cancel', &
   gmhpos=gleft,gmVpos=Gbottom,gmcallback=3163)

! Create button group_Button2 child of group_Panel1
   group_Button2 = gmCreatePushButton(group_Panel1, 10, 25, 75, 25, 'All done', &
   gmhpos=gleft,gmVpos=Gbottom,gmcallback=3161)
