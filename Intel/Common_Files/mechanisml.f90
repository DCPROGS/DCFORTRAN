subroutine mechanism(callid,igraph2,main,models,imodel,imodold,&
ratcons,indrat,irecq,readini,&
pfilem,qmec,efile,text7,records,nrecs,ijmod,nmr,cxtrue,cytrue,readrec,&
thetaf,jfix1,imove,neqold,icprev,conc,iformtext,ioptm,ksim,irqsav,initmec,&
irect,iarca,indrec1,idestf)
...
!	include '\fortwin\hjcfit\hjcfit_definitions.f90'
	...
	integer itext_box(100,10),ititle(10),jopen2(100),nrmodel(100),istat(200),modopen(200)
...
COMMON/MPAR1/NCYC1,NSC1,IM1,JM1
	...
	common/mech_coord/ixm,iym,newmodel,oldmodel,nxmodel,nymodel,mtype,mod_create,&
	       ngraph2,modplot2,jgraph,ipos,lframe,isw,dxs,dys,jmodel,igraph0,newmr,newcons,irecfin(500)
	...
	common/mech_param/k,kf,nsub,kstat,ncon1,statname,icspec,icout,&
		...
		common/irs/irates,jrates
common/ecg/ival1a,ival2a,ival3a
common/ini_pag/ ini,ini1,ini_panel1,ini_panel1_1,ini_Text1_1,ini_Value2_1,ini_TxtArray2,&
...
inipage,isetu,ini_Button8_1,ini_Button8_2,ini_Button8_3,ini_Value6_0,&
ini_Button16_3,ini_Button16_4,ini_text6_01,ini_panel2,iniyes,inino,page_open 
	common/text_attr/bold,italik,underline,ipow,idev,nolabx,ifontrue
common/switch/iswicon,igraphText(100)
	km=100
    dxs=0.6
	dys=0.8
	imod=imodel
...
335			jopen2(igraph2)=1
	    ...
	    case(1101)
			...
			if(imessy.eq.gon) then
				...
				nlm=5
				call text_table(Main,iqwin,'Independent Model',nlm,textiq1,iqt1,textiqt1,1110) !59)
			else
		...
		!		textiq1(4)='Short-lived shut states'
				...
				nlm=4
					call text_table(Main,iqwin,'Model definition',nlm,textiq1,iqt1,textiqt1,1110) !38)
			endif
	...
	case(1110) !38) !new model
		...
		!	call chtoreal(cmodel,vref)
			...
			if(nlig.gt.0) then
				...
				enddo
				call text_table(Main,iqwin,'Name for ligands',nlig,textiq1,iqt1,textiqt1,1111) !39)
			else
	...
	case(1120) !59) ! inmod new model
			...
			do i=1,kcon
					...
					icons(i) = gmCreateTextEntry(iconPanel1,1, i, 6, 1,models(igraph2)%con_states(i), 60, Gedit, &
              		gmType=GSTANDARD, gmFont=GDEFAULT, gmJustify=GLEFT, &
	...
	case(1121 ) ! new model!40
	    if (kcon.gt.0) then
		do i=1,kcon
			call gmEnqTextSetting(icons(i),models(igraph2)%con_states(i))
...
339						call create_model(ixm,iym,igraph2,imodel,main,form1,form1_txtarray1,&
					...
					imod0=imodel
						...
						call copy_model(models,imodel,newmodel,igraph2-1,igraph2)
						mod_create=0
...
case(1301:1325)	!state properties	2101:2600
	...
	case(1330)
		...
		char(124)//'Link to'//char(124)//models(igraph2)%ligname(1)
			else if(models(igraph2)%nlig.eq.2) then
		xtext='State name '//char(124)//'Conductance'//char(124)//'Nr links'//&
		char(124)//'Link to'//char(124)//models(igraph2)%ligname(1)//&
		char(124)//models(igraph2)%ligname(2)
		ncols=6
		else
		ncols=4
		xtext='State name '//char(124)//'Conductance'//char(124)//'Nr links'//&
	...
	titlerp='Titles for stored rate constants'
		!	call toggle_panel(Main,ITOGGLE,itogglepanel,lt,text_tog,intoggle,1500,valdat,idat,itogbutton)
		call radio_panel(main,iradio,lt,iradio_toggle,radio_text,-indratprev,&
		iradiox,radiox,iradioy,radioy,1500,titlerp)
		else
...
!	ratcons(indk)%imod=joldmodel
	...
	models(igraph2)%model=newmodel
	nrmodel(igraph2)=newmodel
	call copy_model(models,joldmodel,newmodel,joldgraph,igraph2)
	models(igraph2)%title_model=' '
	n=1
	val(1)=models(igraph2)%nlig
	...
	call value_table(Main,ivwin,n,vtext,ival,val,1626,itypeval)
	do i=1,10
		itypeval(i)=0
	enddo		
	case(1626)
	val(1)=gmenqvaluesetting(ival(1))
	models(igraph2)%nlig=val(1)
	...
	if(models(igraph2)%nlig.gt.0) then
				...
				do i=1,models(igraph2)%nlig
					...
					textiq1(i)='Ligand'//cnum1(1:2)
				enddo
				call text_table(Main,iqwin,'Name for ligands',models(igraph2)%nlig,textiq1,iqt1,textiqt1,1627)
	...
	call text_table(Main,iqwin,' ',1,textiq1,iqt1,textiqt1,1628)
   CASE(1628)
	...
	!	call gmEnqCellSetting(Form1_TxtArray1(imod,2), 5,j,ratcons(indrat)%iconc(j),actext)
        ...
        if(icalprev.eq.2051.and.readini) then
			...
			if(imessy.eq.gyesbutton) then
				...
				ratcons(indk)%icyc(l,m)=im2(1,m)
				ratcons(indk)%icyc(l,m+1)=jm2(1,m)
				enddo
							
				ratcons(indk)%icyc(l,nsc2(l))=im2(l,nsc2(l))
				ratcons(indk)%icyc(l,1)	=jm2(l,nsc2(l))
				enddo
...
1211    continue
		if(.not.readini.or.newcons) imessy2=gmDisplayMessageBox('','Define MR now?',Gquestion,gyesno)
		if(imessy2.eq.gyesbutton) then
			...
			do m=1,nsc(l)-1
				...
				ratcons(indk)%icyc(l,m)=im(1,m)
				ratcons(indk)%icyc(l,m+1)=jm(1,m)
			enddo
							
						ratcons(indk)%icyc(l,nsc(l))=im(l,nsc(l))
						ratcons(indk)%icyc(l,1)	=jm(l,nsc(l))
			enddo
			if(ncyc.gt.0) then
			    inipage=-9
				text_tog(1)='Spanning Tree Method'
			    text_tog(2)='4 Cycles Method'
	            call toggle_panel(Main,ITOGGLE,itogglepanel,2,text_tog,intoggle,1882,valdat,idat,itogbutton)
		...
		if(imessy.eq.gyesbutton) then
			   ...
			   '*.*'//char(124)//'All Files'
				...
				nb=nblank1(edir)
					...
					call write_model(main,imod,models,indrat,ratcons,irecm0,efile,pfilem,igraph2,irec,theta0)
					irecq=irecm0
...
if(imodel.gt.0) then
	...
	call text_table(Main,iqwin,'',1,'name:',iqt1,textiqt1,1951)
	endif
case(1960) ! change rate title6504
if(indrat.gt.0) then
		textiqt1(1)=' '
	textiqt1(1)=ratcons(indrat)%title
	call text_table(Main,iqwin,'',1,'rate:',iqt1,textiqt1,1961)
...
!================================
	 ...
	 CASE(2010) !(1001) !MR -stree
		if(igraph2.gt.0) then
		if(models(igraph2)%indmod) then
			call gmdisplaymessagebox('','Independent model.No MR at this stage',ginformation,gok)
			goto 1
	   	endif
		...
		nmr=models(igraph2)%ncon-models(igraph2)%n+1
			...
			if(useini) then
			    ...
			    if(nsc2(i).gt.1.and.nsc2(i).lt.models(igraph2)%n) then
					...
					do j=1,nsc(i)
						im(i,j)=ratcons(indrat)%icyc(i,j)
						jm(i,j)=ratcons(indrat)%icyc(i,j+1)
					enddo
					jm(i,nsc(i))=im(i,1)
					enddo
					endif
...
3636				do i=1,5
					...
					iwcell,ncombo,comboname,ncomboname,icombo,2012,2,valcell1)
...
161				format(' The excluded routes contain cycle(s): invalid, try again')
			...
			call CYCQ1(k,nc,icspec,ncyc2,nsc0,im,jm,ncmax)
			if(ncyc2.ne.0) then
				write(7,16) ncyc
				imes=gmdisplaymessagebox('','The constraints contain cycle(s)',gstop,gok)
...
110			format(' ',i3,': ',i3,' - ',i3)
	   ...
	   if(ncyc.gt.0.and.useini) then
			...
			if (imes.eq.gyesbutton) then
				...
				enddo
				enddo
					call gmdisplaymessagebox('','Constraints/ Fix Paramters',ginformation,gok)
...
129		format(' Cycle ',i3,' NOT set by micro rev',/,20(5(i3,'-',i3,3x)),/)
	...
	case (2020) !(3521,3522) ! 4 cycles method
		   ...
		   if(models(igraph2)%indmod) then
			...
			call GETREV(ncyc,nsc,im,jm,ncyc1,nsc1,im1,jm1,useini,nccub)
			!im working array
			!im1 -new order
			!im2= ini
			n=1
...
444					continue
			...
			!	call table_box(main,ratcons(indrat)%ncyc,m,title_box,text_box,itext_box,3511)
				call cyc_array(imod,main,itwin1,ncyc1,5,text_box,&
				itext_box,2022,icyc_form,icmr)
		...
		newmr=0
			...
			do i=1,ncyc
				...
				do j=2,5	
					call gmEnqCellSetting(icyc_Form(imod), j,i,rval,text_box(i,j))
				enddo
				if(text_box(i,4).eq.'no') goto 445
				call chtoint(text_box(i,2),nsc(isetmr(i)))
				cnum0=text_box(i,3)
				nlc=nblank1(cnum0)
				k=1
				l=1
				...
				jr1(i)=num0(1)
				do k=1,nsc1(i)
				  if(ir1(i).eq.im1(i,k)) good=.true.
				enddo
				if(good) then
					good=.false.
					do k=1,nsc1(i)
				    if(jr1(i).eq.jm1(i,k)) good=.true.
					enddo
				endif
				if(good) then
				! another check here !!!!!!
...
445			continue	
		...
		nmr=newmr
			...
			if(isetu.eq.1) call gmsetwidgetstatus(intoggle(4),gchecked)
		!	call gmdisplaymessagebox('','Constraints/ Fix Parameters',ginformation,gok)
		if(inipage.eq.-100) then
			callid=-23
			goto 1
		else
...
409					continue				
		...
		iwcell,ncombo,comboname,ncomboname,icombo,callid+1,1,valcell1)	
		else
...
271				continue
			...
			call setec50(main,iec50,nfixec50,nlig,ligname,ival,val,comboname,npar,2036,iecctype)
    ...
    case (2031,2036,2037) !(734)
		...
		continue
			...
			do i=1,	ncomboname(1)	
			    ...
			    enddo
2222			l=i
				nl1=nblank1(comboname(1,l))
		        cnum0=comboname(1,l)(3:nl1)
		        call chatonos(cnum0,num0,nk)
		        i50=num0(1)
		        j50=num0(2)
			if(ec501.gt.0.and.xqlo.gt.0.and.xqhi.gt.0.and.i50.gt.0.and.j50.gt.0) then
				fixec50=.true.
				nfixec50=1
...
2223			l=i
	...
	if(inipage.eq.-100) then
		...
		callid=1881
		goto 2
	!call gmdisplaymessagebox('','Calculate MR/ Fix Paramters',ginformation,gok)
	endif	
	...
	case(2040) !(703) ! fix parameters0
		...
		!	nfix=0
		if(neqold.gt.0.and.newcons.eq..false.) then
			!	imess2=gmdisplaymessagebox('','Keep old constraints?',gquestion,gyesno)
		!		if(imess2.eq.gnobutton) then
				!	neq=0
					do i=1,100
					!	jfix(i)=0
					!	jcon(i)=0
					!	jmic(i)=0
					enddo
		!			imess1=gmdisplaymessagebox('','Define constraints?',gquestion,gyesno)
		!			if(imess1.eq.gyesbutton) then
	...
	!	call gmsetwidgetstatus(intoggle(4),gchecked)
	!	call gmsetwidgetstatus(intoggle(5),gchecked)
...
!		imess=gmdisplaymessagebox('','Now start fitting (if you already opened the file)',ginformation,gok)
    ...
    case(2053)
	   ...
	   ! ks=kAs+kBs+kCs+kDs
	   kcs=ks-kas-kbs
	   	call ini_page(main,nset,npar,ixgrid,iygrid,nfileb,curvonly,autosim,ksim,readini,initmec)
		...
		!	nxmodel,nymodel,mtype,'',mod_create)
