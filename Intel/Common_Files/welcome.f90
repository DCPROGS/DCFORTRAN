...
! Set up complex dialogue box initialw child of imecform1
 initialw = gmCreateComplexDialogueBox(Main,100,150,370,490, GALL, 'Initial Settings ', &
              	gmhpos=gleft,gmvpos=gtop,gmIconFormat=GDLLICON,gmIconFile='MBIG1052')

   initialw_MainPanel=gmCreatePanel(initialw, 0, 0, 370, 490, gmHpos=GCENTRE, gmVpos=GMIDDLE, gmExpand=GON, gmType=GNOBOUNDARY, &
      gmLineBorder=GOUTEREDGE, gmFillBorder=GOUTEREDGE)

! Create panel initialw_Panel1 child of initialw_MainPanel
   initialw_Panel1 = gmCreatePanel(initialw_MainPanel, 0, 0, 370, 490, gmType=GINVERSECHISEL, gmLineBorder=GOUTEREDGE, &
...
! Create panel initialw_Panel2 child of initialw_Panel1
   initialw_Panel2 = gmCreatePanel(initialw_Panel1, 30, 100, 310, 140, gmType=GINVERSECHISEL, gmLineBorder=GOUTEREDGE, gmFillCol=152, &
...
! Create button initialw_Button4 child of initialw_Panel2
  ...
  initialw_Panel21 = gmCreatePanel(initialw_Panel1, 30, 240, 310, 70, gmType=GINVERSECHISEL, gmLineBorder=GOUTEREDGE, gmFillCol=152, &
      gmFillBorder=GOUTEREDGE, gmScrollMode=GNOBARS, gmVpos=GTOP)
   initialw_Button6 = gmCreatePushButton(initialw_Panel21, 60, 8, 190, 25, 'Run DEMO (1 set)', gmType=GUSERDEFINED, gmOffcol=222, gmTextCol=0,gmVpos=GTOP,&
   gmcallback=-30) !-60)
   call gmSetWidgetStatus(initialw_button6, GSELECTABLE)
   initialw_Button7 = gmCreatePushButton(initialw_Panel21, 60, 37, 190, 25, 'Run DEMO (4 sets)', gmType=GUSERDEFINED, gmOffcol=222, gmTextCol=0,gmVpos=GTOP,&
   gmcallback=-28) !-60)
   call gmSetWidgetStatus(initialw_button6, GSELECTABLE)
 
 
 initwin_Panel3 = gmCreatePanel(initialw_Panel1, 30,310, 310, 130, gmTitle='Setup for this computer', gmType=GPROJECTED, &
...
! iniTextq = gmCreateTextEntry(initwin_Panel3, 20, 90, 280, 25,'Drives : '//drives, 255, GDIsplay, gmBack1Col=0, gmBack2Col=0, &
 ...
 ! Create button initialw_Button1 child of initialw_Panel1
   initialw_Button1 = gmCreatePushButton(initialw_Panel1, 30, 450, 75, 25, 'Help', &
...
! Create button initialw_Button2 child of initialw_Panel1
   initialw_Button2 = gmCreatePushButton(initialw_Panel1, 265, 450, 75, 25, 'Continue', &
