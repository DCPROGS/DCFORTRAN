...
! Create text array Form1_TxtArray1 child of Form1_Panel1
   ...
   enddo
   ictype2 = gmCreateComboBox(icyc_Form(imodel),0,0,iwcell(3),96,Gnone, 1, &
          gmSort=GunSORTED, gmVpos=GTOP, gmExpand=GOFF)
	do i=1,ncomboname(1)
		call gmSetListEntry(ictype2, GADD, gmString=comboname(1,i))
	enddo
	...
	do i=1,ncols
		...
		do j=1,nrows
			...
			if(i.eq.4) then
				...
				call gmSetCellAttribs(icyc_form(imodel), i, j, arrayattribs)
				call gmSetListEntry(ictype1,GSELECT,gmString=text_box(j,i+1))
			else if(i.eq.5) then
				call gmSetCellSetting(icyc_form(imodel), i,j ,gmString=text_box(j,i))
				arrayattribs%flag = ictype2
				call gmSetCellAttribs(icyc_form(imodel), i, j, arrayattribs)
				call gmSetListEntry(ictype2,GSELECT,gmString=text_box(j,i+1))
			else
				call gmSetCellSetting(icyc_form(imodel), i,j ,gmString=text_box(j,i))
			endif
			else
   ...
   ivb=gmCreatePushButton(itwin1,ix/3,0, ix/3, 24,'All cycle obey MR',&
              	gmType=GSTANDARD, gmSize=80,gmTextCol=0, &
              	gmVpos=Gbottom, gmHpos=Gleft,gmExpand=GOFF,gmCallback=2011)
   ivb=gmCreatePushButton(itwin1,2*ix/3,0, ix/3, 24,'No cycle obey MR',&
              	gmType=GSTANDARD, gmSize=80,gmTextCol=0, &
