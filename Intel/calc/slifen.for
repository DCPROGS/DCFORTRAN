...
c      if(pon) write(7,200) M,I,J,PVAL,PEQ(I),Q(i,j),S
c      if(discprt) write(7,200) M,I,J,PVAL,PEQ(I),Q(i,j),S
c200	FORMAT(3I4,4G13.6)
20	CONTINUE	!END OF LIFE CALC
C TYPE SUBSET OCC AND LIFETIME
	IF(M.EQ.1) then
		print 212,PVAL,1.E3/S
      	if(pon) write(7,212) PVAL,1.E3/S
      	if(discprt) write(7,212) PVAL,1.E3/S
c212		FORMAT(/,'    A ',12X,2G13.6)
212		FORMAT('    A ',12X,G13.6,1x,g13.6)	!no CR after title
	else if(M.EQ.2) then
		print 213,PVAL,1.E3/S
      	if(pon) write(7,213) PVAL,1.E3/S
      	if(discprt) write(7,213) PVAL,1.E3/S
c213		FORMAT(/,'    B ',12X,2G13.6)
213		FORMAT('    B ',12X,G13.6,1x,g13.6)	!no CR after title
	else if(M.EQ.3) then
		print 214,PVAL,1.E3/S
	      if(pon) write(7,214) PVAL,1.E3/S
	      if(discprt) write(7,214) PVAL,1.E3/S
214		FORMAT(/,'    C ',12X,G13.6,1x,g13.6)
	else if(M.EQ.4) then
		print 215,PVAL,1.E3/S
	      if(pon) write(7,215) PVAL,1.E3/S
      	if(discprt) write(7,215) PVAL,1.E3/S
...
C TYPE INDIV STATE NO,OCC AND LIFETIME within each subset
	...
	print 17,I,inew(i),PEQ(I),-1.E3/Q(i,i),1.e3*tim(i)
      if(pon) write(7,17) I,inew(i),PEQ(I),-1.E3/Q(i,i),1.e3*tim(i)
16    if(discprt) write(7,17) I,inew(i),PEQ(I),-1.E3/Q(i,i),1.e3*tim(i)
