	subroutine DETERM2(A,n,ndim,det,ndscale)
c
c Alternative routine for determinant of a matrix using LU decomposition
c routine LUDCMP.FOR from Numerical Recipes
c ndscale=number of factors of 10^10 by which determinant has to scaled
c to prevent overflow/underflow
c
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)

	real*8 A(ndim,ndim)
	allocatable::A1
	real*8 A1(:,:)
	
	integer indx(n)
c
	
	if(n.eq.1) then
	   det=A(1,1)
	   ndscale=0
	   RETURN
	endif
	allocate(A1(n,n))		!so input A not destroyed
	TINY=1.0d-157
	do i=1,n
	   do j=1,n
		A1(i,j)=A(i,j)
	   enddo
	enddo
	ndscale=0
	call LUDCMP(A1,n,n,indx,D)
	if(dabs(d).le.tiny) then
	   det=0.0d0
	   RETURN
	endif
	
	do i=1,n
		if(dabs(A1(i,i)).le.tiny) then
			det=0.0d0
			RETURN
	    endif
c overflow check
1	   continue
	   di=dlog(dabs(D))
	   
	   ai=dlog(dabs(A1(i,i)))
	   
	   if(di+ai.gt.308.d0) then	!mult below would overflow
		ndscale=ndscale+1
		d=d*1.d-10
		goto 1	!until OK
	   endif
c Underflow check
2	   continue
		di=dlog(dabs(D))
	   
	   ai=dlog(dabs(A1(i,i)))
	   
	   if(di+ai.lt.-308.d0) then	!mult below would overflow
		ndscale=ndscale-1
		d=d*1.d10
		goto 2	!until OK
	   endif
c The actual loop
	   D=D*A1(i,i)
	enddo
c
	deallocate(A1)
	DET=d
	return
	end


      SUBROUTINE LUDCMP(A,N,NP,INDX,D)
c Double precision version of Numerical Recipes routine
	IMPLICIT DOUBLE PRECISION (A-H,O-Z)
c      PARAMETER (NMAX=100,TINY=1.0E-20)
      PARAMETER (NMAX=200,TINY=1.0d-157)
      DIMENSION A(NP,NP),INDX(N),VV(NMAX)
      nerr=0
	D=1.0d0
      DO 12 I=1,N
        AAMAX=0.0d0
        DO 11 J=1,N
          IF (dABS(A(I,J)).GT.AAMAX) AAMAX=dABS(A(I,J))
11      CONTINUE
c        IF (AAMAX.EQ.0.) PAUSE 'Singular matrix.'
	IF (dabs(AAMAX).le.tiny) then
c		PAUSE 'Singular matrix.'
		nerr=1
		RETURN
	  endif
        VV(I)=1./AAMAX
12    CONTINUE
      DO 19 J=1,N
        IF (J.GT.1) THEN
          DO 14 I=1,J-1
            SUM=A(I,J)
            IF (I.GT.1)THEN
              DO 13 K=1,I-1
                SUM=SUM-A(I,K)*A(K,J)
13            CONTINUE
              A(I,J)=SUM
            ENDIF
14        CONTINUE
        ENDIF
        AAMAX=0.d0
        DO 16 I=J,N
          SUM=A(I,J)
          IF (J.GT.1)THEN
            DO 15 K=1,J-1
              SUM=SUM-A(I,K)*A(K,J)
15          CONTINUE
            A(I,J)=SUM
          ENDIF
          DUM=VV(I)*ABS(SUM)
          IF (DUM.GE.AAMAX) THEN
            IMAX=I
            AAMAX=DUM
          ENDIF
16      CONTINUE
        IF (J.NE.IMAX)THEN
          DO 17 K=1,N
            DUM=A(IMAX,K)
            A(IMAX,K)=A(J,K)
            A(J,K)=DUM
17        CONTINUE
          D=-D
          VV(IMAX)=VV(J)
        ENDIF
        INDX(J)=IMAX
        IF(J.NE.N)THEN
          IF(A(J,J).EQ.0.)A(J,J)=TINY
          DUM=1.d0/A(J,J)
          DO 18 I=J+1,N
            A(I,J)=A(I,J)*DUM
18        CONTINUE
        ENDIF
19    CONTINUE
      IF(n.gt.0.and.A(N,N).EQ.0.)A(N,N)=TINY
      RETURN
      END

