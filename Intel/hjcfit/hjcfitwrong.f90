...
case(-22) ! continue with reading files
			...
			if(itogrec.ne.3.and.itogrec.ne.4) then ! change new/old
				...
				!	if(iopt.gt.5.or.iopt.eq.3) then
					if(iopt.eq.7.or.iopt.eq.3) then
	...
	case(-220,-219) ! read new scan files
		...
		do i=1,nfileb(j)
			...
			if(idatyp.eq.7) then
			    text1='qub.dwt' 
			    dfilth='*.dwt'//char(124)//'QUB files (DWT)'//char(124)//'*.*'//char(124)//'All Files'
	...
	if(icalnext.eq.-45) then
		   ...
		   if(iopt.eq.2.or.iopt.eq.4.or.iopt.eq.6.or.iopt.eq.8) then !read new data or simulate
			    inipage=-500
				!!!! readini=.false.
...
8007            format(a9,i4,1x,a7,1x,i6,1x,a12,1x,f10.5,1x,a10,1x,i1,1x,a11,1x,i1,1x,i1,1x,f4.2,1x,i1,1x,f4.2)	
