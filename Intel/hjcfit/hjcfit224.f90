...
2	continue
	...
	!=====================================================================================
		...
		ntoglik=2
		itcall=-8
		text_tog(1)='Calculate likelihood surface'
		text_tog(2)='No option'
		call toggle_panel(Main,ITOGGLE,ittoglepanel,ntoglik,text_tog,intoggle,itcall,&
		valdat,idat,itogbutton)
...
90				format(/,'  Program HJCFIT Windows Version')
91				 format(/,' Copyright D. Colquhoun, I. Vais, University College London 2004',/,&
				' All rights reserved.(Compaq Visual Fortran/Gino version)',/,&
...
93				format(/,' DATEW of analysis = ',a20)
...
149			continue
		   ...
		   ! nlen=len    
			...
			if(idatyp.eq.1) then
				...
				exten='.EDE'
				iscan1=201	!signals .ede file (but not in file of course)
			else if(idatyp.eq.5) then
				iscan1=301	!signals .evl file (but not in file of course)
				exten='.EVL'
			endif
...
case(-22) ! continue with reading files
	...
	if(icalnext.eq.-45) then
		   ...
		   if(iopt.eq.2.or.iopt.eq.4) then !read new data or simulate
			    inipage=-500
				!!!! readini=.false.
				 if(iopt.eq.2) then
				callid=-152
		...
		if(autosim) then
			if(ksim.eq.-1) then
				...
				itinfoC=gmcreatetextentry(imainpanel,24,2,14,1,&
						'TO ABORTW FIT/SIMULATION: Press "ESC" or "Q" or "q"',100, GDISPLAY, &
             			gmType=GSTANDARD, gmFont=GDEFAULT, gmJustify=GLEFT, &
              			gmBack1Col=0, gmBack2Col=0, gmTextCol=2, &
              			gmVpos=GTOP, gmExpand=GOFF)
...
143			format(/,' Simulation of experiment no:',i5)
	...
	itinfos=gmcreatetextentry(imainpanel,1,2,10,1,&
			...
			call HJCSIM2(main,thtrue,tint0,ampl0,iprops0,conc,nintt,&
				irates,jrates,npars,nmax,nset,dgammas,nsims,ks,imainpanel,itinfos)
			do j=1,nset
			   ...
			   '*.*'//char(124)//'All Files'
				...
				IF(eFILE.ne.' ') then
					...
					call write_model(main,imod,models,indrat,ratcons,irecm0,efile,pfilem,&
					 igraph2,irec,theta0,nvdep,ifit,ncyc0)
					irecq=irecm0
					ifit=0
				endif
...
case(-119) ! resolution
	...
	fc,ffilt,trise,autosim,icall,val9)
	case(-154)
...
!===================================================================================	
		...
		xmin0,xmax0,ymin0,ymax0,itogglepanel,itext_entry,new_text_entry,iplot,iptype,rescalex,&
		isym,symsiz,combo1_11,combo1_12,&
...
!DC March 19 2007 add iptype to parameters, though not clear that it is defined at time of call
	...
	!  will be simpler if xmin, xmax (in log units) defined now
		if(iplot.eq.2.and.rescalex) then
				!shut times
			inext=1
			xmaxsav=xmax
			ymaxsav=ymax
...
!==========================================================================
	case(1001:2100) ! readini+readmec=true
		...
		iqtoggle,icell,iec50,nrows,ncube,cubedef,cubecyc,cubext,qt,liksurf)
		!&
...
case(3302) !simulate data
	...
	!		fc,ffilt,trise,autosim,icall)
			...
			iatext1,iatext2,nvalc,ivals,ivalsim,ligname)
			sim=simulat.or.autosim
			else
...
210		format(' Results of simulation will be written as text to ',a40,/,&
	...
	case(3040)
		...
		stpfac,nintt,avamp,rms,ffilt,trise,fc,iscan,readini,ndev,idest,&
	...
	!	idest=0
		...
		cjump,nsweep,tzero,tsamp,autosim,ipatch,ptype,temp,tedit1,tedit2,&
		tedit3,tedit4,tval1,tval2,tval3,tval4,tval5,tval6,tval7,tval8,&
		tval9,tvalt,tedit5,val9,stpfac)
		endif
	case(3031) 
...
3311	    		format(/,' Set ',i3,': bad sections (from stability plot) are NOT omitted',/)
	...
	CASE(3100)!(9700) ! set reso,concentration
		if(iopt.eq.1.and.(itogrec.eq.5.or.itogrec.eq.6).and.icallprev.ne.-152) then
			callid=3045
			goto 2
		else if(autosim.and.itogrec.eq.8) then
			callid=3045
			goto 2	 
		else
!		 if(autosim) onechan=.false.
...
! Modified 21-04-07, remove covar etc from arguments
	call fitting(callid,igraph2,main,imainpanel,ivwin,iynwin,Form1,Form1_TxtArray1,thetaf,jfix1,tint,ampl,iprops,&
...
case(4450) ! calculate errors
	...
	itype(1)=3
	val(1)=0.1
	text_tog(1)=' For Hessian use step size that decreases log(lik) by'
	
	call val_table(Main,ivwin,1,text_tog,ival,val,4451,itype,iplot_Toggle0)
...
31		format(/,23x,'  true        initial        final      approx SD       CV(%)')
	   endif
	   do m=1,npar
		i=irate(m)
		j=jrate(m)
...
case(4459) 
		...
		if(istat1.eq.gon) then	 
		 	itype(1)=3
			val(1)=0.1
			text_tog(1)=' For Hessian use step size that decreases log(lik) by delta'	
			call val_table(Main,ivwin,1,text_tog,ival,val,4455,itype,iplot_Toggle0)		
		else
			callid=4456
			goto 2
		endif
...
67       format(/,'   concentration of ',a10,' = ',g13.6,/)
    ...
    XAF,XFA,QexpQA,QexpQF,alpha2,beta2,aloglik,kab,kA,kF,km,irt,npar,IQf,irate1,jrate1,nlig,&
    EXPQF,EXPQA,qt,qd)
    if(iplot.eq.6) then
	   iopth=1	
	   hjmcalc=.false.
...
2121 continue
	...
	ALLOCATE(w(kwi,kwj),weight(kwi,kwj))      
	ALLOCATE(ndat(ndimd),isym(ndimd),ijoin(ndimd),symsiz(ndimd))
...
413	FORMAT(/,' Mean and SD of ',i5,' values= ',G13.6,' +/-',g13.6,&
	...
	if(ioptlog.ge.3) then
		...
		text_tog(50)=' Histogram settings:'
	if(ioptlog.eq.1) then
	     
	    itypeval(1)=0   
		if(sbin) then
			val(1)=xaxis(1)
			text_tog(1)=' Histogram to start at (ms)'
			...
			val(2)=nbdec
			text_tog(2)=' Number of bins/decade '
			val(3)=xendo
			text_tog(3)=' Last x value (ms) '
			ntog=3
			itypeval(1)=9  
			itypeval(2)=0
			itypeval(3)=9
			...
			if(sbin) then
				...
				val(1)=xaxis(1)
				text_tog(1)=' Histogram to start at (ms)'
				text_tog(2)=' Number of different bin widths (-1 to skip histo)'
				itypeval(2)=0
				itypeval(1)=9
				val(2)=1
				val(1)=xaxis(1)
	...
	call value_table(Main,ivwin,ntog,text_tog,ival,val,4661,itypeval)
case(4661)
call gmDefineKeySelectCallback(13,0)
	do i=1,ntog
		val(i)=gmenqvaluesetting(ival(i))
...
19	continue
	...
	if(burst(iset).and.iplot.eq.2) then
	   ...
	   if(rescalex) then
! if xmax used here must take antilog because log(xmax taken above at
!	if(logt) then ...   xmin=0.010001
...
! Define Ycal1(i,1), xcal1(i,1), i=1,nrange -
	   ...
	   call MOPADJ(tres,ylo,yhi,nrange,QD,k,kA,kF,&
     	...
     	QEXPQA,QEXPQF,Z00F,Z10F,Z11F,XFA,kAm,kFm,km,EXPQF)
...
! Define xcal1(i,j1) in ms for calc curve (equally spaced on log scale)
      ...
      QEXPQA,QEXPQF,Z00A,Z10A,Z11A,Z00F,Z10F,Z11F,XFA,kAm,kFm,km,EXPQF)
...
case(4900) ! 3D plot
	...
	if(id2.gt.0) then
		...
		call gmDefineKeySelectCallback(13,4905)
		  call value_table(Main,ivwin,5,text_tog,ival,val,4905,itypeval)
	endif
case(4905)! 3d plot
call gmDefineKeySelectCallback(13,0)
		do i=1,5
...
call gmDefineKeySelectCallback(13,0)
	   ...
	   call CDIST3D(iplotype,tintset,amplset,ipropsset,nint(iset),nintt(iset),ndimy,iscan,treso,tresg,tres,idsav,&
        ...
        fill,inter,axis,fillbad,autplt,cross,iplot)
		modplot=igraph
		ngraph=igraph
...
!	   ymax=10000.*ymin
	   ...
	   enddo
		itypeval(5)=0
		text_tog(50)='Fitted dependency plot'
...
8071		            format(' Values logarithmically spaced')
	            ...
	            text_tog(5)='Exit'
	            call toggle_panel(Main,ITOGGLE,ittoglepanel,ntoglik,text_tog,intoggle,itcall,valdat,idat,itogbutton)
            endif
